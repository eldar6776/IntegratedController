 ...
 *
 * @note
 * Ovaj fajl sadrÅ¾i kompletnu logiku za iscrtavanje svih ekrana definisanih
 * u `eScreen` enumu. Koristi STemWin (emWin) grafiÄku biblioteku za
 * kreiranje i upravljanje widgetima (dugmad, spinbox-ovi, itd.).
 *
 * Glavna servisna petlja `DISP_Service()` se poziva iz `main.c` i funkcioniÅ¡e
 * kao state-machine, pozivajuÄ‡i odgovarajuÄ‡u `Service_...Screen()` funkciju
 * u zavisnosti od trenutno aktivnog ekrana.
 *
 * Funkcija `PID_Hook()` je centralna taÄka za obradu korisniÄkog unosa
 * (dodira na ekran) i prosljeÄ‘uje dogaÄ‘aje odgovarajuÄ‡im handlerima.
 *
 * Fajl takoÄ‘e sadrÅ¾i logiku za internacionalizaciju (prevoÄ‘enje tekstova)
 * i upravljanje pozadinskim procesima kao Å¡to je screensaver.
 *
 * --- VodiÄ za dodavanje novog ekrana ---
 *
 ...
 * jedinstveni naziv za VaÅ¡ novi ekran (npr. `SCREEN_NOVI_EKRAN`).
 *
 ...
 * i iscrtava statiÄke elemente ekrana. Poziva se samo jednom pri ulasku.
 ...
 * - `DSP_KillNoviEkranScreen(void)`: UniÅ¡tava sve widgete kreirane u `Init`
 * funkciji. KljuÄna je za sprjeÄavanje curenja memorije.
 ...
 * novi `case` za VaÅ¡ ekran koji poziva njegovu `Service` funkciju:
 * `case SCREEN_NOVI_EKRAN: Service_NoviEkranScreen(); break;`
 *
 * 4.  **Upravljanje Unosom (Touch Events) (display.c):**
 * - Ako VaÅ¡ ekran ima interaktivne zone koje nisu widgeti (npr. crtani
 ...
 * - Funkciju za obradu otpuÅ¡tanja (`HandleRelease_...`) potrebno je
 * implementirati samo u specifiÄnim sluÄajevima:
 * a) Kada treba razlikovati kratak od dugog pritiska.
 * b) Za "drag-and-drop" logiku.
 * c) Za akcije koje se izvrÅ¡avaju tek nakon otpuÅ¡tanja prsta.
 *
 * 5.  **Pravilno "ÄŒiÅ¡Ä‡enje" Ekrana (display.c):**
 * - Da bi se VaÅ¡ ekran pravilno ugasio i oslobodio resurse, poziv za
 * njegovu `Kill` funkciju morate dodati na dva mjesta:
 * a) U `PID_Hook()`: Unutar `switch` bloka koji obraÄ‘uje pritisak na
 ...
 * obraÄ‘uje istek screensaver tajmera, dodajte
 ...
 * @{
 */
#define GUI_REFRESH_TIME                50U     ///< Svrha: Period osvjeÅ¾avanja GUI-ja. Vrijednost: 100 milisekundi (10 puta u sekundi).
#define DATE_TIME_REFRESH_TIME          1000U   ///< Svrha: Period osvjeÅ¾avanja prikaza datuma i vremena. Vrijednost: 1000 milisekundi (svake sekunde).
#define SETTINGS_MENU_ENABLE_TIME       3456U   ///< Svrha: Vrijeme drÅ¾anja pritiska za ulazak u meni. Vrijednost: 3456 milisekundi (~3.5 sekunde).
...
#define VALUE_STEP_TOUT                 15      ///< Svrha: Brzina promjene vrijednosti kod drÅ¾anja dugmeta (npr. dimovanje). Vrijednost: 15 milisekundi.
...
#define FW_UPDATE_BUS_TIMEOUT           15000U  ///< Svrha: Vrijeme (u ms) nakon kojeg smatramo da je FW update na busu zavrÅ¡en ako nema novih paketa.
...
#define QR_CODE_COUNT                   2       ///< Svrha: Ukupan broj QR kodova koje sistem podrÅ¾ava. Vrijednost: 2 (jedan za WiFi, jedan za App).
#define QR_CODE_LENGTH                  50      ///< Svrha: Maksimalna duÅ¾ina stringa za QR kod. Vrijednost: 50 karaktera.
#define DRAWING_AREA_WIDTH              380     ///< Svrha: Å irina glavnog podruÄja za crtanje. Vrijednost: 380 piksela (cijeli ekran je 480px).
#define COLOR_BSIZE                     28      ///< Svrha: VeliÄina `clk_clrs` niza. Vrijednost: 28, mora odgovarati broju boja u nizu.
/** @} */

/** @name Definicije za ikonice svjetala
 * @note PremjeÅ¡teno iz lights.h, privatno za display modul.
 * @{
 */
#define LIGHT_ICON_COUNT                10       ///< Svrha: Ukupan broj razliÄitih tipova ikonica za svjetla. Vrijednost: 2 (sijalica i ventilator).
...
#define MAX_PIN_LENGTH 8    // << NOVO: Maksimalna duÅ¾ina PIN-a

/******************************************************************************
 * @brief       Definicije za raspored alfanumeriÄke tastature.
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Ove konstante se koriste za dinamiÄko kreiranje tastature
...
/** @name Bazni ID-jevi za dinamiÄke widgete
 * @note Koriste se u petljama za kreiranje widgeta kao poÄetna vrijednost.
 * @{
 */
#define ID_CurtainsRelay                0x894   ///< Svrha: PoÄetni ID za widgete zavjesa. Vrijednost: 0x894 (Heksadecimalni broj).
#define ID_LightsModbusRelay            0x8B3   ///< Svrha: PoÄetni ID za widgete svjetala. Vrijednost: 0x8B3 (Heksadecimalni broj).
/** @} */

/** @name ID-jevi za QR kodove
 * @{
 */
#define QR_CODE_WIFI_ID                 1       ///< Svrha: LogiÄki ID za WiFi QR kod. Vrijednost: 1.
#define QR_CODE_APP_ID                  2       ///< Svrha: LogiÄki ID za App QR kod. Vrijednost: 2.
/** @} */
...
#define CLR_LEMON                       GUI_MAKE_COLOR(0x00d6d3)  ///< Svrha: Definicija limun Å¾ute boje.

/*============================================================================*/
/* PRIVATNE STRUKTURE I DEKLARACIJE VARIJABLI                                 */
/*============================================================================*/
/**
 * @brief DefiniÅ¡e stanja za state-machine unutar Numpad-a prilikom promjene PIN-a.
 */
typedef enum {
    PIN_CHANGE_IDLE,            /**< Numpad se koristi za standardne operacije (ne za promjenu PIN-a). */
    PIN_CHANGE_WAIT_CURRENT,    /**< ÄŒeka se unos trenutnog (starog) PIN-a. */
    PIN_CHANGE_WAIT_NEW,        /**< ÄŒeka se unos novog PIN-a. */
    PIN_CHANGE_WAIT_CONFIRM     /**< ÄŒeka se ponovni unos novog PIN-a radi potvrde. */
} PinChangeState_e;
/**
 ******************************************************************************
 * @brief       DefiniÅ¡e modove rada za viÅ¡enamjenski ekran za odabir scene.
 * @note        Ovaj enum omoguÄ‡ava da se isti ekran (`SCREEN_SCENE_APPEARANCE`)
 * koristi u razliÄitim kontekstima unutar aplikacije.
 ******************************************************************************
 */
typedef enum 
{
    SCENE_PICKER_MODE_WIZARD,   /**< Mod za "Äarobnjaka": prikazuje samo neiskoriÅ¡tene izglede za kreiranje nove scene. */
    SCENE_PICKER_MODE_TIMER     /**< Mod za tajmer: prikazuje sve konfigurisane scene radi odabira akcije za alarm. */
} eScenePickerMode;
/******************************************************************************
 * @brief       Struktura koja definiÅ¡e kontekst za univerzalni numeriÄki keypad.
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Ova struktura se koristi za dinamiÄku konfiguraciju keypad-a.
 *****************************************************************************/
typedef struct
{
    const char* title;          /**< PokazivaÄ na string koji Ä‡e biti prikazan kao naslov. */
    char initial_value[12];     /**< PoÄetna vrijednost koja se prikazuje. */
...
/**
 * @brief StatiÄka, privatna instanca konteksta za Numpad.
 * @note  Vidljiva samo unutar display.c. Funkcije unutar ovog fajla Ä‡e
 * popunjavati ovu strukturu prije prebacivanja na ekran keypada.
 */
static NumpadContext_t g_numpad_context;

/******************************************************************************
 * @brief       Struktura koja sadrÅ¾i rezultat unosa sa numeriÄkog keypada.
 * @author      Gemini (po specifikaciji korisnika)
 *****************************************************************************/
typedef struct
{
    char    value[12];          /**< Bafer za konaÄnu unesenu vrijednost. */
    bool    is_confirmed;       /**< Fleg, 'true' ako je korisnik potvrdio unos. */
    bool    is_cancelled;       /**< Fleg, 'true' ako je korisnik otkazao unos. */
} NumpadResult_t;

/**
 * @brief StatiÄka, privatna instanca za rezultat unosa sa Numpad-a.
 * @note  SluÅ¾i za internu komunikaciju izmeÄ‘u logike keypada i logike
 * ekrana koji ga je pozvao (npr. SCREEN_SETTINGS_GATE).
 */
static NumpadResult_t g_numpad_result;
/**
 * @brief DefiniÅ¡e moguÄ‡e vizuelne statuse za UI alarma.
 */
typedef enum {
...
/**
 * @brief Niz koji Äuva TRENUTNO VIZUELNO stanje za svaku komponentu alarma.
 * @note Index 0 = Sistem, Index 1-3 = Particije 1-3.
 */
static AlarmUIState_e alarm_ui_state[SECURITY_PARTITION_COUNT + 1];
/******************************************************************************
 * @brief       Struktura koja definiÅ¡e kontekst za univerzalnu alfanumeriÄku tastaturu.
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Ova struktura se popunjava prije poziva funkcije
 * Display_ShowKeyboard() kako bi se tastatura prilagodila
 * specifiÄnoj potrebi (npr. unos imena svjetla).
 *****************************************************************************/
typedef struct
{
    const char* title;          /**< PokazivaÄ na string koji Ä‡e biti prikazan kao naslov. */
    char initial_value[32];     /**< String koji sadrÅ¾i poÄetnu vrijednost za editovanje. */
    uint8_t max_len;            /**< Maksimalan broj karaktera koji se mogu unijeti. */
} KeyboardContext_t;

/**
 * @brief StatiÄka, privatna instanca konteksta za alfanumeriÄku tastaturu.
 * @note  Vidljiva samo unutar display.c.
 */
static KeyboardContext_t g_keyboard_context;

/******************************************************************************
 * @brief       Struktura koja sadrÅ¾i rezultat unosa sa alfanumeriÄke tastature.
 * @author      Gemini (po specifikaciji korisnika)
 * @note        SluÅ¾i kao mehanizam za komunikaciju izmeÄ‘u tastature i ekrana
 * koji ju je pozvao, bez upotrebe callback funkcija.
 *****************************************************************************/
typedef struct
{
    char    value[32];          /**< Bafer u koji se upisuje konaÄni uneseni tekst. */
...
/**
 * @brief StatiÄka, privatna instanca za rezultat unosa sa tastature.
 * @note  Vidljiva samo unutar display.c.
 */
static KeyboardResult_t g_keyboard_result;

/**
 * @brief Matrica koja sadrÅ¾i rasporede tastera za sve podrÅ¾ane jezike.
 * @note  Struktura: [JEZIK][SHIFT_STANJE][RED][TASTER]
 * Ovo je "izvor istine" za iscrtavanje tastature. Funkcija
 * `DSP_InitKeyboardScreen` Ä‡e na osnovu odabranog jezika i
 * shift stanja odabrati odgovarajuÄ‡i set karaktera za ispis.
...
/**
 * @brief Definicija opÅ¡teg tipa za zonu dodira (pravougaonik).
 * @note  OlakÅ¡ava kreiranje i rad sa layout strukturama.
 */
typedef struct {
    int16_t x0, y0, x1, y1;
} TouchZone_t;
 ...
 * za jedan red u meniju za podeÅ¡avanje svjetala.
 */
typedef struct
{
    ...
    SPINBOX_Handle offTime;                 // Handle za spinbox za vrijeme automatskog gaÅ¡enja.
    ...
    CHECKBOX_Handle rememberBrightness;     // Handle za checkbox za pamÄ‡enje zadnje svjetline.
} LIGHT_settingsWidgets;

/**
 * @brief PomoÄ‡na struktura koja objedinjuje sve GUI widgete (handle-ove)
 * za meni za podeÅ¡avanje odmrzivaÄa (Defroster).
...
/**
 * @brief PomoÄ‡na struktura za definisanje pozicije i dimenzija widget-a.
 */
typedef struct 
{
    int16_t x, y, w, h;
...
/** 
*   brief PomoÄ‡na struktura za definisanje horizontalne linije. 
*/
typedef struct 
{
    int16_t y, x0, x1;
...
/**
 ******************************************************************************
 * @brief       Struktura koja sadrÅ¾i konstante za iscrtavanje "hamburger" menija.
 * @author      Gemini
 * @note        Centralizuje sve dimenzije i koordinate za gornju desnu i donju
 * lijevu ikonu kako bi se izbjegli "magiÄni brojevi" u kodu i
 * olakÅ¡alo buduÄ‡e odrÅ¾avanje. [cite: 63, 64, 65]
 ******************************************************************************
 */
static const struct
{
    ...
    struct {
        int16_t x_start; /**< PoÄetna X koordinata (lijevo). */
        int16_t y_start; /**< PoÄetna Y koordinata (gore). */
        int16_t width;   /**< Å irina linija. */
        int16_t y_gap;   /**< Vertikalni razmak izmeÄ‘u linija. */
    } top_right;
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za globalne elemente GUI-ja.
 * @note  Trenutno sadrÅ¾i samo zonu za hamburger meni, ali se moÅ¾e proÅ¡iriti.
 */
static const struct
{
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za raspored elemenata (layout)
 * na glavnom ekranu.
 */
static const struct
{
...
main_screen_layout =
{
    ...
    // NOVE VRIJEDNOSTI
    .time_pos_standard  = { 5, 245 },   // BivÅ¡e (5, 245)
    .time_pos_scrnsvr   = { 240, 136 },  // BivÅ¡e (240, 136)
...
static const struct
{
    TouchZone_t clean_zone;         /**< Zona za odabir menija za ÄiÅ¡Ä‡enje. */
...
{
    .clean_zone       = { .x0 = 0,   .y0 = 80, .x1 = 126, .y1 = 200 }, // Prva treÄ‡ina
    .wifi_zone        = { .x0 = 126, .y0 = 80, .x1 = 253, .y1 = 200 }, // Druga treÄ‡ina
    .app_zone         = { .x0 = 253, .y0 = 80, .x1 = 380, .y1 = 200 }, // TreÄ‡a treÄ‡ina
 ...
 * "magiÄni brojevi" u funkciji za iscrtavanje.
 */
static const struct
{
    int16_t items_per_row;      /**< Broj ikonica u jednom redu. */
    int16_t slot_width;         /**< Å irina jednog slota za ikonicu. */
...
/**
 * @brief Struktura koja sadrÅ¾i sve konstante za raspored elemenata (layout)
 * na ekranu termostata. Sve koordinate su na jednom mjestu radi lakÅ¡eg odrÅ¾avanja.
 */
static const struct
{
    TouchZone_t increase_zone;      /**< Zona za poveÄ‡anje temperature (+) */
    TouchZone_t decrease_zone;      /**< Zona za smanjenje temperature (-) */
    TouchZone_t on_off_zone;        /**< Zona za paljenje/gaÅ¡enje dugim pritiskom */
}
thermostat_layout =
{
...
static const struct
{
    int16_t icon_width;     /**< Å irina zone dodira za jednu ikonicu. */
    int16_t icon_height;    /**< Visina zone dodira za jednu ikonicu (ukljuÄujuÄ‡i tekst). */
}
...
static const struct
{
    ...
    TouchZone_t next_arrow_zone;      /**< Zona za strelicu SLJEDEÄ†A. */
}
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za raspored elemenata (layout)
 * na ekranu za detaljno podeÅ¡avanje svjetala (dimer i RGB).
 */
static const struct
{
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za raspored elemenata (layout)
 * na ekranu za resetovanje menija (ponaÅ¡a se kao glavni prekidaÄ).
 */
static const struct
{
    TouchZone_t main_switch_zone;  /**< Velika centralna zona koja se ponaÅ¡a kao glavni prekidaÄ. */
}
...
/**
 * @brief Struktura koja sadrÅ¾i sve konstante za ISCRTAVANJE elemenata
 * na prvom ekranu za odabir (Select Screen 1).
 * @note  Ova struktura je redizajnirana da podrÅ¾i dinamiÄki "Smart Grid" raspored.
 * SadrÅ¾i parametre za iscrtavanje rasporeda sa 1, 2, 3 ili 4 ikonice,
 * Äime se izbjegavaju "magiÄni brojevi" unutar `Service_SelectScreen1` funkcije.
 */
static const struct
{
    ...
    /** @brief Vertikalni razmak (u pikselima) izmeÄ‘u dna ikonice i vrha teksta ispod nje. */
    int16_t text_vertical_offset;
    /** @brief PoÄetna Y koordinata (vrh) za KRATKE vertikalne separatore. */
    ...
    /** @brief PoÄetna Y koordinata (vrh) za DUGAÄŒKI desni separator. */
    int16_t long_separator_y_start;
    /** @brief Krajnja Y koordinata (dno) za DUGAÄŒKI desni separator. */
    ...
    /** @brief DefiniÅ¡e kompletnu zonu dodira za "NEXT" dugme. */
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za ISCRTAVANJE i ZONE DODIRA
 * na drugom ekranu za odabir.
 * @note  Verzija 2.6.0: ZadrÅ¾ano originalno ime `select_screen2_drawing_layout`.
 * Redizajnirana da podrÅ¾i fiksni 2x2 raspored.
 */
static const struct
{
    /** @brief Zona dodira za gornji lijevi kvadrant (ÄŒiÅ¡Ä‡enje). */
    ...
    /** @brief Zona dodira za donji desni kvadrant (PodeÅ¡avanja). */
    ...
    int16_t text_vertical_offset;
    /** @brief PoÄetna Y koordinata za vertikalne separatore. */
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za ISCRTAVANJE elemenata
 * na prvom ekranu za podeÅ¡avanja (Termostat i Ventilator).
 */
static const struct
{
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za ISCRTAVANJE elemenata
 * na drugom ekranu za podeÅ¡avanja (Vrijeme, Datum, Screensaver).
 * @note  Verzija 3.0: SadrÅ¾i pozicije za SVE elemente bez izuzetka.
 */
static const struct
{
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za ISCRTAVANJE elemenata
 * na treÄ‡em ekranu za podeÅ¡avanja (Defroster, Ventilator).
 */
static const struct
{
    WidgetRect_t defroster_cycle_time_pos;      /**< @brief Pozicija i dimenzije za SPINBOX ciklusa odmrzivaÄa. */
    WidgetRect_t defroster_active_time_pos;     /**< @brief Pozicija i dimenzije za SPINBOX aktivnog vremena odmrzivaÄa. */
    WidgetRect_t defroster_pin_pos;             /**< @brief Pozicija i dimenzije za SPINBOX pina odmrzivaÄa. */
    ...
    WidgetRect_t ventilator_delay_off_pos;      /**< @brief Pozicija i dimenzije za SPINBOX odgode gaÅ¡enja ventilatora. */
    WidgetRect_t ventilator_trigger1_pos;       /**< @brief Pozicija i dimenzije za SPINBOX prvog okidaÄa ventilatora. */
    WidgetRect_t ventilator_trigger2_pos;       /**< @brief Pozicija i dimenzije za SPINBOX drugog okidaÄa ventilatora. */
    ...
    GUI_POINT label_ventilator_delay_off[2];    /**< @brief Pozicije za dvorednu labelu za odgodu gaÅ¡enja. */
    GUI_POINT label_ventilator_trigger1[2];     /**< @brief Pozicije za dvorednu labelu za okidaÄ 1. */
    GUI_POINT label_ventilator_trigger2[2];     /**< @brief Pozicije za dvorednu labelu za okidaÄ 2. */
    ...
    HLine_t   line_defroster_title;             /**< @brief Linija ispod naslova za odmrzivaÄ. */
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za ISCRTAVANJE elemenata
 * na Äetvrtom ekranu za podeÅ¡avanja (Roletne).
 * @note  DefiniÅ¡e pravila za dinamiÄko iscrtavanje mreÅ¾e 2x2.
 */
static const struct
{
    /** @brief PoÄetna taÄka (gore-lijevo) za prvi widget u mreÅ¾i. */
    GUI_POINT    grid_start_pos;
    /** @brief Å irina jednog SPINBOX widgeta. */
    int16_t      widget_width;
    /** @brief Visina jednog SPINBOX widgeta. */
    int16_t      widget_height;
    /** @brief Vertikalni razmak izmeÄ‘u "UP" i "DOWN" spinbox-a za istu roletnu. */
    int16_t      y_row_spacing;
    /** @brief Vertikalni razmak izmeÄ‘u grupa widgeta za razliÄite roletne. */
    int16_t      y_group_spacing;
    /** @brief Horizontalni razmak izmeÄ‘u prve i druge kolone widgeta. */
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za ISCRTAVANJE elemenata
 * na petom ekranu za podeÅ¡avanja (detaljne postavke za svjetla).
 * @note  DefiniÅ¡e pravila za iscrtavanje elemenata u dvije kolone.
 */
static const struct
{
    ...
    int16_t      col2_x;
    /** @brief PoÄetna Y pozicija za prvi red. */
    int16_t      start_y;
    /** @brief Vertikalni razmak (visina reda) izmeÄ‘u widgeta. */
    int16_t      y_step;
 ...
 ******************************************************************************
 * @brief       Struktura koja sadrÅ¾i konstante za ISCRTAVANJE elemenata
 * na Å¡estom ekranu za podeÅ¡avanja (OpÅ¡te postavke).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        AÅ½URIRANA VERZIJA: Uklonjen je `enable_scenes_checkbox_pos` jer
...
static const struct
{
    WidgetRect_t device_id_pos;                 /**< @brief Pozicija i dimenzije za SPINBOX ID-a ureÄ‘aja. */
    WidgetRect_t curtain_move_time_pos;         /**< @brief Pozicija i dimenzije za SPINBOX vremena kretanja roletni. */
    WidgetRect_t leave_scrnsvr_checkbox_pos;    /**< @brief Pozicija i dimenzije za CHECKBOX ponaÅ¡anja screensaver-a. */
    WidgetRect_t night_timer_checkbox_pos;      /**< @brief Pozicija i dimenzije za CHECKBOX noÄ‡nog tajmera za svjetla. */
    WidgetRect_t enable_security_checkbox_pos;  /**< @brief Pozicija i dimenzije za CHECKBOX koji omoguÄ‡ava/onemoguÄ‡ava security modul. */
    ...
    GUI_POINT    device_id_label_pos[2];        /**< @brief Pozicije za dvorednu labelu za ID ureÄ‘aja. */
    ...
    WidgetRect_t select_control_1_pos;          /**< @brief Pozicija i dimenzije za DROPDOWN za dinamiÄku ikonu 1. */
    GUI_POINT    select_control_1_label_pos;    /**< @brief Pozicija za labelu za dinamiÄku ikonu 1. */
    WidgetRect_t select_control_2_pos;          /**< @brief Pozicija i dimenzije za DROPDOWN za dinamiÄku ikonu 2. */
    GUI_POINT    select_control_2_label_pos;    /**< @brief Pozicija za labelu za dinamiÄku ikonu 2. */
}
settings_screen_6_layout =
{
    ...
    .enable_security_checkbox_pos = { 10, 165, 240, 20 }, // Postavljamo novu kontrolu na logiÄnu poziciju
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za ISCRTAVANJE elemenata
 * na sedmom ekranu za podeÅ¡avanja (Scene Backend).
 */
static const struct
{
    WidgetRect_t enable_scenes_checkbox_pos;    /**< @brief Pozicija i dimenzije za CHECKBOX koji omoguÄ‡ava/onemoguÄ‡ava sistem scena. */
    GUI_POINT    grid_start_pos;                /**< @brief PoÄetna taÄka (gore-lijevo) za prvi widget u mreÅ¾i okidaÄa. */
    int16_t      widget_width;                  /**< @brief Å irina jednog SPINBOX widgeta. */
    int16_t      widget_height;                 /**< @brief Visina jednog SPINBOX widgeta. */
    int16_t      y_spacing;                     /**< @brief Vertikalni razmak izmeÄ‘u redova. */
    int16_t      x_col_spacing;                 /**< @brief Horizontalni razmak izmeÄ‘u kolona. */
 ...
 ******************************************************************************
 * @brief       Struktura koja sadrÅ¾i konstante za ISCRTAVANJE elemenata
 * na osmom ekranu za podeÅ¡avanja (Kapije).
 * @author      Gemini
 * @note        Ova struktura je namjerno i u potpunosti identiÄna kopija
 * strukture `settings_screen_5_layout` kako bi se osigurala
 * apsolutna vizuelna i funkcionalna konzistentnost sa ekranom
 * za podeÅ¡avanje svjetala, u skladu sa projektnim zahtjevom.
 ******************************************************************************
 */
static const struct
{
...
/**
 ******************************************************************************
 * @brief       Struktura koja sadrÅ¾i konstante za raspored elemenata na
 * ekranu za podeÅ¡avanje alarma (`SCREEN_SETTINGS_9`).
 ******************************************************************************
 */
static const struct
{
    GUI_POINT    start_pos;         /**< @brief PoÄetna gornja-lijeva pozicija za prvu grupu kontrola. */
    WidgetRect_t spinbox_size;      /**< @brief Dimenzije za svaki SPINBOX. */
    int16_t      y_group_spacing;   /**< @brief Vertikalni razmak izmeÄ‘u grupa (particija). */
    int16_t      x_col_spacing;     /**< @brief Horizontalni razmak izmeÄ‘u kolona. */
}
...
/**
 * @brief Struktura koja sadrÅ¾i konstante za raspored elemenata (layout)
 * na ekranu za podeÅ¡avanje datuma i vremena.
 */
static const struct
{
    // Definicija mreÅ¾e (grid) za pozicioniranje grupa
    ...
    int16_t x_col_3;        /**< X koordinata treÄ‡e kolone (Godina). */

    // Dimenzije i razmaci za +/- dugmad
    int16_t btn_size;       /**< VeliÄina (Å¡irina i visina) kvadratnih +/- dugmadi. */
    int16_t btn_pair_gap_x; /**< Horizontalni razmak izmeÄ‘u '+' i '-' dugmeta u paru. */
    ...
    int16_t ok_btn_width;   /**< Å irina 'OK' dugmeta. */
...
static const struct
{
    // Elementi za prikaz kada VRIJEME NIJE PODEÅ ENO
    GUI_POINT   datetime_icon_pos;      /**< TaÄna pozicija za ikonicu 'podesi vrijeme'. */
    GUI_POINT   datetime_text_pos;      /**< TaÄna pozicija za tekst 'Podesite Datum i Vrijeme'. */

    // Elementi za prikaz kada JE VRIJEME PODEÅ ENO
    ...
    GUI_POINT   status_text_pos;        /**< Pozicija za ispis statusa (ukljuÄen/iskljuÄen). */
}
timer_screen_layout =
{
    ...
    .toggle_icon_pos    = { 0, 180 }, // X se raÄuna dinamiÄki
...
static const struct
{
    ...
    int16_t     time_hour_width;        /**< Å irina kvadrata za HH. */
    GUI_POINT   time_colon_pos;         /**< Pozicija za ispis dvotaÄke (:). */
    int16_t     time_colon_width;       /**< Å irina kvadrata za :. */
    GUI_POINT   time_minute_pos;        /**< Pozicija za ispis minuta (MM). */
    int16_t     time_minute_width;      /**< Å irina kvadrata za MM. */

    // Dugmad za podeÅ¡avanje vremena
    ...
    int16_t     time_btn_size;          /**< VeliÄina (Å¡irina i visina) +/- dugmadi. */

    // Odabir dana
    GUI_POINT   day_labels_pos;         /**< Pozicija za ispis skraÄ‡enica dana (PON, UTO...). */
    GUI_POINT   day_checkbox_start_pos; /**< PoÄetna pozicija za prvi checkbox (ponedjeljak). */
    int16_t     day_checkbox_gap_x;     /**< Horizontalni razmak izmeÄ‘u checkbox-ova. */
...
/**
 ******************************************************************************
 * @brief       SadrÅ¾i konstante za raspored elemenata u dinamiÄkoj mreÅ¾i
 * ikonica (koristi se za ekrane Svjetla i Kapije).
 * @author      Gemini
 * @note        Ova struktura centralizuje sve parametre za layout, omoguÄ‡avajuÄ‡i
 * lako i precizno podeÅ¡avanje izgleda bez izmjene koda funkcija.
 ******************************************************************************
 */
static const struct
{
    /** @brief PoÄetna Y pozicija za iscrtavanje kada je prikazan samo JEDAN red ikonica. */
    int16_t y_start_pos_single_row;

    /** @brief PoÄetna Y pozicija za iscrtavanje kada su prikazana DVA reda ikonica. */
    int16_t y_start_pos_multi_row;

    /** @brief Ukupna visina jednog reda. Koristi se za izraÄunavanje centra reda i pozicije drugog reda. */
    int16_t row_height;

    /** @brief Vertikalni razmak u pikselima izmeÄ‘u teksta i ikonice. */
...
static const struct
{
    GUI_POINT    start_pos;         /**< @brief PoÄetna gornja-lijeva pozicija za prvo dugme. */
    int16_t      button_size;       /**< @brief VeliÄina (Å¡irina i visina) kvadratnog dugmeta. */
    int16_t      y_spacing;         /**< @brief Vertikalni razmak izmeÄ‘u redova. */
    int16_t      label_x_offset;    /**< @brief Horizontalni razmak od dugmeta do poÄetka teksta labele. */
}
...
{
    // 1. Privremeno redefiniÅ¡emo makro WIDGET da vraÄ‡a samo prvi argument (ID)
#define WIDGET(id, val, comment) id,
    // 2. UkljuÄujemo naÅ¡u glavnu listu
#include "settings_widgets.def"
    // 3. Odmah poniÅ¡tavamo naÅ¡u privremenu definiciju da ne bi smetala ostatku koda
#undef WIDGET
};

/**
 * @brief Niz sa pokazivaÄima na bitmape za ikonice svjetala.
 * @note  IZMIJENJENO: Redoslijed u ovom nizu sada MORA TAÄŒNO ODGOVARATI
...
/**
 * @brief Niz sa pokazivaÄima na bitmape ISKLJUÄŒIVO za ikonice scena.
 * @note  Redoslijed u ovom nizu MORA taÄno odgovarati redoslijedu enum-a
 * koji poÄinje sa ICON_SCENE_WIZZARD u fajlu display.h.
 */
...
/**
 ******************************************************************************
 * @brief       Niz sa pokazivaÄima na bitmape za sve tipove i stanja kapija.
 * @author      Gemini
 * @note        Ovaj niz je organizovan u blokove od po 5 ikonica za svaki osnovni
 * tip ureÄ‘aja definisan u `IconID` enumu (poÄevÅ¡i od `ICON_GATE_SWING`).
 * Redoslijed unutar svakog bloka je: ZATVORENO, OTVORENO, OTVARANJE, ZATVARANJE, DJELIMIÄŒNO.
 * AÅ¾urirano: Dodani su blokovi za SIGURNOSNA VRATA i PODZEMNU RAMPU.
 ******************************************************************************
 */
static GUI_CONST_STORAGE GUI_BITMAP* gate_icon_images[] = 
{
    ...
    // Blok za ICON_GATE_GARAGE (GaraÅ¾na vrata) - 5 STANJA
    ...
    // === POÄŒETAK ISPRAVKE: DODAVANJE BLOKOVA KOJI NEDOSTAJU ===
    ...
    // KORISTIMO IKONICE ZA OBIÄŒNU RAMPU KAO ZAMJENU
    &bmicons_gate_ramp_closed,
    &bmicons_gate_ramp_open,
    &bmicons_gate_ramp_opening,
    &bmicons_gate_ramp_closing,
...
/** @name ZajedniÄki Handle-ovi za Navigaciju i Akcije
 * @brief Handle-ovi za dugmad koja se koriste na viÅ¡e ekrana za podeÅ¡avanja.
 * @{
 */
static BUTTON_Handle   hBUTTON_Ok;             /**< @brief Handle za dugme "OK" ili "SAVE" za potvrdu i snimanje izmjena. */
static BUTTON_Handle   hBUTTON_Next;           /**< @brief Handle za dugme "NEXT" za prelazak na sljedeÄ‡i ekran u sekvenci podeÅ¡avanja. */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Sistema (SCREEN_SETTINGS_6)
 * @{
 */
static BUTTON_Handle   hBUTTON_SET_DEFAULTS;   /**< @brief Handle za dugme "SET DEFAULTS" za vraÄ‡anje na fabriÄke postavke. */
static BUTTON_Handle   hBUTTON_SYSRESTART;     /**< @brief Handle za dugme "RESTART" za softverski restart ureÄ‘aja. */
static SPINBOX_Handle  hDEV_ID;                /**< @brief Handle za SPINBOX za podeÅ¡avanje adrese ureÄ‘aja na RS485 busu. */
static DROPDOWN_Handle hDRPDN_Language;        /**< @brief Handle za DROPDOWN za odabir jezika korisniÄkog interfejsa. */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Termostata (SCREEN_SETTINGS_1)
 * @{
 */
static RADIO_Handle    hThstControl;           /**< @brief Handle za RADIO grupu za odabir moda rada termostata (Grijanje/HlaÄ‘enje/Off). */
static RADIO_Handle    hFanControl;            /**< @brief Handle za RADIO grupu za odabir moda rada ventilatora (ON-OFF / 3 Brzine). */
static SPINBOX_Handle  hThstMaxSetPoint;       /**< @brief Handle za SPINBOX za podeÅ¡avanje maksimalne dozvoljene temperature. */
static SPINBOX_Handle  hThstMinSetPoint;       /**< @brief Handle za SPINBOX za podeÅ¡avanje minimalne dozvoljene temperature. */
static SPINBOX_Handle  hFanDiff;               /**< @brief Handle za SPINBOX za podeÅ¡avanje temperaturne razlike (histereze) za ventilator. */
...
static SPINBOX_Handle  hThstGroup;             /**< @brief Handle za SPINBOX za podeÅ¡avanje ID-ja grupe termostata. */
static CHECKBOX_Handle hThstMaster;            /**< @brief Handle za CHECKBOX za postavljanje ureÄ‘aja kao Master u grupi. */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Ekrana i Vremena (SCREEN_SETTINGS_2)
 * @{
 */
static SPINBOX_Handle  hSPNBX_DisplayHighBrightness;   /**< @brief Handle za SPINBOX za podeÅ¡avanje visoke (radne) svjetline ekrana. */
static SPINBOX_Handle  hSPNBX_DisplayLowBrightness;    /**< @brief Handle za SPINBOX za podeÅ¡avanje niske (screensaver) svjetline ekrana. */
static SPINBOX_Handle  hSPNBX_ScrnsvrTimeout;          /**< @brief Handle za SPINBOX za podeÅ¡avanje vremena neaktivnosti za ulazak u screensaver. */
...
static CHECKBOX_Handle hCHKBX_ScrnsvrClock;            /**< @brief Handle za CHECKBOX za omoguÄ‡avanje prikaza sata na screensaveru. */
static SPINBOX_Handle  hSPNBX_Hour;                    /**< @brief Handle za SPINBOX za podeÅ¡avanje sata (RTC). */
static SPINBOX_Handle  hSPNBX_Minute;                  /**< @brief Handle za SPINBOX za podeÅ¡avanje minute (RTC). */
static SPINBOX_Handle  hSPNBX_Day;                     /**< @brief Handle za SPINBOX za podeÅ¡avanje dana u mjesecu (RTC). */
static SPINBOX_Handle  hSPNBX_Month;                   /**< @brief Handle za SPINBOX za podeÅ¡avanje mjeseca (RTC). */
static SPINBOX_Handle  hSPNBX_Year;                    /**< @brief Handle za SPINBOX za podeÅ¡avanje godine (RTC). */
static DROPDOWN_Handle hDRPDN_WeekDay;                 /**< @brief Handle za DROPDOWN za odabir dana u sedmici (RTC). */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Ventilatora i OdmrzivaÄa (SCREEN_SETTINGS_3)
 * @{
 */
...
static SPINBOX_Handle  hVentilatorDelayOff;        /**< @brief Handle za SPINBOX za vrijeme odgode gaÅ¡enja ventilatora. */
static SPINBOX_Handle  hVentilatorTriggerSource1;  /**< @brief Handle za SPINBOX za odabir prvog svjetla kao okidaÄa. */
static SPINBOX_Handle  hVentilatorTriggerSource2;  /**< @brief Handle za SPINBOX za odabir drugog svjetla kao okidaÄa. */
static SPINBOX_Handle  hVentilatorLocalPin;        /**< @brief Handle za SPINBOX za odabir lokalnog GPIO pina za ventilator. */
static Defroster_settingsWidgets defroster_settingWidgets; /**< @brief Struktura koja objedinjuje handle-ove za postavke odmrzivaÄa. */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Roletni (SCREEN_SETTINGS_4)
 * @{
 */
...
static SPINBOX_Handle  hCurtainsMoveTime;                 /**< @brief Handle za SPINBOX za podeÅ¡avanje globalnog vremena kretanja roletni. */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Svjetala (SCREEN_SETTINGS_5)
 * @{
 */
static LIGHT_settingsWidgets lightsWidgets[LIGHTS_MODBUS_SIZE]; /**< @brief Niz struktura, gdje svaka sadrÅ¾i sve handle-ove za podeÅ¡avanje jednog svjetla. */
static BUTTON_Handle   hButtonRenameLight;                      /**< @brief Handle za dugme za preimenovanje svjetla (pokreÄ‡e tastaturu). */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja OpÅ¡tih Opcija (SCREEN_SETTINGS_6)
 * @{
 */
static CHECKBOX_Handle hCHKBX_ONLY_LEAVE_SCRNSVR_AFTER_TOUCH; /**< @brief Handle za CHECKBOX za promjenu ponaÅ¡anja screensavera pri dodiru. */
static CHECKBOX_Handle hCHKBX_LIGHT_NIGHT_TIMER;    /**< @brief Handle za CHECKBOX za omoguÄ‡avanje noÄ‡nog tajmera za svjetla. */
static CHECKBOX_Handle hCHKBX_EnableSecurity;       /**< @brief NOVO: Handle za CHECKBOX za omoguÄ‡avanje security modula. */
static DROPDOWN_Handle hSelectControl_1;            /**< @brief Handle za DROPDOWN za dinamiÄku ikonu na SelectScreen1. */
static DROPDOWN_Handle hSelectControl_2;            /**< @brief Handle za DROPDOWN za dinamiÄku ikonu na SelectScreen2. */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Scena (SCREEN_SETTINGS_7 i Wizard)
 * @{
 */
static CHECKBOX_Handle hCHKBX_EnableScenes;         /**< @brief Handle za CHECKBOX za globalno omoguÄ‡avanje/onemoguÄ‡avanje sistema scena. */
static SPINBOX_Handle  hSPNBX_SceneTriggers[SCENE_MAX_TRIGGERS];  /**< @brief Niz handle-ova za SPINBOX-ove za adrese okidaÄa "Povratak" scene. */
static BUTTON_Handle   hButtonChangeAppearance;     /**< @brief Handle za dugme "[ Promijeni ]" unutar Äarobnjaka za scene. */
static BUTTON_Handle   hButtonDeleteScene;          /**< @brief Handle za dugme "[ ObriÅ¡i ]" za brisanje postojeÄ‡e scene. */
static BUTTON_Handle   hButtonDetailedSetup;        /**< @brief Handle za dugme "[ Detaljna PodeÅ¡avanja ]" za ulazak u "anketni" Äarobnjak. */
static CHECKBOX_Handle hCheckboxSceneLights;        /**< @brief Handle za Checkbox "Svjetla" u Äarobnjaku. */
static CHECKBOX_Handle hCheckboxSceneCurtains;      /**< @brief Handle za Checkbox "Roletne" u Äarobnjaku. */
static CHECKBOX_Handle hCheckboxSceneThermostat;    /**< @brief Handle za Checkbox "Termostat" u Äarobnjaku. */
static BUTTON_Handle   hButtonWizNext;              /**< @brief Handle za dugme "[ Dalje ]" unutar Äarobnjaka. */
static BUTTON_Handle   hButtonWizBack;              /**< @brief Handle za dugme "[ Nazad ]" unutar Äarobnjaka. */
static BUTTON_Handle   hButtonWizCancel;            /**< @brief Handle za dugme "[ OtkaÅ¾i ]" unutar Äarobnjaka. */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Kapija (SCREEN_SETTINGS_8 i SCREEN_GATE_SETTINGS)
 * @{
 */
static SPINBOX_Handle  hGateSelect;                    /**< @brief Handle za SPINBOX za odabir kapije (1-6) na ekranu za podeÅ¡avanja. */
...
static BUTTON_Handle   hGateControlButtons[6];         /**< @brief Niz handle-ova za dinamiÄki kreiranu dugmad (Otvori, Zatvori, Stop...) na kontrolnom panelu. */
/** @} */

/** @name Handle-ovi za Univerzalne Tastature (NumPad i AlfanumeriÄka)
 * @{
 */
static BUTTON_Handle   hKeypadButtons[12];             /**< @brief Niz handle-ova za 12 dugmadi na numeriÄkoj tastaturi. */
static WM_HWIN         hKeyboardButtons[KEY_ROWS * KEYS_PER_ROW]; /**< @brief Niz handle-ova za tastere sa karakterima na alfanumeriÄkoj tastaturi. */
static WM_HWIN         hKeyboardSpecialButtons[5];     /**< @brief Niz handle-ova za specijalne tastere (Shift, Space, Del, OK) na alfanumeriÄkoj tastaturi. */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Datuma i Vremena (SCREEN_SETTINGS_DATETIME)
 * @{
 */
...
/** @name Handle-ovi za PodeÅ¡avanja Tajmera (SCREEN_SETTINGS_TIMER)
 * @{
 */
...
static BUTTON_Handle   hButtonTimerSave;               /**< @brief Handle za dugme "Snimi" na ekranu za podeÅ¡avanje tajmera. */
static BUTTON_Handle   hButtonTimerCancel;             /**< @brief Handle za dugme "OtkaÅ¾i" na ekranu za podeÅ¡avanje tajmera. */
/** @} */

/** @name Handle-ovi za PodeÅ¡avanja Alarma (SCREEN_SETTINGS_ALARM)
 * @{
 */
...
uint32_t dispfl;
/**
 * @brief Varijable koje Äuvaju trenutno stanje korisniÄkog interfejsa.
 * @note Moraju biti globalne jer drugi moduli (rs485, lights, curtain)
 * Äitaju 'screen' i postavljaju 'shouldDrawScreen' da zatraÅ¾e osvjeÅ¾avanje.
 */
...
/*============================================================================*/
/* STATIÄŒKE VARIJABLE NA NIVOU MODULA                                         */
/*============================================================================*/
/**
 * @brief SluÅ¾i kao interni state-machine fleg za `Service_ThermostatScreen` funkciju.
 * @note Vrijednost `0` oznaÄava da ekran treba inicijalno iscrtati (pozadinu, statiÄke elemente),
 * dok vrijednost `1` oznaÄava da je ekran veÄ‡ iscrtan i da treba samo obraÄ‘ivati unose i
 * aÅ¾urirati dinamiÄke podatke. Resetuje se na `0` prilikom napuÅ¡tanja ekrana.
 */
static uint8_t thermostatMenuState = 0;
/**
 * @brief Fleg koji sluÅ¾i kao mehanizam za komunikaciju izmeÄ‘u modula.
 * @note Drugi moduli (npr. `defroster`, `ventilator`) pozivaju javnu funkciju `DISP_SignalDynamicIconUpdate()`
 * koja postavlja ovaj fleg na `true`. Funkcija `Service_SelectScreen1` periodiÄno provjerava
 * ovaj fleg i, ako je postavljen, pokreÄ‡e ponovno iscrtavanje dinamiÄke ikonice.
 */
static bool dynamicIconUpdateFlag = false;
/**
 * @brief SluÅ¾i kao tajmer (Äuvar `HAL_GetTick()` vrijednosti) za periodiÄne akcije koje se deÅ¡avaju svake sekunde.
 * @note Koristi se u `Handle_PeriodicEvents` i `Service_ThermostatScreen` funkcijama za provjeru da li je
 * proÅ¡lo 1000ms, kako bi se aÅ¾urirao prikaz vremena na ekranu bez blokiranja ostatka koda.
 */
static uint32_t rtctmr;
/**
 * @brief Koristi se za detekciju dugog pritiska na ON/OFF zonu na ekranu termostata.
 * @note `HandlePress_ThermostatScreen` funkcija pokreÄ‡e tajmer (upisuje `HAL_GetTick()`), a
 * `Service_ThermostatScreen` provjerava da li je proÅ¡lo dovoljno vremena (`> 2 sekunde`) da
 * bi se akcija (paljenje/gaÅ¡enje) izvrÅ¡ila.
 */
static uint32_t thermostatOnOffTouch_timer = 0;
/**
 * @brief Glavni tajmer za screensaver. ÄŒuva `HAL_GetTick()` vrijednost posljednje korisniÄke aktivnosti.
 ...
 * @note `HandlePress_LightsScreen` pokreÄ‡e tajmer, a `Handle_PeriodicEvents` provjerava da li je proÅ¡lo
 * dovoljno vremena (`> 2 sekunde`) kako bi se preÅ¡lo na ekran za detaljno podeÅ¡avanje
...
/**
 * @brief Zastarjela varijabla. U priloÅ¾enom kodu se deklariÅ¡e, ali se nigdje ne koristi.
 */
...
/**
 * @brief PomoÄ‡ni tajmer unutar `Handle_PeriodicEvents` koji se inkrementira svake sekunde.
 * @note Kada dostigne vrijednost `10`, pokreÄ‡e aÅ¾uriranje izmjerene temperature (`MVUpdateSet()`)
 * na ekranu termostata.
 */
static uint32_t refresh_tmr = 0;
/**
 * @brief Tajmer koji se koristi iskljuÄivo u `Service_CleanScreen` funkciji za odbrojavanje
 * vremena (60 sekundi) tokom kojeg je ekran zakljuÄan za ÄiÅ¡Ä‡enje.
 */
static uint32_t clean_tmr = 0;
/**
 * @brief Fleg koji se postavlja na `true` u `PID_Hook` funkciji ako je poÄetni pritisak detektovan
 * unutar zone "hamburger" menija.
 * @note Koristi se da bi se sprijeÄilo da se pritisak na meni propagira i aktivira
 ...
 * @note Vrijednost `0` oznaÄava prvi ulazak na ekran kada se inicijalizuje tajmer,
 * dok `1` oznaÄava da je odbrojavanje u toku.
 */
static uint8_t menu_clean = 0;
/**
 * @brief ÄŒuva stanje menija za odabir kontrole (npr. koji QR kod treba prikazati).
...
/**
 * @brief ÄŒuva trenutnu stranicu (indeks) u meniju za podeÅ¡avanje zavjesa.
...
/**
 * @brief ÄŒuva indeks svjetla (stranicu) koje se trenutno podeÅ¡ava u meniju `SCREEN_SETTINGS_5`.
...
/**
 * @brief ÄŒuva indeks svjetla na koje je korisnik kliknuo na ekranu `SCREEN_LIGHTS`.
 * @note Vrijednost `LIGHTS_MODBUS_SIZE + 1` se koristi da oznaÄi da nijedno svjetlo nije odabrano.
 * Koristi se za `toggle` operacije i za ulazak u `SCREEN_LIGHT_SETTINGS` za specifiÄno svjetlo.
 */
 ...
 * kada se podeÅ¡avaju sva svjetla odjednom.
 */
static uint8_t lights_allSelected_hasRGB = 0;
/**
 * @brief Globalni fleg za sve ekrane sa podeÅ¡avanjima. Postavlja se na `1` kada korisnik
...
/**
 * @brief Fleg specifiÄan za `Service_SettingsScreen_1`, oznaÄava da je doÅ¡lo do promjene
 * u podeÅ¡avanjima termostata.
 * @note Osigurava da se `THSTAT_Save()` poziva samo kada je to zaista potrebno.
 */
static uint8_t thsta = 0;
/**
 * @brief Zastarjeli fleg, vjerovatno za promjene u podeÅ¡avanjima svjetala (lights).
 * @note Koristi se u `Service_SettingsScreen_2` ali mu se vrijednost postavlja na zastarjeli naÄin.
 ...
 * @note Postavlja se na `1` u `PID_Hook` kada dodir zapoÄne u zoni hamburger menija.
 */
static uint8_t btnset = 0;
/**
 * @brief Par flegova za "edge detection" mehanizam za dugme za poveÄ‡anje vrijednosti (npr. na termostatu).
 * @note `btninc` se postavlja na `1` dok je dugme pritisnuto. `_btninc` se koristi da
 * osigura da se akcija izvrÅ¡i samo jednom po pritisku, a ne u svakom ciklusu petlje.
 */
...
/**
 * @brief ÄŒuva prethodnu vrijednost minuta.
 * @note Koristi se za detekciju promjene minute kako bi se izbjeglo nepotrebno iscrtavanje ekrana svake sekunde.
 * U priloÅ¾enom kodu se resetuje, ali se njegova vrijednost ne provjerava aktivno.
 */
static uint8_t old_min = 60;
/**
 * @brief ÄŒuva prethodnu vrijednost dana u sedmici.
...
/**
 * @brief Dvodimenzionalni bafer koji u RAM-u Äuva tekstualne sadrÅ¾aje za QR kodove.
 * @note Podaci se uÄitavaju iz EEPROM-a u `DISP_Init` funkciji i koriste u `Service_QrCodeScreen`
 * za generisanje slike QR koda.
 */
static uint8_t qr_codes[QR_CODE_COUNT][QR_CODE_LENGTH] = {0};
/**
 * @brief OdreÄ‘uje koji QR kod treba iscrtati na ekranu `SCREEN_QR_CODE`.
 * @note Vrijednost `1` je za WiFi, `2` za Aplikaciju. Postavlja se u `HandlePress_SelectScreenLast`.
 */
static uint8_t qr_code_draw_id = 0;
/**
 * @brief BrojaÄ za odbrojavanje na ekranu za ÄiÅ¡Ä‡enje (`SCREEN_CLEAN`).
 * @note Inicijalizuje se na 60 i dekrementira svake sekunde u `Service_CleanScreen` funkciji.
 */
static uint8_t clrtmr = 0;
/**
 * @brief ÄŒuva kompletno stanje (koordinate i status pritiska) posljednjeg detektovanog pritiska na ekran.
 * @note Koristi se na `SCREEN_MAIN` da bi se pri otpuÅ¡tanju znalo da li je poÄetni pritisak
 * bio unutar zone glavnog prekidaÄa.
 */
static GUI_PID_STATE last_press_state;
/**
 * @brief ÄŒuva indeks scene (0-5) koja je odabrana kao akcija za tajmer.
 * @note Vrijednost -1 znaÄi da nijedna scena nije odabrana. `Service_SettingsTimerScreen`
 * ga koristi za snimanje postavki i prikaz naziva odabrane scene.
 */
static int8_t timer_selected_scene_index = -1;
/**
 * @brief Fleg koji oznaÄava da li je ekran za podeÅ¡avanje tajmera (`SCREEN_SETTINGS_TIMER`) veÄ‡ inicijalizovan.
 * @note SprjeÄava ponovno uÄitavanje vrijednosti iz `Timer` modula i reinicijalizaciju
 * widgeta unutar `Service_SettingsTimerScreen` funkcije.
 */
static bool timer_screen_initialized = false;
/**
 * @brief       ÄŒuva odabranu akciju (0=System, 1-3=Particija) sa ekrana alarma.
 * @note        Vrijednost se postavlja u `Service_SecurityScreen` kada korisnik
 * pritisne dugme, a zatim se Äita u `Service_NumpadScreen` nakon
 * uspjeÅ¡ne provjere PIN-a kako bi se izvrÅ¡ila odgovarajuÄ‡a komanda.
 */
static int8_t selected_action = -1;
/** @{ */

/** @name StatiÄke varijable za AlfanumeriÄku Tastaturu */

/**
 * @brief Niz handle-ova za sve dinamiÄki kreirane tastere sa karakterima na alfanumeriÄkoj tastaturi.
...
/**
 * @brief Niz handle-ova za specijalne tastere (Shift, Del, Space, OK) na alfanumeriÄkoj tastaturi.
 * @note Koristi se u `Service_KeyboardScreen` za obradu specijalnih komandi.
 */
static WM_HWIN hKeyboardSpecialButtons[5];
/**
 * @brief Interni bafer u koji se upisuje tekst prilikom kucanja na alfanumeriÄkoj tastaturi.
 * @note Njegov sadrÅ¾aj se ispisuje na ekranu i kopira u `g_keyboard_result` nakon potvrde.
 */
static char keyboard_buffer[32];
/**
 * @brief PokazivaÄ (indeks) na trenutnu poziciju za unos u `keyboard_buffer`.
 * @note PonaÅ¡a se kao kursor, inkrementira se pri unosu karaktera, a dekrementira pri brisanju.
 */
static uint8_t keyboard_buffer_idx = 0;
/**
 * @brief Fleg koji prati da li je `Shift` taster na alfanumeriÄkoj tastaturi aktivan.
 * @note Mijenja se pritiskom na "Shift" i koristi se u `DSP_InitKeyboardScreen`
 * da odabere odgovarajuÄ‡i set karaktera (mala/velika slova) za iscrtavanje.
 */
...
/**
 * @brief Bafer u koji se upisuju brojevi uneseni preko numeriÄke tastature (`SCREEN_NUMPAD`).
 * @note SadrÅ¾aj se prikazuje na ekranu (kao brojevi ili zvjezdice) i koristi za validaciju.
 */
static char pin_buffer[MAX_PIN_LENGTH + 1];
/**
 * @brief PokazivaÄ (indeks) na trenutnu poziciju za unos u `pin_buffer`.
 */
static uint8_t pin_buffer_idx = 0;
/**
 * @brief Tajmer za maskiranje karaktera na PIN tastaturi.
 * @note PokreÄ‡e se pri unosu novog broja. Kada istekne (`PIN_MASK_DELAY`),
 * pokreÄ‡e ponovno iscrtavanje koje zamijeni broj sa zvjezdicom.
 */
static uint32_t pin_mask_timer = 0;
/**
 * @brief Fleg koji oznaÄava da li treba prikazati poruku o greÅ¡ci (npr. pogreÅ¡an PIN).
 * @note Ako je `true`, `DSP_DrawNumpadText` Ä‡e ispisati "GRESKA" umjesto unesenih brojeva.
 */
static bool pin_error_active = false;
/**
 * @brief ÄŒuva posljednji uneseni karakter na PIN tastaturi.
 * @note Koristi se da bi se omoguÄ‡ilo privremeno prikazivanje broja prije
 * nego Å¡to ga `pin_mask_timer` pretvori u zvjezdicu.
 */
static char pin_last_char = 0;
/**
 * @brief Tajmer za detekciju dugog pritiska na naziv svjetla za pokretanje izmjene.
 * @note PokreÄ‡e se kada korisnik pritisne zonu sa tekstom u `SCREEN_LIGHT_SETTINGS`.
 * Provjerava se u `Handle_PeriodicEvents`, a resetuje u `HandleTouchReleaseEvent`.
 */
static uint32_t rename_light_timer_start = 0;
/**
 * @brief ÄŒuva ID posljednjeg pritisnutog "EDIT" dugmeta na ekranu za podeÅ¡avanje kapija.
 * @note  Koristi se za obradu rezultata vraÄ‡enog sa Numpad-a.
 */
static int active_gate_edit_button_id = 0;
/**
 * @brief Indeks kapije (0-5) koja je trenutno odabrana za podeÅ¡avanje.
 */
static uint8_t settings_gate_selected_index = 0;
/**
 * @brief ÄŒuva ID ekrana na koji se treba vratiti nakon zatvaranja Numpad-a.
 */
static eScreen numpad_return_screen = SCREEN_MAIN;
/**
 * @brief ÄŒuva ID ekrana na koji se treba vratiti nakon zatvaranja alfanumeriÄke tastature.
 */
static eScreen keyboard_return_screen = SCREEN_MAIN;
/**
 * @brief ÄŒuva ID ekrana na koji se treba vratiti nakon izlaska iz podeÅ¡avanja svjetla.
 * @note  Ovo je neophodno jer se u SCREEN_LIGHT_SETTINGS moÅ¾e uÄ‡i sa viÅ¡e
 * razliÄitih ekrana (npr. SCREEN_MAIN ili SCREEN_LIGHTS).
 */
 ...
 * @note  Ovaj fleg je kljuÄan za kontekstualnu promjenu ponaÅ¡anja ekrana.
 * Kada je `true`, ekrani kao Å¡to su SCREEN_LIGHTS ili SCREEN_THERMOSTAT
 * Ä‡e prikazati "Dalje" dugme umjesto standardnog interfejsa.
 * Postavlja se na `true` pri ulasku u "anketni" Äarobnjak, a na `false`
 ...
 * @author      Gemini
 * @note        Koristi se za izraÄunavanje trajanja pritiska kako bi se
 * razlikovao kratak klik (toggle akcija) od dugog pritiska
 * (ulazak u kontrolni panel). Vrijednost 0 oznaÄava da pritisak
 * nije aktivan.
 ******************************************************************************
 */
 ...
 * @author      Gemini
 * @note        ÄŒuva se prilikom pritiska, a koristi prilikom otpuÅ¡tanja ili
 * detekcije dugog pritiska. Vrijednost -1 oznaÄava da nijedan
 * slot nije pritisnut.
 ******************************************************************************
 */
 ...
 * a `DSP_KillGateSettingsScreen` ga resetuje na `false`, Äime se
 * forsira reinicijalizacija internih statiÄkih varijabli servisa.
 ******************************************************************************
 */
 ...
 * @author      Gemini
 * @note        Ova varijabla se postavlja prilikom dugog pritiska i sluÅ¾i
 * `Service_GateControlPanel` funkciji da zna za koji ureÄ‘aj treba
 * iscrtati kontrole i slati komande.
 ******************************************************************************
 */
static uint8_t gate_control_panel_index = 0;
/**
 * @brief ÄŒuva trenutni mod rada za ekran za odabir scene.
 * @note  Ovu varijablu postavljaju funkcije koje pozivaju ekran (npr. iz `Service_SettingsTimerScreen`)
 * kako bi definisale njegovo ponaÅ¡anje. Inicijalno je postavljen na WIZARD mod.
 */
static eScenePickerMode current_scene_picker_mode = SCENE_PICKER_MODE_WIZARD;
/**
 * @brief ÄŒuva adresu ekrana na koji se treba vratiti nakon odabira scene.
 * @note  Ovo omoguÄ‡ava da se ekran za odabir pozove sa viÅ¡e razliÄitih mjesta u aplikaciji
 * i da se uvijek osigura ispravan povratak.
 */
static eScreen scene_picker_return_screen = SCREEN_SCENE_EDIT;
/** 
 * @brief Tajmer za detekciju dugog pritiska na dinamiÄkoj ikonici na SelectScreen1. 
 */
static uint32_t dynamic_icon1_press_timer = 0;
/** 
 * @brief Tajmer za detekciju dugog pritiska na dinamiÄkoj ikonici na SelectScreen2. 
 */
static uint32_t dynamic_icon2_press_timer = 0;
/**
 * @brief ÄŒuva indeks particije (0-2) koja je odabrana za promjenu naziva.
 * @note  Vrijednost -1 oznaÄava da nijedna nije odabrana.
 */
...
/**
 * @brief Privremeni bafer za Äuvanje novog PIN-a tokom procedure promjene.
 */
...
/**
 * @name Funkcije za Inicijalizaciju i UniÅ¡tavanje Ekrana PodeÅ¡avanja
 * @brief Grupa funkcija odgovornih za kreiranje (`Init`) i uniÅ¡tavanje (`Kill`)
 * svih GUI widgeta na pojedinaÄnim ekranima za podeÅ¡avanja.
 * @{
 */
 ...
 * petlji (`DISP_Service`). Poziva se periodiÄno dok je odgovarajuÄ‡i
 * ekran aktivan i sadrÅ¾i logiku za taj ekran.
 * @{
 */
 ...
 * promjenu stanja dodira. Ona je ulazna taÄka za sve korisniÄke interakcije.
 * @param pTS PokazivaÄ na strukturu sa stanjem dodira (koordinate, pritisnut/otpuÅ¡ten).
 */
static void PID_Hook(GUI_PID_STATE * pTS);
/**
 * @brief Upravlja svim periodiÄnim dogaÄ‘ajima i tajmerima.
 * @note Poziva se iz `DISP_Service`. Odgovorna za logiku screensaver-a,
 * noÄ‡nog tajmera za svjetla, aÅ¾uriranje vremena i druge pozadinske zadatke.
 */
static void Handle_PeriodicEvents(void);
/**
 * @brief DispeÄer za dogaÄ‘aje pritiska na ekran.
 * @note Poziva se iz `PID_Hook` kada je ekran pritisnut. Na osnovu trenutnog
 * ekrana (`screen`), prosljeÄ‘uje dogaÄ‘aj odgovarajuÄ‡oj `HandlePress_...` funkciji.
 * @param pTS PokazivaÄ na strukturu sa stanjem dodira.
 * @param click_flag PokazivaÄ na fleg koji se postavlja ako treba generisati zvuÄni signal.
 */
...
/**
 * @brief DispeÄer za dogaÄ‘aje otpuÅ¡tanja dodira sa ekrana.
...
// NOVI PROTOTIPOVI ZA ALFANUMERIÄŒKU TASTATURU
...
/**
 * @name PomoÄ‡ne (Utility) Funkcije
 * @brief Grupa internih funkcija za razne zadatke kao Å¡to su snimanje,
 * iscrtavanje zajedniÄkih elemenata, upravljanje menijima, itd.
 * @{
 */
...
/**
 * @brief Postavlja sve postavke displeja na sigurne fabriÄke vrijednosti.
 */
...
/**
 * @brief Iscrtava i aÅ¾urira prikaz datuma i vremena na ekranu.
 */
...
/**
 * @brief Detektuje i obraÄ‘uje dugi pritisak za ulazak u meni za podeÅ¡avanja.
 * @param btn Fleg koji ukazuje na poÄetak pritiska (postavljen u `PID_Hook`).
 * @retval uint8_t 1 ako je dugi pritisak detektovan, inaÄe 0.
 */
static uint8_t DISPMenuSettings(uint8_t btn);
/**
 * @brief Provjerava status aÅ¾uriranja firmvera i prikazuje odgovarajuÄ‡u poruku.
 * @retval uint8_t 1 ako je aÅ¾uriranje aktivno, inaÄe 0.
 */
static uint8_t Service_HandleFirmwareUpdate(void);
/**
 * @brief Forsirano uniÅ¡tava sve widgete sa ekrana za podeÅ¡avanja.
 * @note Koristi se kao "fail-safe" mehanizam za ÄiÅ¡Ä‡enje "duhova" sa ekrana.
 */
static void ForceKillAllSettingsWidgets(void);
/**
 * @brief Mapa koja povezuje indeks u DROPDOWN listi sa stvarnom ControlMode vrijednoÅ¡Ä‡u za Ikonu 1.
 */
static int8_t control_mode_map_1[MODE_COUNT];
/**
 * @brief Mapa koja povezuje indeks u DROPDOWN listi sa stvarnom ControlMode vrijednoÅ¡Ä‡u za Ikonu 2.
 */
 ...
 * @note Poziva se jednom na poÄetku programa iz main() funkcije.
 * Inicijalizuje STemWin, postavlja hook za touch, uÄitava
...
void DISP_Init(void)
{
    ...
    // Inicijalizacija STemWin grafiÄke biblioteke
    GUI_Init();
    // Povezivanje (hook) funkcije za obradu dodira sa GUI sistemom
    GUI_PID_SetHook(PID_Hook);
    // OmoguÄ‡avanje viÅ¡estrukog baferovanja za fluidnije iscrtavanje
    WM_MULTIBUF_Enable(1);
    // Postavljanje UTF-8 enkodiranja za podrÅ¡ku specijalnim karakterima
    GUI_UC_SetEncodeUTF8();
    // Odabir i ÄiÅ¡Ä‡enje prvog sloja (layer 0)
    GUI_SelectLayer(0);
    GUI_Clear();
    ...
    // =======================================================================
    // === POÄŒETAK NOVE "PAMETNE" LOGIKE ODABIRA POÄŒETNOG EKRANA ===
    // =======================================================================
    // KORAK 1: Dobijamo handle-ove za sve relevantne module.
    THERMOSTAT_TypeDef* pThst = Thermostat_GetInstance();
    // Ovdje Ä‡emo dodati i gettere za Gate, Timer, Security kada njihovi moduli budu imali tu funkciju.
    ...
    // bool has_gates = (Gates_GetCount() > 0); // Primjer za buduÄ‡nost

    if (has_lights) {
        // Ako ima svjetala, poÄetni ekran je uvijek Main screen.
        screen = SCREEN_MAIN;
    } else if (has_thermostat && !has_curtains /* && !has_gates... */) {
        // Ako je JEDINA konfigurisana funkcija termostat.
        screen = SCREEN_THERMOSTAT;
    } else if (has_thermostat || has_curtains /* || has_gates... */) {
        // Ako nema svjetala, ali ima VIÅ E drugih funkcija, scene su najbolji dashboard.
        screen = SCREEN_SCENE;
    } else {
        // Ako nema NIJEDNE konfigurisane glavne funkcije, provjeravamo SelectScreen1.
        // Ovdje Ä‡emo dodati provjeru za Ventilator/Defroster.
        // Za sada, ako nema niÄega, idemo na poruku za konfiguraciju.
    ...
    if (screen == 0) { // Sigurnosna provjera
        screen = SCREEN_MAIN; // VraÄ‡amo na Main kao najsigurniju opciju
    }
    ...
    // AÅ¾urira se shouldDrawScreen za prvu petlju da bi se iscrtao odabrani ekran.
    shouldDrawScreen = 1;
}
/**
 * @brief Glavna servisna funkcija za korisniÄki interfejs.
 * @note Poziva se periodiÄno iz glavne petlje programa (while(1) u main.c).
 * Odgovorna je za aÅ¾uriranje GUI-ja, obradu tajmera i pozivanje
 * specifiÄnih servisnih funkcija u zavisnosti od aktivnog ekrana.
 */
void DISP_Service(void)
{
    static uint32_t guitmr = 0;

    // AÅ¾uriranje GUI-ja i obrada TS-a u fiksnim intervalima
    if ((HAL_GetTick() - guitmr) >= GUI_REFRESH_TIME) {
        guitmr = HAL_GetTick();
        GUI_Exec(); // IzvrÅ¡ava sve pending operacije iscrtavanja
    }

    // Provjera i prikaz poruke o aÅ¾uriranju firmvera
    if (Service_HandleFirmwareUpdate()) {
        return; // Ako je aÅ¾uriranje u toku, prekini dalje izvrÅ¡avanje GUI logike
    }
    ...
    case SCREEN_RESET_MENU_SWITCHES:
        ...
        break;
    default:
        // U sluÄaju nepoznatog stanja, resetuj flegove menija
        menu_lc = 0;
        thermostatMenuState = 0;
        break;
    }

    // Upravljanje periodiÄnim dogaÄ‘ajima i tajmerima (npr. screensaver)
    Handle_PeriodicEvents();

    // Provjera da li treba uÄ‡i u meni za podeÅ¡avanja (dugi pritisak)
    if (DISPMenuSettings(btnset) && (screen < SCREEN_SETTINGS_1)) {
        // Inicijalizuj prvi ekran podeÅ¡avanja
        DSP_InitSet1Scrn();
...
void DISPSetPoint(void)
{
    ...
    // Koristi se viÅ¡estruko baferovanje za iscrtavanje bez treperenja
    GUI_MULTIBUF_BeginEx(1);

    // OÄisti pravougaonik gdje se nalazi vrijednost
    ...
    // IspiÅ¡i vrijednost zadate temperature
    GUI_DispDec(Thermostat_GetSetpoint(pThst), 2);

    // ZavrÅ¡i operaciju iscrtavanja
...
void DISPResetScrnsvr(void)
{
    ...
    // Ako je screensaver bio aktivan, vrati se na poÄetni ekran
...
/**
 * @brief Hook funkcija za obradu dogaÄ‘aja sa ekrana osjetljivog na dodir.
 * @note  Ovo je centralna taÄka za obradu korisniÄkog unosa. STemWin poziva
 * ovu funkciju svaki put kada detektuje promjenu stanja dodira.
 * @param pTS Pointer na strukturu sa stanjem dodira (koordinate, pritisnut/otpuÅ¡ten).
 */
/**
 * @brief Hook funkcija za obradu dogaÄ‘aja sa ekrana osjetljivog na dodir.
 * @param pTS Pointer na strukturu sa stanjem dodira (koordinate, pritisnut/otpuÅ¡ten).
...
{
    static uint8_t release = 0; // Fleg koji prati da li je dodir bio pritisnut i Äeka otpuÅ¡tanje.
    uint8_t click = 0;          // Fleg koji signalizira da treba generisati zvuÄni signal (klik).
    ...
    if (screen == SCREEN_ALARM_ACTIVE) {
        ...
        return; // PreskoÄi ostatak logike
    }

    // NOVO: Potpuna blokada dodira ako je detektovana aktivnost aÅ¾uriranja na busu.
    if (IsBusFwUpdateActive()) {
        // Ako je aÅ¾uriranje aktivno, resetujemo screensaver da ekran ostane upaljen
        // i vidljiv, ali ignoriÅ¡emo svaki unos dodirom.
        DISPResetScrnsvr();
        return;
    }

    // Provjera da li je dodir registrovan na poÄetku s nulama, i resetuj btnset.
    // NOVO: Svaka linija je objaÅ¡njena
    if(pTS->x == 0 && pTS->y == 0 && pTS->Pressed == 0) { // Provjerava da li su koordinate nula i pritisak nula
        btnset = 0; // Ako je poÄetno stanje, resetuj fleg za dugi pritisak
        return; // Izlazak iz funkcije
    }

    // Ostatak koda ostaje nepromijenjen
    if (screen == SCREEN_CLEAN) { // Ako je ekran za ÄiÅ¡Ä‡enje, ignoriraj dodir
        return; // Izlazak iz funkcije
    }

    // --- Obrada pritiska na ekran ---
    if (pTS->Pressed == 1U) { // Ako je pritisak registrovan
        pTS->Layer = 1U; // DogaÄ‘aj se odnosi na gornji sloj (layer 1)
        release = 1;     // Postavi fleg da se Äeka otpuÅ¡tanje
        ...
        {
            touch_in_menu_zone = true; // Postavi fleg da je dodir poÄeo u zoni menija
            click = 1;                 // Svaki dodir u ovoj zoni generiÅ¡e zvuÄni signako setuje l

            // << ISPRAVKA: Centralizovano brisanje ekrana prilikom svake navigacije >>
            // Ovo osigurava da je ekran uvijek Äist prije iscrtavanja novog sadrÅ¾aja.
            GUI_SelectLayer(0);
            GUI_Clear();
            GUI_SelectLayer(1);
            GUI_Clear();
            ...
            // =======================================================================
            // === POÄŒETAK IZMJENE: Nova, ispravna logika navigacije ===
            // =======================================================================
            switch(screen) {
            ...
            case SCREEN_SETTINGS_DATETIME:
                ...
                screen = SCREEN_TIMER; // VraÄ‡amo se na Timer ekran
                break;
            ...
            case SCREEN_SECURITY:
                DSP_KillSecurityScreen(); // <<< OBAVEZNO UNIÅ TI DUGMAD PRIJE IZLASKA
                screen = SCREEN_SELECT_2;
                break;
            ...
            // Specijalni sluÄajevi ostaju isti
            case SCREEN_LIGHT_SETTINGS:
            ...
            shouldDrawScreen = 1; // Uvijek zatraÅ¾i iscrtavanje nakon promjene ekrana
    ...
    else { // Ako je dodir otpuÅ¡ten
        if(release) {
            release = 0;
            HandleTouchReleaseEvent(pTS);
            touch_in_menu_zone = false;
        }
        // ÄŒim korisnik podigne prst, bez obzira gdje, vraÄ‡amo osjetljivost na normalu.
 ...
 * stvarajuÄ‡i efekat skrolujuÄ‡eg loga.
 * @param pbuf Pointer na string koji treba prikazati.
 */
void DISP_UpdateLog(const char *pbuf)
{
    static char displog[6][128]; // StatiÄki 2D niz za Äuvanje historije loga (6 linija)
    int i = 5;

    // OÄisti podruÄje na ekranu gdje se ispisuje log
    ...
    // IspiÅ¡i novu poruku na dnu loga Å¾utom bojom
 ...
 * @note Ova funkcija vraÄ‡a vrijednost internog fleg-a koji
...
/**
 * @brief Signalizira da je potrebno aÅ¾urirati dinamiÄku ikonu.
 * @note Ova funkcija postavlja interni fleg-a `dynamicIconUpdateFlag`
 * na 'true'. Glavna servisna funkcija Ä‡e proÄitati ovaj fleg i
 * ponovo iscrtati dinamiÄku ikonu (za defroster/ventilator).
 * @param None
 * @retval None
 */
...
/**
 * @brief VraÄ‡a pointer na odgovarajuÄ‡i string iz tabele prevoda.
 * @param t ID teksta koji treba uÄitati (iz TextID enum-a).
...
const char* lng(uint8_t t)
{
    ...
    // U sluÄaju nevalidnog ID-ja, vrati pointer na prazan string
    return language_strings[0][0]; // Index 0 (TXT_DUMMY) je definisan kao prazan string
}
/**
 * @brief Provjerava da li je duÅ¾ina podataka za QR kod validna.
 * @param dataLength DuÅ¾ina podataka.
 * @return true ako je duÅ¾ina manja od definisanog maksimuma, inaÄe false.
 */
...
/**
 * @brief Provjerava da li Ä‡e string stati u bafer za QR kod.
 * @param data Pointer na string sa podacima.
 * @return true ako podaci staju, inaÄe false.
 */
...
/**
 * @brief VraÄ‡a pointer na podatke za specifiÄni QR kod.
...
uint8_t* QR_Code_Get(const uint8_t qrCodeID)
{
    ...
    if (qrCodeID > 0 && qrCodeID <= QR_CODE_COUNT) {
        return qr_codes[qrCodeID - 1]; // Vrati traÅ¾eni QR kod
    }
    return qr_codes[0]; // Vrati prvi kao fallback u sluÄaju greÅ¡ke
}
/**
 * @brief Postavlja podatke za specifiÄni QR kod.
 * @note  Funkcija provjerava da li podaci staju u predviÄ‘eni bafer prije kopiranja.
 * @param qrCodeID ID QR koda (1 za WiFi, 2 za App).
 * @param data Pointer na string sa podacima.
 */
void QR_Code_Set(const uint8_t qrCodeID, const uint8_t *data)
{
    // Provjeri da li je ID validan i da li Ä‡e podaci stati u bafer
    if(QR_Code_willDataFit(data) && qrCodeID > 0 && qrCodeID <= QR_CODE_COUNT)
    {
        // Sigurno kopiraj string u odgovarajuÄ‡i red 2D niza
        sprintf((char*)(qr_codes[qrCodeID - 1]), "%s", (char*)data);
    }
}
/*============================================================================*/
/* IMPLEMENTACIJA STATIÄŒKIH (PRIVATNIH) FUNKCIJA                              */
/*============================================================================*/
static void DISP_Animation(void)
{
    DISPSetBrightnes(20);
    // Deklaracija niza pokazivaÄa na sve frejmove animacije
    ...
    // Definirajte vrijeme zadrÅ¾avanja svakog frejma u milisekundama
    ...
    for (int i = 0; i < (sizeof(animation_frames) / sizeof(animation_frames[0])); i++)
    {
        ...
        // ÄŒekanje prije prelaska na sljedeÄ‡i frejm
    ...
    // =======================================================================
    // === POÄŒETAK NOVE LOGIKE ZA ISPIS TEKSTA NA DNU EKRANA ===
    // =======================================================================
    ...
    // Tekst koji Å¾elimo animirati
    const char* text = "www.imedia.ba";

    // IzraÄunavanje pozicije i dimenzija teksta
    ...
    for(int current_width = 0; current_width <= text_width; current_width += 5) {
        ...
        // Iscrtavanje cijelog teksta unutar definiranog "kliping" podruÄja
        GUI_DispStringAt(text, x_start, y_bottom);

        // Obavezno resetirajte "kliping" podruÄje nakon svake operacije
        GUI_SetClipRect(NULL);

        GUI_MULTIBUF_End();
        GUI_Exec();

        // GeneriÅ¡i "klik" zvuk za upravo iscrtano slovo
//        BuzzerOn();
//        HAL_Delay(1);
//        BuzzerOff();

        HAL_Delay(50); // Podesite kaÅ¡njenje za brzinu animacije
    }
...
#define ANIMATION_REPEATS 20 // Podesite broj ponavljanja animacije
    ...
    // IzraÄunavanje dimenzija za brisanje (koristimo dimenzije prvog frejma)
    const int xPos = 118;
    const int yPos = 80;
    ...
    {
        // UnutraÅ¡nja petlja za prikaz svakog frejma
        for (int i = 0; i < (sizeof(animation_flame) / sizeof(animation_flame[0])); i++)
        {
            GUI_MULTIBUF_Begin();

            // DinamiÄko ÄiÅ¡Ä‡enje podruÄja bitmape na njenoj poziciji
...
/**
 ******************************************************************************
 * @brief       PomoÄ‡na funkcija za popunjavanje dropdown liste dinamiÄkim opcijama.
 * @author      Gemini
 * @note        ISPRAVLJENA VERZIJA. Ova funkcija viÅ¡e ne koristi nepostojeÄ‡u
 * ...UserData funkciju. Umjesto toga, dinamiÄki kreira 'control_mode_map'
 ...
 * @param[in]   exclusion_mode Vrijednost iz `ControlMode` enuma koju treba preskoÄiti.
 * @param[out]  map_array PokazivaÄ na niz koji Ä‡e biti popunjen mapiranim vrijednostima.
 * @param[in]   map_size VeliÄina `map_array` niza.
...
static int PopulateControlDropdown(DROPDOWN_Handle hDropdown, int8_t exclusion_mode, int8_t* map_array, int map_size)
{
    ...
    int items_added = 0;
    // PoÅ¡to DeleteAllItems ne postoji, briÅ¡emo stavke jednu po jednu unazad.
...
/**
 ******************************************************************************
 * @brief Provjerava da li je u toku aÅ¾uriranje firmvera bilo gdje na RS485 busu.
 * @author Gemini & [VaÅ¡e Ime]
 * @note  Funkcija provjerava globalni tajmer koji se resetuje svakim primljenim
 * FIRMWARE_UPDATE paketom. Ako je od zadnjeg paketa proÅ¡lo manje od
 * FW_UPDATE_BUS_TIMEOUT, smatra se da je aÅ¾uriranje i dalje aktivno.
 * @retval bool true ako je aÅ¾uriranje aktivno, inaÄe false.
 ******************************************************************************
 */
static bool IsBusFwUpdateActive(void)
{
    // Provjera da li je tajmer inicijalizovan (da ne bi bilo laÅ¾no pozitivnih na startu)
    if (g_last_fw_packet_timestamp == 0) {
        return false;
    }
    // Provjera da li je proÅ¡lo manje vremena od definisanog timeout-a
...
static void Display_SetDefault(void)
{
    ...
    // Postavljanje logiÄkih poÄetnih vrijednosti
    ...
    g_display_settings.language = BSHC; // PoÄetni jezik je bosanski
    g_display_settings.scenes_enabled = true;
}

/**
 * @brief  ÄŒuva kompletnu konfiguraciju displeja u EEPROM, ukljuÄujuÄ‡i CRC.
 * @param  None
 * @retval None
 */
static void Display_Save(void)
{
    // Postavljanje magiÄnog broja kao "potpisa"
    g_display_settings.magic_number = EEPROM_MAGIC_NUMBER;
    // Privremeno nuliranje CRC-a radi ispravnog izraÄuna
    g_display_settings.crc = 0;
    // IzraÄunavanje CRC-a nad cijelom strukturom
...
static void Display_InitSettings(void)
{
    // UÄitavanje cijelog konfiguracionog bloka iz EEPROM-a
    EE_ReadBuffer((uint8_t*)&g_display_settings, EE_DISPLAY_SETTINGS, sizeof(Display_EepromSettings_t));

    // Provjera "magiÄnog broja"
    if (g_display_settings.magic_number != EEPROM_MAGIC_NUMBER) {
        // Podaci su nevaÅ¾eÄ‡i, uÄitaj fabriÄke postavke
        Display_SetDefault();
        // Odmah snimi ispravne fabriÄke postavke
        Display_Save();
    } else {
        // MagiÄni broj je ispravan, provjeri integritet podataka pomoÄ‡u CRC-a
        ...
        if (received_crc != calculated_crc) {
            // Podaci su oÅ¡teÄ‡eni, uÄitaj sigurne fabriÄke postavke
            Display_SetDefault();
            Display_Save();
        }
    }

    // Nakon validacije, primijeni uÄitane/defaultne postavke na globalne varijable
    ...
    // Ovo omoguÄ‡ava da ostatak koda ne morate mijenjati odmah.
}

/**
* @brief OÄisti sve duhove sa ekrana
...
static void ForceKillAllSettingsWidgets(void)
{
    ...
    // --- 1. UniÅ¡tavanje statiÄkih widgeta sa nove, pregledne liste ---
    ...
    // --- 2. UniÅ¡tavanje dinamiÄkih widgeta (petlje ostaju) ---
    // Zavjese
 ...
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Ova funkcija je refaktorisana da eliminiÅ¡e dupliciranje koda i
 * "magiÄne brojeve". Koristi centralizovane konstante iz
 * `hamburger_menu_layout` strukture. Poziva se sa razliÄitih
 * ekrana za iscrtavanje standardnih navigacionih elemenata. [cite: 52, 53, 54]
 * @param       position OdreÄ‘uje lokaciju iscrtavanja: [cite: 58]
...
static void DrawHamburgerMenu(uint8_t position)
{
    ...
    {
        return; // Nepoznata pozicija, ne radi niÅ¡ta.
    }
 ...
 * @note AÅ¾urira se svake sekunde i odgovorna je za aktivaciju/deaktivaciju
 * screensavera na osnovu postavljenih sati. Ova refaktorirana verzija koristi
 * GUI funkcije LCD_GetXSize() i LCD_GetYSize() za dinamiÄko odreÄ‘ivanje
 * dimenzija ekrana, Äime se uklanjaju fiksne numeriÄke vrijednosti ("magiÄni brojevi")
 * i poboljÅ¡ava prenosivost koda.
 */
static void DISPDateTime(void)
{
    /**
     * @brief Konstante za dimenzioniranje podruÄja za brisanje
    ...
    // Logika za automatsko paljenje/gaÅ¡enje screensaver-a
    ...
    if (IsScrnsvrActiv() && IsScrnsvrEnabled() && IsScrnsvrClkActiv()) {
        ...
        // POPRAVAK: Brisanje ekrana za screensaver bez magiÄnih brojeva
         ...
         * @note Koristi se za dinamiÄko prevoÄ‘enje imena dana na screensaver-u.
         */
         ...
         * @note Koristi se za dinamiÄko prevoÄ‘enje imena mjeseci na screensaver-u.
         */
...
/**
 ******************************************************************************
 * @brief       [DUMMY] Prikazuje skoÄni prozor sa porukom o greÅ¡ci.
 * @author      Gemini
 * @note        Trenutno samo ispisuje poruku na ekranu. Kasnije Ä‡e biti
 * zamijenjena sa pravim modalnim dijalogom.
 * @param[in]   device_name Naziv ureÄ‘aja koji je u greÅ¡ci.
 * @param[in]   device_index Indeks ureÄ‘aja da bismo znali koji handle da resetujemo.
 ******************************************************************************
 */
static void Display_ShowErrorPopup(const char* device_name, uint8_t device_index)
{
    ...
    // Nakon potvrde, pozovi backend da resetuje stanje greÅ¡ke
    Gate_Handle* handle = Gate_GetInstance(device_index);
    //Gate_AcknowledgeFault(handle);

    shouldDrawScreen = 1; // ZatraÅ¾i ponovno iscrtavanje
}
/**
 * @brief Otkriva i obraÄ‘uje dugi pritisak za ulazak u meni za podeÅ¡avanja.
 * @param btn Fleg koji ukazuje na poÄetak pritiska (postavljen u PID_Hook).
 * @return uint8_t 1 ako je dugi pritisak detektovan, inaÄe 0.
 */
static uint8_t DISPMenuSettings(uint8_t btn)
{
    ...
    if ((btn == 1U) && (last_state == 0U)) {
        // PoÄetak pritiska, postavi fleg i pokreni tajmer.
        last_state = 1U;
        ...
        if((HAL_GetTick() - menu_tmr) >= SETTINGS_MENU_ENABLE_TIME) {
            ...
            return (1U); // UspjeÅ¡an dugi pritisak.
        }
...
/**
 * @brief Inicijalizuje prvi ekran podeÅ¡avanja (kontrola termostata i ventilatora).
 * @note  Ova funkcija kreira sve potrebne GUI widgete, kao Å¡to su RADIO
 * i SPINBOX kontrole, te dugmad "NEXT" i "SAVE" za navigaciju i Äuvanje.
...
static void DSP_InitSet1Scrn(void)
{
    /** @brief Dobijamo handle za termostat kako bismo proÄitali trenutne postavke. */
    THERMOSTAT_TypeDef* pThst = Thermostat_GetInstance();

    /**
     * @brief Inicijalizacija iscrtavanja.
     * @note  PokreÄ‡e se viÅ¡estruko baferovanje i Äiste se oba grafiÄka sloja.
     */
    GUI_SelectLayer(0);
    GUI_Clear();
    GUI_SelectLayer(1);
     ...
     * @note  Pozicije, dimenzije i poÄetne vrijednosti se uzimaju iz layout strukture
    ...
    // << ISPRAVKA: Dodat je 7. argument (ExFlags) sa vrijednoÅ¡Ä‡u 0. >>
...
/**
 * @brief BriÅ¡e GUI widgete sa prvog ekrana podeÅ¡avanja.
 * @note Ova funkcija se poziva pre prelaska na novi ekran kako bi se oslobodila
 * memorija i sprijeÄili konflikti sa novim widgetima.
 */
static void DSP_KillSet1Scrn(void)
{
...
/**
 * @brief Inicijalizuje drugi ekran podeÅ¡avanja (vrijeme, datum, screensaver).
 * @note  Verzija 2.2: Potpuno refaktorisana, bez magiÄnih brojeva, sa kompletnim
 * kodom za iscrtavanje i ispravnim pozivima CreateEx funkcija.
 * Kreira sve GUI widgete i iscrtava statiÄke elemente (labele, linije)
 * koristeÄ‡i iskljuÄivo konstante iz `settings_screen_2_layout` strukture.
 */
static void DSP_InitSet2Scrn(void)
{
    GUI_SelectLayer(0);
    ...
    /** @brief UÄitavanje trenutnog vremena i datuma sa RTC-a za inicijalizaciju widgeta. */
    HAL_RTC_GetTime(&hrtc, &rtctm, RTC_FORMAT_BCD);
    HAL_RTC_GetDate(&hrtc, &rtcdt, RTC_FORMAT_BCD);

    /** @brief Kreiranje svih widgeta koristeÄ‡i pozicije iz `settings_screen_2_layout`. */
    ...
    // << ISPRAVKA 3: Inicijalizacija se sada vrÅ¡i iz EEPROM strukture `g_display_settings` >>
    ...
    /** @brief Iscrtavanje labela, linija i pregleda boje, koristeÄ‡i pozicije iz layout strukture. */
    GUI_SetColor(clk_clrs[g_display_settings.scrnsvr_clk_clr]);
...
/**
 * @brief BriÅ¡e GUI widgete sa drugog ekrana podeÅ¡avanja.
 * @note BriÅ¡e sve widgete vezane za vrijeme, datum, screensaver i svjetlinu
 * ekrana.
 */
static void DSP_KillSet2Scrn(void)
{
...
/**
 * @brief Inicijalizuje treÄ‡i ekran podeÅ¡avanja (ventilator i odmrzivaÄ).
 * @note Kreira widgete za postavke releja, odgode paljenja i gaÅ¡enja
 * ventilatora i odmrzivaÄa. Sav raspored elemenata je definisan u
 * `settings_screen_3_layout` strukturi.
 */
static void DSP_InitSet3Scrn(void)
{
    /** @brief Dobijamo handle-ove za module Äije postavke se mijenjaju. */
    ...
    /** @brief Kreiranje widgeta za postavke odmrzivaÄa (Defroster). */
...
/**
 * @brief BriÅ¡e GUI widgete sa treÄ‡eg ekrana podeÅ¡avanja.
 * @note BriÅ¡e sve widgete vezane za postavke ventilatora.
 */
static void DSP_KillSet3Scrn(void)
{
...
/**
 * @brief Inicijalizuje Äetvrti ekran podeÅ¡avanja (zavjese).
 * @note  DinamiÄki kreira SPINBOX-ove za do 4 zavjese po ekranu
 * za podeÅ¡avanje releja "GORE" i "DOLJE". Koristi `settings_screen_4_layout`
 * strukturu da dinamiÄki izraÄuna pozicije svih elemenata u 2x2 mreÅ¾i.
 */
static void DSP_InitSet4Scrn(void)
{
    /**
    ...
    for(uint8_t i = curtainSettingMenu * 4; i < (((CURTAINS_SIZE - (curtainSettingMenu * 4)) >= 4) ? ((curtainSettingMenu * 4) + 4) : CURTAINS_SIZE); i++) {
        ...
        /** @brief Dobijamo handle za roletnu po njenom fiziÄkom indeksu u nizu. */
        Curtain_Handle* handle = Curtain_GetInstanceByIndex(i);

        /**
         * @brief DinamiÄko izraÄunavanje pozicija za trenutnu roletnu u mreÅ¾i.
         ...
         * @note  Pozicija se raÄuna na osnovu pozicije "GORE" widgeta i `y_row_spacing` konstante.
         */
         ...
         * @note  Pozicije labela se raÄunaju relativno u odnosu na poziciju widgeta.
         */
        GUI_SetColor(GUI_WHITE);
        GUI_SetFont(GUI_FONT_13_1);
        GUI_SetTextAlign(GUI_TA_LEFT|GUI_TA_VCENTER);
...
/**
 * @brief BriÅ¡e GUI widgete sa Äetvrtog ekrana podeÅ¡avanja.
 * @note BriÅ¡e dinamiÄki kreirane widgete za zavjese na trenutnoj stranici.
 */
static void DSP_KillSet4Scrn(void)
{
    for(uint8_t i = curtainSettingMenu * 4; i < (((CURTAINS_SIZE - (curtainSettingMenu * 4)) >= 4) ? ((curtainSettingMenu * 4) + 4) : CURTAINS_SIZE); i++) {
...
/**
 * @brief Inicijalizuje peti ekran podeÅ¡avanja (detaljne postavke za svjetla).
 * @note  Ova funkcija dinamiÄki kreira sve GUI widgete za podeÅ¡avanje jednog
...
static void DSP_InitSet5Scrn(void)
{
    ...
    /** @brief Dohvatamo indeks i handle svjetla Äije postavke trenutno prikazujemo. */
     ...
     * @note  Pozicije se raÄunaju na osnovu poÄetnih koordinata i visine reda (y_step)
    ...
    // << ISPRAVKA 2: Promijenjen mnoÅ¾ilac za ID-jeve sa 12 na 16 radi izbjegavanja preklapanja >>
    const int id_step = 16;

    // << ISPRAVKA 1: VraÄ‡ena linija za kreiranje RELAY spinbox-a >>
    ...
    /** @brief Postavljanje poÄetnih vrijednosti za sve kreirane widgete. */
...
/**
 * @brief BriÅ¡e GUI widgete sa petog ekrana podeÅ¡avanja.
 * @note BriÅ¡e dinamiÄki kreirane widgete za svjetla na trenutnoj stranici.
 */
static void DSP_KillSet5Scrn(void)
{
    // RaÄunamo indeks svjetla Äiji su widgeti trenutno na ekranu.
    uint8_t i = lightsModbusSettingsMenu; // Ovdje ne treba mnoÅ¾enje

    // BriÅ¡emo svaki widget pojedinaÄno za taj specifiÄni indeks.
    ...
    // BriÅ¡emo i zajedniÄke dugmiÄ‡e.
    WM_DeleteWindow(hBUTTON_Next);
    WM_DeleteWindow(hBUTTON_Ok);
}
/**
 ******************************************************************************
 * @brief       Inicijalizuje Å¡esti ekran podeÅ¡avanja (opÅ¡te postavke).
 * @author      Gemini & [VaÅ¡e Ime]
...
/**
 * @brief BriÅ¡e GUI widgete sa Å¡estog ekrana podeÅ¡avanja.
 * @note BriÅ¡e sve widgete vezane za Device ID, screensaver i navigaciju.
 */
static void DSP_KillSet6Scrn(void)
{
...
/**
 ******************************************************************************
 * @brief       Inicijalizuje sedmi ekran podeÅ¡avanja (Scene Backend).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija kreira korisniÄki interfejs za napredna podeÅ¡avanja
 * sistema scena. UkljuÄuje opciju za globalno omoguÄ‡avanje/onemoguÄ‡avanje
 * scena i tabelu za mapiranje do 8 logiÄkih okidaÄa za "Povratak"
...
static void DSP_InitSet7Scrn(void)
{
    ...
    // Naslov za sekciju okidaÄa
    GUI_SetFont(GUI_FONT_13_1);
    GUI_SetColor(GUI_WHITE);
    GUI_DispStringAt("Mapiranje Okidaca za 'Povratak' Scenu:", 10, 30);

    // --- Kreiranje MreÅ¾e za Mapiranje OkidaÄa (8 komada) ---
    for (uint8_t i = 0; i < SCENE_MAX_TRIGGERS; i++)
    {
        // DinamiÄko izraÄunavanje pozicija u dvije kolone
        int col = i / 4; // 0 za prvu kolonu (okidaÄi 1-4), 1 za drugu (okidaÄi 5-8)
        ...
        // TODO: UÄitati snimljenu vrijednost iz EEPROM-a kada se definiÅ¡e struktura za to
...
/**
 ******************************************************************************
 * @brief       BriÅ¡e GUI widgete sa sedmog ekrana podeÅ¡avanja (Scene Backend).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija se poziva prilikom napuÅ¡tanja ekrana
 * `SCREEN_SETTINGS_7` kako bi se oslobodila memorija i sprijeÄili
 * konflikti sa widgetima na drugim ekranima. Uklanja sve
 * dinamiÄki kreirane elemente.
 ******************************************************************************
 */
static void DSP_KillSet7Scrn(void)
{
    // BriÅ¡emo checkbox za omoguÄ‡avanje scena
    WM_DeleteWindow(hCHKBX_EnableScenes);

    // BriÅ¡emo svih 8 spinbox-ova za adrese okidaÄa
    for (uint8_t i = 0; i < SCENE_MAX_TRIGGERS; i++)
    {
...
/**
 ******************************************************************************
 * @brief       Inicijalizuje osmi ekran podeÅ¡avanja (Kapije).
 * @author      Gemini
 * @note        Ova funkcija kreira kompletan korisniÄki interfejs za konfiguraciju
 * jednog odabranog ureÄ‘aja za kontrolu pristupa (kapije, rampe, brave).
 * Arhitektura, raspored elemenata i naÄin inicijalizacije widgeta
 * su namjerno i u potpunosti preslikani sa funkcije `DSP_InitSet5Scrn`
 * (podeÅ¡avanja za svjetla), kako bi se osigurala vizuelna i
 ...
 * AÅ¾urirano: Spinbox za trajanje impulsa sada koristi opseg 0-50 i jedinicu x100ms.
 ******************************************************************************
 */
static void DSP_InitSet8Scrn(void)
{
    ...
    // Dohvatamo handle za kapiju koja se trenutno konfiguriÅ¡e
    ...
    // Kreiranje navigacionih dugmadi - identiÄno kao na ekranu za svjetla
    ...
    // Iscrtavanje labela - identiÄna logika poravnanja kao kod svjetala
    GUI_SetColor(GUI_WHITE);
    GUI_SetFont(GUI_FONT_13_1);
    GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete sa osmog ekrana podeÅ¡avanja (Kapije).
 * @author      Gemini
 * @note        Ova funkcija se poziva prilikom napuÅ¡tanja ekrana `SCREEN_SETTINGS_8`.
 * Njena uloga je da oslobodi sve resurse zauzete od strane widgeta,
 * Äime se sprjeÄava curenje memorije i osigurava Äisto stanje za
 * iscrtavanje sljedeÄ‡eg ekrana. Arhitektura je identiÄna funkciji
...
static void DSP_KillSet8Scrn(void)
{
    ...
    // Brisanje zajedniÄkih navigacionih dugmadi
    if (WM_IsWindow(hBUTTON_Next)) WM_DeleteWindow(hBUTTON_Next);
    if (WM_IsWindow(hBUTTON_Ok))   WM_DeleteWindow(hBUTTON_Ok);
}
/**
 ******************************************************************************
 * @brief       Inicijalizuje deveti ekran za podeÅ¡avanja (Alarm).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        FINALNA KORIGOVANA VERZIJA: Layout sada striktno koristi
 * `y_group_spacing` konstantu za vertikalni razmak izmeÄ‘u SVIH
 * redova kontrola, ukljuÄujuÄ‡i i zajedniÄke postavke ispod
...
static void DSP_InitSet9Scrn(void)
{
    ...
    // KoriÅ¡tenje konstanti iz layout strukture
    ...
    const int16_t y_spacing = settings_screen_9_layout.y_group_spacing; // JEDINI KORIÅ TENI RAZMAK

    // PomoÄ‡ne varijable za pozicioniranje labela
    ...
    // === POÄŒETAK ISPRAVLJENOG DIJELA ===
    // PoÄetna Y pozicija za prvi red ispod particija, KORISTEÄ†I y_spacing
    ...
    // Pomjeramo Y poziciju za sljedeÄ‡i red KORISTEÄ†I y_spacing
    y_current += y_spacing;
    ...
    GUI_SetColor(GUI_WHITE);
    GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
    GUI_DispStringAt("DuÅ¾ina pulsa", col2_x + lbl_off_x, y_current + lbl_line1_off_y);
    GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
...
/**
 ******************************************************************************
 * @brief       BriÅ¡e GUI widgete sa devetog ekrana za podeÅ¡avanja (Alarm).
 ******************************************************************************
 */
static void DSP_KillSet9Scrn(void)
{
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete kreirane za glavni ekran tajmera.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija osigurava da se dinamiÄki kreirano dugme za
 * podeÅ¡avanja (`hButtonTimerSettings`) ispravno obriÅ¡e prilikom
 * napuÅ¡tanja ekrana, Äime se sprjeÄava curenje memorije.
 * @param       None
 * @retval      None
 ******************************************************************************
 */
...
/**
 ******************************************************************************
 * @brief       Kreira i inicijalizuje sve GUI widgete za ekran podeÅ¡avanja tajmera.
 * @author      Gemini
 * @note        Ova funkcija koristi `timer_settings_screen_layout` strukturu za
 * pozicioniranje svih elemenata. Kreira kontrole za podeÅ¡avanje
 * vremena, 7 checkbox-ova za dane, i dva toggle dugmeta za akcije.
 * Inicijalne vrijednosti se uÄitavaju iz `Timer` modula.
 ******************************************************************************
 */
static void DSP_InitSettingsTimerScreen(void)
{
...
//    const GUI_BITMAP* toggle_icon_scene = Timer_GetActionScene() ? icon_on : icon_off; // Pretpostavljena funkcija Timer_GetActionScene
    ...
    hButtonTimerSceneSelect = BUTTON_CreateEx(timer_settings_screen_layout.scene_select_btn_pos.x,
                              ...
                              bmicons_button_select_40_sqaured.XSize, // Automatsko preuzimanje Å¡irine
...
//    BUTTON_SetText(hButtonTimerSceneSelect, lng(TXT_SCENES));
    ...
    hButtonTimerSave = BUTTON_CreateEx(timer_settings_screen_layout.save_button_pos.x,
                                       ...
                                       bmicons_button_save_50_squared.XSize, // Automatsko preuzimanje Å¡irine
...
//    BUTTON_SetText(hButtonTimerSave, lng(TXT_SAVE));
    ...
    hButtonTimerCancel = BUTTON_CreateEx(timer_settings_screen_layout.cancel_button_pos.x,
                                         ...
                                         bmicons_button_cancel_50_squared.XSize, // Automatsko preuzimanje Å¡irine
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete sa ekrana za podeÅ¡avanje alarma.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija je ispravljena da briÅ¡e samo widgete koji se zaista
 * kreiraju u `DSP_InitSettingsTimerScreen` funkciji, Äime se
 * rjeÅ¡ava greÅ¡ka "undeclared identifier" prilikom kompilacije.
 ******************************************************************************
 */
static void DSP_KillSettingsTimerScreen(void)
{
    // Brisanje handle-ova za widgete podeÅ¡avanja vremena
    if (WM_IsWindow(hButtonTimerHourUp)) WM_DeleteWindow(hButtonTimerHourUp);
    if (WM_IsWindow(hButtonTimerHourDown)) WM_DeleteWindow(hButtonTimerHourDown);
    ...
    // Brisanje handle-ova za akcijske i navigacione dugmiÄ‡e
    if (WM_IsWindow(hButtonTimerBuzzer)) WM_DeleteWindow(hButtonTimerBuzzer);
    if (WM_IsWindow(hButtonTimerScene)) WM_DeleteWindow(hButtonTimerScene);
    if (WM_IsWindow(hButtonTimerSceneSelect)) WM_DeleteWindow(hButtonTimerSceneSelect);
    if (WM_IsWindow(hButtonTimerSave)) WM_DeleteWindow(hButtonTimerSave);
    ...
    // Postavljanje fleg-a na false kako bi se osigurala ponovna inicijalizacija pri sljedeÄ‡em ulasku
...
/**
 ******************************************************************************
 * @brief       Kreira i inicijalizuje sve GUI widgete za ekran podeÅ¡avanja datuma i vremena.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija dinamiÄki kreira kompletan korisniÄki interfejs,
 * koristeÄ‡i iskljuÄivo parametre definisane u `datetime_settings_layout`
 * strukturi. Implementira specifiÄan 3+2 raspored elemenata
 * (3 u gornjem redu za datum, 2 u donjem za vrijeme) kroz dvije
 * odvojene petlje, Äime se eliminiÅ¡u sve fiksne ("magiÄne")
 * vrijednosti iz koda.
 * @param       None
 * @retval      None
 ******************************************************************************
...
static void DSP_InitSettingsDateTimeScreen(void)
{
    ...
    // Nizovi za lakÅ¡e kreiranje elemenata u petljama
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete kreirane za ekran podeÅ¡avanja vremena.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija se poziva prilikom napuÅ¡tanja ekrana
 * SCREEN_SETTINGS_DATETIME, bez obzira na razlog (potvrda unosa,
 * izlazak preko menija, screensaver timeout). Sistematski prolazi
 * kroz sve handle-ove, provjerava da li widget postoji i briÅ¡e ga,
 * osiguravajuÄ‡i potpuno ÄiÅ¡Ä‡enje i oslobaÄ‘anje resursa.
 * @param       None
 * @retval      None
 ******************************************************************************
 */
static void DSP_KillSettingsDateTimeScreen(void)
{
    // Petlja briÅ¡e svih 5 TEXT widgeta i 10 BUTTON widgeta za +/-
    for (int i = 0; i < 5; i++) {
...
/**
 ******************************************************************************
 * @brief       Kreira i inicijalizuje sve GUI widgete za glavni ekran "ÄŒarobnjaka".
 * @author      Gemini & [VaÅ¡e Ime]
 ...
 * u mod izmjene izgleda, te "Snimi" i "OtkaÅ¾i".
 * @param       None
 * @retval      None
 ******************************************************************************
 */
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete kreirane za glavni ekran "ÄŒarobnjaka".
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        FINALNA VERZIJA. Ova funkcija sada provjerava i briÅ¡e sve
 * dinamiÄki kreirane widgete ("Promijeni", "ObriÅ¡i",
 * "Detaljna PodeÅ¡avanja") i resetuje njihove handle-ove na nulu,
 * Äime se spreÄava pojava "duhova" na drugim ekranima.
 ******************************************************************************
 */
static void DSP_KillSceneEditScreen(void)
{
...
/**
 ******************************************************************************
 * @brief       [VERZIJA 2.1] Inicijalizuje i iscrtava viÅ¡enamjenski ekran za odabir scene.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Funkcija je aÅ¾urirana da u `TIMER` modu viÅ¡e ne prikazuje
 * opciju "Nijedna", veÄ‡ koristi svih 6 slotova za prikaz
 * postojeÄ‡ih, konfigurisanih scena.
 ******************************************************************************
 */
static void DSP_InitSceneAppearanceScreen(void)
{
    ...
    {
        /********************************************/
        /* MOD: ODABIR POSTOJEÄ†E SCENE ZA TAJMER    */
        /********************************************/
        GUI_DispStringAt("Odaberite Scenu za Alarm", LCD_GetXSize() / 2, 5);

        // KORAK 1: Uklonjen kod za iscrtavanje opcije "Nijedna"

        // KORAK 2: Iscrtaj sve konfigurisane scene, poÄevÅ¡i od prvog slota
        uint8_t display_index = 0; // PoÄinjemo od prvog slota
...
/**
 ******************************************************************************
 * @brief       Kreira i inicijalizuje GUI za prvi korak Äarobnjaka: Odabir UreÄ‘aja.
 * @author      Gemini & [VaÅ¡e Ime]
 ...
 * konfigurisane ureÄ‘aje i dinamiÄki prikazuje checkbox-ove samo
 * za dostupne opcije, Äime se korisniÄki interfejs Äini Äistijim i
 * relevantnijim.
 ******************************************************************************
 */
static void DSP_InitSceneWizDevicesScreen(void)
{
    ...
    // --- KORAK 1: Provjera dostupnosti grupa ureÄ‘aja ---
    ...
    // --- KORAK 2: DinamiÄko kreiranje i pozicioniranje Checkbox-ova ---
    const int chkbx_x = 50;
    const int chkbx_w = 200;
    const int chkbx_h = 30;
    const int y_spacing = 40; // Razmak izmeÄ‘u checkbox-ova
    int16_t current_y = 80;   // PoÄetna Y pozicija
    ...
    if (lights_available)
    {
        ...
        current_y += y_spacing; // PoveÄ‡aj Y poziciju za sljedeÄ‡i element
    }
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete kreirane za ekran odabira ureÄ‘aja u Äarobnjaku.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija se poziva prilikom napuÅ¡tanja ekrana
 * `SCREEN_SCENE_WIZ_DEVICES`. Odgovorna je za brisanje svih dinamiÄki
 * kreiranih widgeta (checkbox-ova i dugmadi) kako bi se oslobodili
 * resursi i sprijeÄilo curenje memorije.
 ******************************************************************************
 */
static void DSP_KillSceneWizDevicesScreen(void)
{
    // Provjeri da li widget postoji prije brisanja da se izbjegne greÅ¡ka
...
/**
 * @brief       UniÅ¡tava sve GUI widgete kreirane za ekran podeÅ¡avanja svjetla.
 * @author      Gemini (po specifikaciji korisnika)
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete kreirane za glavni ekran sa scenama.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija se poziva prilikom napuÅ¡tanja ekrana `SCREEN_SCENE`.
 * PoÅ¡to ovaj ekran nema dinamiÄki kreirane widgete, njena uloga je
...
static void DSP_KillSceneScreen(void)
{
    // Trenutno ovaj ekran nema dinamiÄke widgete, pa samo Äistimo ekran.
    // Ako bismo u buduÄ‡nosti dodali npr. dugmad, ovdje bi iÅ¡ao njihov WM_DeleteWindow poziv.
    GUI_Clear();
}
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI elemente sa ekrana za odabir izgleda scene.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija se poziva prilikom napuÅ¡tanja ekrana
 * `SCREEN_SCENE_APPEARANCE`. S obzirom da ovaj ekran ne koristi
 * dinamiÄki kreirane widgete sa handle-ovima, veÄ‡ samo direktne
 * funkcije za iscrtavanje, njen zadatak je da obriÅ¡e ekran.
 ******************************************************************************
 */
static void DSP_KillSceneAppearanceScreen(void)
{
    GUI_Clear(); // BriÅ¡e kompletan sadrÅ¾aj aktivnog sloja (layer-a)
}
/**
 ******************************************************************************
 * @brief       UniÅ¡tava widgete kreirane od strane `Service_SceneEditLightsScreen`.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        SpecifiÄno briÅ¡e "[Next]" dugme kreirano za wizard mod.
 ******************************************************************************
 */
static void DSP_KillSceneEditLightsScreen(void)
{
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava widgete kreirane od strane `Service_SceneEditCurtainsScreen`.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        SpecifiÄno briÅ¡e "[Next]" dugme kreirano za wizard mod na ekranu
 * za roletne, osiguravajuÄ‡i Äistu navigaciju.
 ******************************************************************************
 */
static void DSP_KillSceneEditCurtainsScreen(void)
{
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava widgete kreirane od strane `Service_SceneEditThermostatScreen`.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        SpecifiÄno briÅ¡e "[Next]" dugme kreirano za wizard mod na ekranu
 * za termostat.
 ******************************************************************************
...
/**
 ******************************************************************************
 * @brief       Kreira GUI za finalni ekran Äarobnjaka.
 * @note        Prikazuje poruku za potvrdu i dugmad "Snimi Scenu" i "OtkaÅ¾i".
 ******************************************************************************
 */
static void DSP_InitSceneWizFinalizeScreen(void)
{
...
//    DSP_DrawWizardScreenHeader("Zavrsetak Podesavanja", buffer);
    ...
    // Kreiramo samo "Snimi" i "OtkaÅ¾i" dugmad
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava widgete sa finalnog ekrana Äarobnjaka.
 ******************************************************************************
 */
static void DSP_KillSceneWizFinalizeScreen(void)
{
...
/**
 * @brief UniÅ¡tava elemente sa ekrana aktivnog alarma.
 */
static void DSP_KillAlarmActiveScreen(void)
{
    GUI_Clear();
 ...
 * DinamiÄki kreira set kontrolnih dugmadi na osnovu "Profila Kontrole"
 ...
 * Ovo korisniku pruÅ¾a intuitivan interfejs prilagoÄ‘en naÄinu rada ureÄ‘aja.
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete sa ekrana za detaljnu kontrolu kapije.
 * @author      Gemini
 * @note        Ova funkcija se poziva prilikom napuÅ¡tanja ekrana
 * `SCREEN_GATE_SETTINGS` kako bi se oslobodili resursi. Prolazi
 * kroz niz `hGateControlButtons` i briÅ¡e sve handle-ove koji su
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete sa ekrana za  kontrolu kapije.
 * @author      Gemini
 * @note        Ova funkcija se poziva prilikom napuÅ¡tanja ekrana
 ******************************************************************************
 */
static void DSP_KillGateScreen(void)
{
    // Ovaj ekran nema dinamiÄke widgete, pa samo Äistimo ekran.
    GUI_Clear();
}
/**
 ******************************************************************************
 * @brief       Upravlja svim periodiÄnim dogaÄ‘ajima i tajmerima.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija je srÅ¾ logike za pozadinske procese koji se ne
 * odnose direktno na trenutni ekran. UkljuÄuje logiku
 * screensaver-a, tajmera za automatsko paljenje svjetala,
 * detekciju dugog pritiska i aÅ¾uriranje vremena. Refaktorisana
 * je da koristi iskljuÄivo javne API-je drugih modula.
 ******************************************************************************
 */
static void Handle_PeriodicEvents(void)
{
    ...
    if (gate_press_timer_start != 0 && (HAL_GetTick() - gate_press_timer_start) > LONG_PRESS_DURATION)
    {
        ...
        // Pokreni tranziciju na novi ekran za podeÅ¡avanja kapije
        DSP_KillGateScreen();                  // 1. Ubij stari ekran (dashboard)
        DSP_InitGateSettingsScreen();          // 2. Inicijalizuj novi ekran (podeÅ¡avanja)
        screen = SCREEN_GATE_SETTINGS;         // 3. Postavi novo stanje
        shouldDrawScreen = 0;                  // 4. Ne treba ponovo crtati jer je Init to veÄ‡ uradio
    }
    // === KRAJ AÅ½URIRANOG BLOKA ===
    ...
    if (rename_light_timer_start && (HAL_GetTick() - rename_light_timer_start) >= 2000)
    {
        ...
        return; // Prekini dalje izvrÅ¡avanje u ovoj funkciji
    }
    ...
    if ((HAL_GetTick() - ghost_widget_scan_timer) >= GHOST_WIDGET_SCAN_INTERVAL) {
        ...
        // AÅ¾urirano da koristi novi `SCREEN_SELECT_LAST`
            ...
            if (handle) {
                if (LIGHT_GetOnHour(handle) != -1) {
                    if ((LIGHT_GetOnHour(handle) == currentHour) && (LIGHT_GetOnMinute(handle) == currentMinute)) {
                        ...
                        // Logika za aÅ¾uriranje ekrana ostaje ista
    ...
    // === TAJMER ZA ULAZAK U MOD PODEÅ AVANJA (Ovaj dio ostaje nepromijenjen) ===
    ...
    if (dynamic_icon_alarm_press_timer != 0 && (HAL_GetTick() - dynamic_icon_alarm_press_timer) > LONG_PRESS_DURATION)
    {
        ...
        // Pokreni tranziciju na ekran za PODEÅ AVANJA alarma.
    ...
    if (dynamic_icon_timer_press_timer != 0 && (HAL_GetTick() - dynamic_icon_timer_press_timer) > LONG_PRESS_DURATION)
    {
        ...
        // Pokreni tranziciju na ekran za PODEÅ AVANJA tajmera.
        Timer_Suppress();
    ...
    if (!IsScrnsvrActiv()) {
        if ((HAL_GetTick() - scrnsvr_tmr) >= (uint32_t)(g_display_settings.scrnsvr_tout * 1000)) {
            // =======================================================================
            // ===          NOVI BLOK: Provjera i gaÅ¡enje ÄŒarobnjaka prvo         ===
            // =======================================================================
            if (is_in_scene_wizard_mode)
            {
                // UniÅ¡ti widgete sa bilo kog ekrana koji je dio Äarobnjaka
                switch (screen)
                {
                case SCREEN_SCENE_EDIT:
                    DSP_KillSceneEditScreen();
                ...
                // KljuÄni korak: Resetuj fleg za Wizard Mode
            ...
            // =======================================================================
            // ===        Originalna logika za ekrane podeÅ¡avanja ostaje         ===
            // =======================================================================
    ...
    if (HAL_GetTick() - rtctmr >= 1000) {
        ...
        // Poziv za iscrtavanje sata (logika Ä‡e biti doraÄ‘ena kako smo diskutovali)
        if (screen < SCREEN_SELECT_1) DISPDateTime();
    }
}
/**
 ******************************************************************************
 * @brief       Centralni dispeÄer za obradu dogaÄ‘aja PRITISKA na ekran.
 * @author      [Original Author] & Gemini
 * @note        Ova funkcija se poziva iz `PID_Hook`. Njena uloga je da, na osnovu
 * trenutnog ekrana, pozove odgovarajuÄ‡u `HandlePress_...` funkciju.
 * Za `SCREEN_MAIN`, poziva logiku za menije i pamti koordinate pritiska.
 * @param       pTS PokazivaÄ na strukturu sa stanjem dodira.
 * @param       click_flag PokazivaÄ na fleg za zvuÄni signal.
 ******************************************************************************
 */
static void HandleTouchPressEvent(GUI_PID_STATE * pTS, uint8_t *click_flag)
{
    if     (screen == SCREEN_MAIN) 
    {
        ...
        // Provjera 2: Da li je dodir unutar CENTRALNOG GLAVNOG PREKIDAÄŒA?
                 ...
                 pTS->y <  reset_menu_switches_layout.main_switch_zone.y1)
        {
            *click_flag = 1;
            ...
            // pozivamo funkciju koja pokreÄ‡e tajmer za dugi pritisak.
            HandlePress_MainScreenSwitch(pTS);
        }

        // Ako dodir nije bio ni u jednoj od ove dvije zone, ne radi se niÅ¡ta.
        // OnemoguÄ‡en je `else` blok koji je uzrokovao greÅ¡ku.

        // Uvijek saÄuvaj koordinate pritiska za kasniju provjeru pri otpuÅ¡tanju
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje dogaÄ‘aj otpuÅ¡tanja dodira sa ekrana, u zavisnosti od ekrana.
 * @author      [Original Author] & Gemini
 * @note        Ova funkcija je centralna taÄka za logiku koja se deÅ¡ava nakon
 * Å¡to korisnik podigne prst sa ekrana. SadrÅ¾i originalnu, kompletnu
 * logiku za sve ekrane, sa dodatkom provjere zone za glavni prekidaÄ.
 * @param       pTS PokazivaÄ na strukturu sa stanjem dodira.
 ******************************************************************************
 */
static void HandleTouchReleaseEvent(GUI_PID_STATE * pTS)
{
    // Provjera 1: Da li je otpuÅ¡tanje prsta rezultat uspjeÅ¡nog dugog pritiska?
    // Ako je tastatura veÄ‡ aktivirana, 'screen' varijabla Ä‡e biti SCREEN_KEYBOARD_ALPHA.
    if (screen == SCREEN_KEYBOARD_ALPHA)
    {
        // U ovom sluÄaju, dugi pritisak je uspio i ekran se veÄ‡ mijenja.
        // Jedino Å¡to treba da uradimo je da resetujemo tajmer i izaÄ‘emo.
        rename_light_timer_start = 0;
        return;
    }

    // Ako NIJE bio uspjeÅ¡an dugi pritisak, nastavljamo sa standardnom logikom.

    // PoniÅ¡ti tajmer za dugi pritisak (za sluÄaj kratkog pritiska)
    rename_light_timer_start = 0;
    ...
    // =======================================================================
    // === POÄŒETAK IZMJENE: Logika za MAIN_SCREEN sa provjerom zone ===
    // =======================================================================
    if(screen == SCREEN_MAIN && !touch_in_menu_zone) 
    {
        // Provjerava se da li je POÄŒETNI PRITISAK bio unutar zone
    ...
    // =======================================================================
    // === KRAJ IZMJENE (Ostatak funkcije je VaÅ¡ originalni, saÄuvani kod) ===
    // =======================================================================
    else if(screen == SCREEN_LIGHTS) 
    {
    ...
    // === POÄŒETAK BLOKA ZA ZAMJENU: LOGIKA ZA KRATAK PRITISAK NA KAPIJU ===
    else if (screen == SCREEN_GATE) 
    {
        // Provjeravamo da li je tajmer bio aktivan. Ako je dugi pritisak obraÄ‘en,
        ...
        if (gate_press_timer_start != 0) {
            if (gate_pressed_index != -1) {
                ...
                if (handle) {
                    ...
                    // Implementacija state maÅ¡ine taÄno prema korisniÄkoj specifikaciji
                    ...
                    case GATE_STATE_PARTIALLY_OPEN:
                        // Ako je djelimiÄno otvorena -> pokreni zatvaranje
                    ...
                    default:
                        // U nedefinisanim ili stanjima greÅ¡ke, pokreÄ‡emo ciklus otvaranja kao najsigurniju akciju
                        presumed_next_state = GATE_STATE_OPENING;
                        Gate_TriggerFullCycleOpen(handle);
                        break;
                    }

                    // Ako je doÅ¡lo do promjene stanja, aÅ¾uriraj UI ("Optimistic UI Update")
                    if (presumed_next_state != current_state) {
                        // 1. Odmah aÅ¾uriraj stanje u RAM-u radi trenutnog vizuelnog odziva.
                        Gate_SetState(handle, presumed_next_state);
                        // 2. ZatraÅ¾i ponovno iscrtavanje ekrana da se prikaÅ¾e nova ikonica/stanje.
                        shouldDrawScreen = 1;
                    }
                }
            }
        }
        // U svakom sluÄaju, resetuj tajmer i stanje pritiska pri otpuÅ¡tanju
        gate_press_timer_start = 0;
        gate_pressed_index = -1;
    }
    ...
    // =======================================================================
    // Logika se izvrÅ¡ava ISKLJUÄŒIVO ako smo na ekranu SCREEN_SELECT_2
    else if (screen == SCREEN_SELECT_2)
    {
    ...
    // =======================================================================
    // Resetovanje svih opÅ¡tih kontrolnih flegova.
    btnset = 0;
    btndec = 0U;
    btninc = 0U;
...
/**
 * @brief Obrada dogaÄ‘aja pritiska za ekran "Control Select".
 * @note  Verzija 2.3.3: Ispravljena greÅ¡ka gdje nisu bili inicijalizovani
 * handle-ovi za module, Å¡to je spreÄavalo ispravan rad funkcije.
 * @param pTS PokazivaÄ na strukturu sa stanjem dodira.
 * @param click_flag PokazivaÄ na fleg koji se postavlja ako treba generisati zvuÄni signal.
 */
static void HandlePress_SelectScreen1(GUI_PID_STATE * pTS, uint8_t *click_flag)
{
    ...
    // --- KORAK 1: Detekcija aktivnih modula (identiÄno kao u Service_... funkciji) ---
    typedef struct {
        const GUI_BITMAP* icon;
        TextID text_id;
        eScreen target_screen;
        bool is_dynamic_toggle; // Fleg za dinamiÄku ikonu
    } DynamicMenuItem;
    ...
    DynamicMenuItem all_modules[] = {
        ...
        { NULL, TXT_DUMMY, SCREEN_SELECT_1, true } // Mjesto za dinamiÄku ikonu
    };
    ...
    // --- KORAK 2: DinamiÄko raÄunanje zona dodira i provjera ---
    TouchZone_t zone;
    bool touched = false;
    ...
    if (touched) {
        // Ako je pritisnuta dinamiÄka ikona, uradi toggle
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje dogaÄ‘aj pritiska za drugi ekran za odabir (SCREEN_SELECT_2).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija dinamiÄki izraÄunava koja je ikonica pritisnuta na osnovu
 * aktivnih modula. Za ikonice Alarma i Tajmera, ova funkcija sada
 * pokreÄ‡e tajmere (`dynamic_icon_alarm_press_timer`, `dynamic_icon_timer_press_timer`)
 * kako bi se omoguÄ‡ila detekcija dugog pritiska. Za sve ostale ikonice,
 * ponaÅ¡anje ostaje nepromijenjeno.
 * @param[in]   pTS PokazivaÄ na strukturu sa stanjem dodira (`GUI_PID_STATE`).
 * @param[out]  click_flag PokazivaÄ na fleg koji signalizira potrebu za zvuÄnim signalom.
 * @retval      None
 ******************************************************************************
 */
static void HandlePress_SelectScreen2(GUI_PID_STATE * pTS, uint8_t *click_flag)
{
    // Resetuj tajmere na poÄetku svake obrade pritiska
    ...
    // --- DinamiÄko raÄunanje zona dodira (nepromijenjeno) ---
    TouchZone_t zone;
    ...
    // --- Obrada rezultata dodira (AÅ½URIRANA LOGIKA) ---
    if (touched_module_index != -1)
    {
        *click_flag = 1;
        ...
        {
            // PostojeÄ‡a logika za ostale dinamiÄke ikonice ostaje nepromijenjena
        ...
        {
            // Za sve ostale statiÄke ikonice (npr. Gate), odmah mijenjaj ekran
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje dogaÄ‘aj pritiska za posljednji ekran za odabir.
 * @author      [Original Author] & Gemini
 * @note        FINALNA ISPRAVLJENA VERZIJA. Logika je restrukturirana da
 * ispravno rukuje specijalnim sluÄajem poziva PIN tastature. Nakon
 * poziva `Display_ShowNumpad`, funkcija se odmah zavrÅ¡ava (`return`)
 * kako bi se sprijeÄilo pogreÅ¡no vraÄ‡anje na prethodni ekran.
 * @param       pTS PokazivaÄ na strukturu sa stanjem dodira.
 * @param       click_flag PokazivaÄ na fleg za zvuÄni signal.
 ******************************************************************************
 */
static void HandlePress_SelectScreenLast(GUI_PID_STATE * pTS, uint8_t *click_flag)
{
    // Provjera dodira na zonu za ÄŒIÅ Ä†ENJE
    ...
    // Provjera dodira na zonu za PODEÅ AVANJA
        ...
        NumpadContext_t pin_context = {
            .title = lng(TXT_ALARM_ENTER_PIN), // KORISTI POSTOJEÄ†I PREVOD
            .initial_value = "",
            .min_val = 0,
            .max_val = 9999,
            .max_len = 4,
        ...
        *click_flag = 1;
        ...
        // =======================================================================
        // ===       KLJUÄŒNA ISPRAVKA: Odmah izaÄ‘i iz funkcije               ===
        // =======================================================================
        // Nakon poziva `Display_ShowNumpad`, koji je veÄ‡ postavio novi ekran,
        // moramo odmah izaÄ‡i da sprijeÄimo dalju logiku u ovoj funkciji da
        // pogreÅ¡no vrati ekran na staru vrijednost.
        return;
    }
    // Provjera dodira na "NEXT" dugme
    ...
    // =======================================================================
    // ===       SAÄŒUVANA BLOKIRAJUÄ†A PETLJA ZA SPREÄŒAVANJE           ===
    // =======================================================================
    // Ova petlja se izvrÅ¡ava samo ako je doÅ¡lo do promjene ekrana
    if (*click_flag) {
...
/**
 * @brief Obrada dogaÄ‘aja pritiska za ekran "Thermostat".
 * @note Rukuje dodirima na zone za poveÄ‡anje (+) i smanjenje (-) zadane temperature,
 * kao i detekcijom poÄetka dugog pritiska za paljenje/gaÅ¡enje termostata.
 * Koordinate zona dodira su definisane kao lokalne konstante radi bolje
 * enkapsulacije i Äitljivosti koda.
 * @param pTS PokazivaÄ na strukturu sa stanjem dodira.
 * @param click_flag PokazivaÄ na fleg koji se postavlja ako treba generisati zvuÄni signal.
 */
static void HandlePress_ThermostatScreen(GUI_PID_STATE * pTS, uint8_t *click_flag)
{
    // Provjera da li je dodir unutar zone za POVEÄ†ANJE temperature
             ...
             (pTS->y >= thermostat_layout.on_off_zone.y0) && (pTS->y < thermostat_layout.on_off_zone.y1))
    {
        *click_flag = 1; // Signaliziraj "klik"
        ...
        // Sama logika gaÅ¡enja/paljenja se nalazi u `Service_ThermostatScreen` funkciji
...
/**
 * @brief Obrada dogaÄ‘aja pritiska za ekran "Lights".
 * @note  Detektuje dodir na ikonu specifiÄnog svjetla. Ako svjetlo nije binarno (dimer/RGB),
 * pokreÄ‡e tajmer za dugi pritisak kako bi se omoguÄ‡io ulazak u meni za podeÅ¡avanja.
 * Refaktorisana je da koristi iskljuÄivo `lights` API i centralizovanu layout strukturu.
 * @param pTS PokazivaÄ na strukturu sa stanjem dodira.
 * @param click_flag PokazivaÄ na fleg koji se postavlja ako treba generisati zvuÄni signal.
 */
static void HandlePress_LightsScreen(GUI_PID_STATE * pTS, uint8_t *click_flag)
{
    // Resetujemo globalne varijable stanja na poÄetku obrade dodira.
    light_selectedIndex = LIGHTS_MODBUS_SIZE + 1;
    light_settingsTimerStart = 0;

    // Logika za dinamiÄki raspored i detekciju dodira.
    ...
    for(uint8_t row = 0; row < LIGHTS_Rows_getCount(); ++row) {
        ...
        for(uint8_t i_light = 0; i_light < lightsInRow; ++i_light) {
            ...
            // koristeÄ‡i dimenzije iz `lights_screen_layout` strukture.
            if((pTS->x > x) && (pTS->x < (x + lights_screen_layout.icon_width)) &&
                    (pTS->y > y) && (pTS->y < (y + lights_screen_layout.icon_height)))
            {
                *click_flag = 1; // Signaliziraj "klik"
                light_selectedIndex = lightsInRowSum + i_light; // ZabiljeÅ¾i koje je svjetlo pritisnuto

                LIGHT_Handle* handle = LIGHTS_GetInstance(light_selectedIndex);

                if (handle) {
                    // Sada se tajmer za dugi pritisak pokreÄ‡e za SVE tipove svjetala.
                    light_settingsTimerStart = HAL_GetTick();
                }

                // Zaustavljamo noÄ‡ni tajmer ako je bio aktivan.
                LIGHTS_StopNightTimer();

                // Prekidamo obje petlje jer smo pronaÅ¡li pritisnutu ikonicu.
                goto exit_loops;
            }
        }
...
exit_loops:; // Labela za izlazak iz ugnijeÅ¾denih petlji
}

/**
 * @brief Obrada dogaÄ‘aja pritiska za ekran "Curtains".
 * @note  Rukuje pritiskom na trouglove za pomjeranje GORE/DOLE,
 * kao i strelicama za prebacivanje izmeÄ‘u pojedinaÄnih roletni i grupe "SVE".
...
static void HandlePress_CurtainsScreen(GUI_PID_STATE * pTS, uint8_t *click_flag)
{
    ...
    // Provjera da li je dodir u zoni za PRETHODNU roletnu (samo ako ima viÅ¡e od jedne)
    ...
    // Provjera da li je dodir u zoni za SLJEDEÄ†U roletnu (samo ako ima viÅ¡e od jedne)
    else if (Curtains_getCount() > 1 &&
...
/**
 ******************************************************************************
 * @brief       Obrada dogaÄ‘aja pritiska za ekran "Light Settings".
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        KONAÄŒNA ISPRAVKA: Ova verzija sadrÅ¾i kompletan kod. Ispravno
 * provjerava da li je dodir u zoni za izmjenu naziva i pokreÄ‡e
 * tajmer. Ako nije, izvrÅ¡ava kompletnu, originalnu logiku za
 * obradu dodira na slajderu svjetline i paleti boja.
 ******************************************************************************
 */
static void HandlePress_LightSettingsScreen(GUI_PID_STATE * pTS)
{
    // Provjera da li je dodir unutar definisane zone za izmjenu naziva (samo za pojedinaÄna svjetla)
    ...
    {
        // =======================================================================
        // === POÄŒETAK KOMPLETNOG, ORIGINALNOG KODA ZA SLAJDERE I PALETU ===
        // =======================================================================
...
/**
 * @brief Obrada dogaÄ‘aja pritiska u zoni glavnog prekidaÄa na ekranu.
 * @note PokreÄ‡e tajmer za dugi pritisak kako bi se uÅ¡lo u podeÅ¡avanja
...
static void HandlePress_MainScreenSwitch(GUI_PID_STATE * pTS)
{
    ...
    if((!g_display_settings.leave_scrnsvr_on_release) || (g_display_settings.leave_scrnsvr_on_release && (!IsScrnsvrActiv()))) {
        ...
        // Resetujemo flegove na poÄetku
        light_selectedIndex = LIGHTS_MODBUS_SIZE + 1; // Koristi se kao fleg da li je dimer/RGB pronaÄ‘en
        lights_allSelected_hasRGB = false;

        // =======================================================================
        // === POÄŒETAK REFAKTORISANJA ===
        //
        // Prolazimo kroz sva konfigurisana svjetla koristeÄ‡i novi API.
        for(uint8_t i = 0; i < LIGHTS_getCount(); i++) {
            LIGHT_Handle* handle = LIGHTS_GetInstance(i);

            // Provjeravamo da li handle postoji i da li je svjetlo vezano za glavni prekidaÄ.
            if (handle && LIGHT_isTiedToMainLight(handle) && !LIGHT_isBinary(handle)) {
                // Ako smo pronaÅ¡li bar jedno svjetlo koje nije binarno (dimer ili RGB),
        ...
        // Ako smo pronaÅ¡li bar jedno dimer/RGB svjetlo, pokreÄ‡emo tajmer za dugi pritisak.
        if(light_selectedIndex == LIGHTS_MODBUS_SIZE) {
            light_settingsTimerStart = HAL_GetTick();
        }
    }
...
/**
 * @brief Obrada otpuÅ¡tanja dodira na glavnom ekranu (glavni prekidaÄ).
 * @note Ova funkcija implementira "toggle" logiku za sva svjetla koja su povezana
 * sa glavnim prekidaÄem. TakoÄ‘e upravlja pokretanjem i zaustavljanjem
 * noÄ‡nog tajmera. Refaktorisana je da bude drastiÄno jednostavnija
 * koriÅ¡tenjem novog `lights` API-ja.
 * @param pTS PokazivaÄ na strukturu sa stanjem dodira.
 */
static void HandleRelease_MainScreenSwitch(GUI_PID_STATE * pTS)
{
    ...
    // << KLJUÄŒNA ISPRAVKA: Resetovanje tajmera za dugi pritisak >>
    // Ovo osigurava da se, nakon kratkog pritiska, ne aktivira
    // meni za podeÅ¡avanje svjetala.
    light_settingsTimerStart = 0;
    ...
    // i vraÄ‡a `true` ako je ijedno upaljeno.
    bool isAnyLightCurrentlyOn = LIGHTS_isAnyLightOn();

    // KORAK 2: OdreÄ‘ujemo novo, Å¾eljeno stanje.
    // Ako je bilo Å¡ta bilo upaljeno, novo stanje je "ugasi sve".
    // Ako je sve bilo ugaÅ¡eno, novo stanje je "upali sve".
    ...
    for(uint8_t i = 0; i < LIGHTS_getCount(); ++i) {
        ...
        // ...i ako su vezana za glavni prekidaÄ, postavljamo im novo stanje.
        if (handle && LIGHT_isTiedToMainLight(handle)) {
            LIGHT_SetState(handle, newStateIsOn);
        }
    }

    // KORAK 4: Upravljanje noÄ‡nim tajmerom preko novog, Äistog API-ja.
    // Provjeravamo da li je noÄ‡ni tajmer omoguÄ‡en u postavkama i da li je noÄ‡.
    if (g_display_settings.light_night_timer_enabled && !((Bcd2Dec(rtctm.Hours) > 6) && (Bcd2Dec(rtctm.Hours) < 20))) {
        ...
        // Ako je dan ili je tajmer onemoguÄ‡en, osiguravamo da je zaustavljen.
        LIGHTS_StopNightTimer();
    }
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje dogaÄ‘aj pritiska za glavni ekran "ÄŒarobnjaka" scena.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija detektuje pritisak na tri moguÄ‡a dugmeta: "Snimi",
 * "OtkaÅ¾i" i "[ Promijeni ]". Na osnovu pritisnutog dugmeta,
 * izvrÅ¡ava odgovarajuÄ‡u akciju: snima scenu, otkazuje izmjene
 * ili pokreÄ‡e novi ekran za odabir izgleda scene.
 * @param       pTS PokazivaÄ na strukturu sa stanjem dodira (nije koriÅ¡ten).
 * @param       click_flag PokazivaÄ na fleg za generisanje zvuÄnog signala.
 ******************************************************************************
 */
static void HandlePress_SceneEditScreen(GUI_PID_STATE* pTS, uint8_t* click_flag)
{
    ...
    {
        *click_flag = 1; // Aktiviraj zvuÄni signal
        // Pozovi backend da "uslika" trenutno stanje svih relevantnih ureÄ‘aja
        Scene_Memorize(scene_edit_index);
        // Pozovi backend da snimi sve scene (ukljuÄujuÄ‡i novu izmjenu) u EEPROM
        Scene_Save();

        // Vrati se na ekran sa pregledom svih scena
        DSP_KillSceneEditScreen(); // ObriÅ¡i widgete prije prelaska
        screen = SCREEN_SCENE;
        shouldDrawScreen = 1;
    }
    // Provjera pritiska na dugme "OtkaÅ¾i" (ID_Next)
...
/**
 ******************************************************************************
 * @brief       [VERZIJA 2.2 - KOMPLETNA] ObraÄ‘uje dogaÄ‘aj pritiska za viÅ¡enamjenski ekran odabira.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija je svjesna konteksta. Na osnovu `current_scene_picker_mode`:
 * - U `WIZARD` modu: Mijenja izgled scene koja se kreira i vraÄ‡a na editor. SadrÅ¾i
 * kompletnu logiku za paginaciju ("Next" dugme).
 * - U `TIMER` modu: BiljeÅ¾i indeks odabrane scene u statiÄku varijablu
 * `timer_selected_scene_index` i vraÄ‡a se na ekran koji je pozvao
 * ovaj biraÄ (podeÅ¡avanje tajmera).
 * @param       pTS PokazivaÄ na strukturu sa stanjem dodira.
 * @param       click_flag PokazivaÄ na fleg za generisanje zvuÄnog signala.
 ******************************************************************************
 */
    ...
    if (current_scene_picker_mode == SCENE_PICKER_MODE_TIMER)
    {
        ...
        for (int i = 0; i < SCENE_MAX_COUNT; i++) {
            ...
            if (scene_handle && scene_handle->is_configured) {
                if (configured_scene_counter == touched_display_index) {
                    timer_selected_scene_index = i; // Spremi stvarni fiziÄki indeks scene
                    break;
                }
                ...
                pTS->y >= next_button_zone.y0 && pTS->y < next_button_zone.y1)
        {
            *click_flag = 1;
            ...
            // Ponovo filtriraj da dobijeÅ¡ taÄan broj stranica
            uint8_t used_appearance_ids[SCENE_MAX_COUNT] = {0};
            uint8_t used_count = 0;
            for (int i = 0; i < SCENE_MAX_COUNT; i++)
            {
        ...
        // Ponovo filtriraj listu da pronaÄ‘eÅ¡ taÄno koja je ikonica pritisnuta
        int total_available_appearances = 0;
        const SceneAppearance_t* available_appearances[sizeof(scene_appearance_table)/sizeof(SceneAppearance_t)];
        uint8_t used_appearance_ids[SCENE_MAX_COUNT] = {0};
        uint8_t used_count = 0;
        ...
        if (actual_index_in_available_list < total_available_appearances)
        {
            *click_flag = 1;
            ...
            // PronaÄ‘i originalni ID u `scene_appearance_table`
            ...
            DSP_InitSceneEditScreen();          // 2. Inicijalizuj prethodni ekran (Äarobnjaka)
            screen = SCREEN_SCENE_EDIT;         // 3. Postavi novo stanje
            shouldDrawScreen = 0;               // 4. Ne treba ponovo crtati
        }
    }
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje dogaÄ‘aj PRITISKA za ekran sa scenama (SCREEN_SCENE).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Verzija 2.0: Ova funkcija sada samo biljeÅ¾i koji je slot
 * dodirnut i pokreÄ‡e tajmer za mjerenje trajanja pritiska.
 ******************************************************************************
 */
static void HandlePress_SceneScreen(GUI_PID_STATE * pTS, uint8_t *click_flag)
{
    ...
    // --- Provjera dodira na ikonicu ÄŒarobnjaka ---
    ...
    {
        *click_flag = 1;
        scene_pressed_index = configured_scenes_count; // Postavi indeks na poziciju Äarobnjaka
        scene_press_timer_start = HAL_GetTick() ? HAL_GetTick() : 1;
    }
    // --- Provjera dodira na MreÅ¾u sa Scenama ---
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje dogaÄ‘aj pritiska za ekran detaljne kontrole kapije.
 * @author      Gemini
 * @note        Funkcija provjerava da li je neko od dinamiÄki kreiranih dugmadi
 * pritisnuto. Ako jeste, na osnovu ID-ja dugmeta (koji odgovara
 * `UI_Command_e` enumu), poziva odgovarajuÄ‡u `Gate_Trigger...()` API
 * funkciju za izvrÅ¡avanje komande.
 ******************************************************************************
 */
static void HandlePress_GateSettingsScreen(GUI_PID_STATE* pTS, uint8_t* click_flag)
{
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje dogaÄ‘aj pritiska za "dashboard" ekran sa kapijama.
 * @author      Gemini
 * @note        Ova funkcija se poziva iz `HandleTouchPressEvent` kada je `screen`
 * postavljen na `SCREEN_GATE`. Njen zadatak je da, koristeÄ‡i istu
 * logiku za raspored kao i `Service_GateScreen`, odredi koja je
 * ikonica pritisnuta. Ako je pritisak validan, biljeÅ¾i indeks
 * pritisnute ikonice i pokreÄ‡e tajmer za mjerenje trajanja pritiska.
 * @param[in]   pTS PokazivaÄ na strukturu sa stanjem dodira.
 * @param[out]  click_flag PokazivaÄ na fleg za generisanje zvuÄnog signala.
 ******************************************************************************
 */
static void HandlePress_GateScreen(GUI_PID_STATE * pTS, uint8_t *click_flag)
{
    ...
    if (gate_count == 0 || pTS->x >= DRAWING_AREA_WIDTH) {
        return; // Nema Å¡ta da se pritisne ili je pritisak u zoni menija
    }
    ...
    // OdreÄ‘ivanje reda na osnovu Y koordinate
    uint8_t row_touched = (pTS->y - y_row_start) / y_row_height;
    if (row_touched >= rows) return;

    // OdreÄ‘ivanje broja stavki u dodirnutom redu
    ...
    // OdreÄ‘ivanje kolone na osnovu X koordinate
    ...
    // IzraÄunavanje finalnog indeksa
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje dogaÄ‘aj OTPUÅ TANJA za ekran sa scenama (SCREEN_SCENE).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        FINALNA VERZIJA: Ova funkcija obraÄ‘uje iskljuÄivo KRATAK KLIK.
 * Ispravno aktivira postojeÄ‡u scenu ili pokreÄ‡e Äarobnjaka za
 * kreiranje nove, poÅ¡tujuÄ‡i "Kill -> Init" navigacioni patern.
 ******************************************************************************
 */
static void HandleRelease_SceneScreen(void)
{
    // Logika ostaje ista, jer je `HandlePress_SceneScreen` veÄ‡ ispravno
    // postavio `scene_pressed_index`.

    if (scene_press_timer_start == 0) return; // Dugi pritisak je veÄ‡ obraÄ‘en
    ...
    if ((HAL_GetTick() - scene_press_timer_start) < LONG_PRESS_DURATION)
    {
        ...
        {
            // Kratak klik na postojeÄ‡u scenu -> Aktiviraj je
            uint8_t scene_counter = 0;
            for (int i = 0; i < SCENE_MAX_COUNT; i++)
            {
                Scene_t* temp_handle = Scene_GetInstance(i);
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje dogaÄ‘aj pritiska za glavni ekran tajmera.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Funkcija je aÅ¾urirana prema preciznim uputstvima.
 * Blok koda `if (!IsRtcTimeValid())` je prekopiran iz VaÅ¡e radne verzije
    ...
    if (!IsRtcTimeValid())
    {
        ...
        if (pTS->x < DRAWING_AREA_WIDTH)
        {
            *click_flag = 1;
            ...
            // 3. Resetuj fleg za crtanje jer je ekran veÄ‡ iscrtan.
            shouldDrawScreen = 0;

            return; // Izlazimo odmah da sprijeÄimo dalju obradu dodira.
        }
    }
    // ====================================================================
    // === NOVA IMPLEMENTACIJA ZA SLUÄŒAJ KADA JE VRIJEME PODEÅ ENO ===
    // ====================================================================
    else
    {
        // DefiniÅ¡emo zonu dodira za 'toggle' dugme.
 ...
 * Sada je univerzalna i konfiguriÅ¡e se Äitanjem parametara iz
 * statiÄke `g_numpad_context` strukture. DinamiÄki iscrtava
 * tastaturu i naslov prema proslijeÄ‘enom kontekstu.
 * @param       None
 * @retval      None
 *****************************************************************************/
static void DSP_InitNumpadScreen(void)
{
    ...
    // Tekstovi i ID-jevi za dugmad se definiÅ¡u dinamiÄki
    ...
    // DinamiÄka konfiguracija zadnje tri tipke na osnovu konteksta
    ...
    // Inicijalizacija i iscrtavanje poÄetnog stanja
    pin_buffer_idx = 0;
...
/**
 ******************************************************************************
 * @brief       Servisira univerzalni numeriÄki keypad (KONAÄŒNA ISPRAVLJENA VERZIJA).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova verzija je bazirana na originalnoj, ispravnoj logici koju ste
 * priloÅ¾ili. VraÄ‡ena je "Optimistic UI" funkcionalnost gdje se status
 * "NaoruÅ¾avanje/RazoruÅ¾avanje" prikazuje ODMAH nakon unosa ispravnog PIN-a.
 * PaÅ¾ljivo je integrisana nova state-machine (`pin_change_state`) za
 * trostupanjsku promjenu PIN-a, bez naruÅ¡avanja postojeÄ‡ih funkcionalnosti.
 ******************************************************************************
 */
static void Service_NumpadScreen(void)
{
    ...
    if (currently_pressed_id == -1 && button_pressed_id != -1) {
        ...
        if (Id >= ID_PINPAD_0 && Id <= ID_PINPAD_9) {
            ...
            {
                // *** POÄŒETAK ISPRAVLJENOG BLOKA (VRAÄ†ENA LOGIKA IZ display - Copy.c) ***
                bool is_valid = false;
                ...
                if (numpad_return_screen == SCREEN_SECURITY)
                {
                    if (Security_ValidateUserCode(pin_buffer)) {
                        ...
                        // VRAÄ†ENA "OPTIMISTIC UI" LOGIKA ZA ALARM
                ...
                else // Podrazumijevani kontekst: Ulazak u glavna podeÅ¡avanja (SERVISNI PIN)
                {
                    if (strcmp(pin_buffer, system_pin) == 0) {
                        is_valid = true;
                        DSP_KillNumpadScreen();
                        // VRAÄ†ENA LINIJA KOJA NEDOSTAJE
                        DSP_InitSet1Scrn();
                        screen = SCREEN_SETTINGS_1; 
                        shouldDrawScreen = 1;
                    }
    ...
    if (pin_mask_timer != 0 && (HAL_GetTick() - pin_mask_timer) >= PIN_MASK_DELAY) {
        ...
        if (pin_error_active) {
            pin_error_active = false;
            // OÄisti bafer tek nakon isteka tajmera za greÅ¡ku
            pin_buffer_idx = 0;
            memset(pin_buffer, 0, sizeof(pin_buffer));
        }
        DSP_DrawNumpadText();
...
/******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete kreirane za Numpad.
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Adaptirana `DSP_KillPinpadScreen` funkcija. Poziva se prilikom
 * napuÅ¡tanja `SCREEN_NUMPAD` ekrana kako bi se oslobodili resursi.
 * @param       None
 * @retval      None
 *****************************************************************************/
static void DSP_KillNumpadScreen(void)
...
/******************************************************************************
 * @brief       PomoÄ‡na funkcija koja iscrtava tekstualni unos za Numpad.
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Adaptirana `DSP_DrawPinpadText` funkcija. Centralizuje logiku
 * za prikaz naslova, unesene vrijednosti ili poruke o greÅ¡ci.
 * @param       None
 * @retval      None
 *****************************************************************************/
static void DSP_DrawNumpadText(void)
{
    ...
    GUI_SetTextAlign(GUI_TA_HCENTER | GUI_TA_VCENTER);
    // ISPRAVKA: Y pozicija je sada ista kao za poruku o greÅ¡ci (y_text_center)
 ...
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Ovo je interna, ne-blokirajuÄ‡a funkcija. Kopira proslijeÄ‘eni
 * kontekst u internu statiÄku varijablu, resetuje rezultat,
 * i postavlja fleg za promjenu ekrana.
 * @param       context PokazivaÄ na NumpadContext_t strukturu sa parametrima.
 * @retval      None
 *****************************************************************************/
static void Display_ShowNumpad(const NumpadContext_t* context)
{
    // 1. SaÄuvaj ekran sa kojeg je funkcija pozvana, ali samo ako to nije sam Numpad
    if (screen != SCREEN_NUMPAD) {
        numpad_return_screen = screen;
    }
    // 2. Kopiraj proslijeÄ‘eni kontekst u internu, statiÄku varijablu
    if (context != NULL) {
...
/******************************************************************************
 * @brief       Kreira i inicijalizuje sve GUI widgete za alfanumeriÄku tastaturu.
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Ova funkcija je srce UI dijela `SCREEN_KEYBOARD_ALPHA`. Ona Äita
 * trenutno odabrani jezik iz `g_display_settings` i, na osnovu
 * njega i `shift` stanja, bira odgovarajuÄ‡i raspored iz
 * `key_layouts` matrice. Zatim dinamiÄki kreira sve tastere,
...
static void DSP_InitKeyboardScreen(void)
{
    ...
    // === 2. Odabir jeziÄkog rasporeda ===
    // Ovaj pokazivaÄ Ä‡e pokazivati na odgovarajuÄ‡i 2D niz karaktera (npr. BHS, mala slova)
    ...
    // === 3. DinamiÄko kreiranje tastera sa karakterima ===
    for (int row = 0; row < KEY_ROWS; row++) {
        for (int col = 0; col < KEYS_PER_ROW; col++) {
            // PreskoÄi ako za neku poziciju nije definisan taster (za kraÄ‡e redove)
...
/**
 ******************************************************************************
 * @brief       Servisira univerzalnu alfanumeriÄku tastaturu.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Funkcija obraÄ‘uje pritiske na sve tastere, upravlja unosom,
 * brisanjem i shift stanjem.
 * KljuÄna logika se nalazi u obradi "OK" dugmeta, gdje se na osnovu
 * konteksta (`keyboard_return_screen` i `selected_partition_for_rename`)
 * odluÄuje koja `setter` funkcija Ä‡e biti pozvana za snimanje podatka.
 ******************************************************************************
 */
static void Service_KeyboardScreen(void)
{
    ...
    // DogaÄ‘aj se aktivira kada se taster OTPUSTI
        ...
        if (Id >= GUI_ID_USER && Id < (GUI_ID_USER + (KEY_ROWS * KEYS_PER_ROW))) {
            ...
            case GUI_ID_OKAY:
                ...
                if (keyboard_return_screen == SCREEN_SETTINGS_ALARM)
                {
                    ...
                    {
                        // Mijenjamo naziv specifiÄne particije
                ...
                DSP_KillKeyboardScreen();
                screen = keyboard_return_screen;
                shouldDrawScreen = 1; // ZatraÅ¾i ponovno iscrtavanje prethodnog ekrana
                return;
            }
        }
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve GUI widgete kreirane za alfanumeriÄku tastaturu.
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Poziva se prilikom napuÅ¡tanja `SCREEN_KEYBOARD_ALPHA` ekrana.
 * @param       None
 * @retval      None
 ******************************************************************************
 */
...
/**
 ******************************************************************************
 * @brief       Priprema i prebacuje sistem na ekran alfanumeriÄke tastature.
 * @author      Gemini (po specifikaciji korisnika)
 * @note        Pamti ekran sa kojeg je pozvana radi povratka.
 * @param       context PokazivaÄ na KeyboardContext_t strukturu.
 * @retval      None
 ******************************************************************************
 */
static void Display_ShowKeyboard(const KeyboardContext_t* context)
{
    ...
    memset(&g_keyboard_result, 0, sizeof(KeyboardResult_t));
    keyboard_shift_active = false; // Uvijek poÄni sa malim slovima
...
/**
 * @brief Provjerava status aÅ¾uriranja firmvera i prikazuje odgovarajuÄ‡u poruku.
 * @note  Ova funkcija blokira ostatak GUI logike dok je aÅ¾uriranje u toku.
 * @return uint8_t 1 ako je aÅ¾uriranje aktivno, inaÄe 0.
 */
static uint8_t Service_HandleFirmwareUpdate(void)
{
    static uint8_t fwmsg = 2; // StatiÄki fleg za praÄ‡enje stanja iscrtavanja poruke

    // IZMJENA: Sada provjeravamo da li je aÅ¾uriranje aktivno BILO GDJE NA BUSU.
    if (IsBusFwUpdateActive()) {
        // Ako je aÅ¾uriranje aktivno, a poruka joÅ¡ nije iscrtana
        if (!fwmsg) {
        ...
        return 1; // Vrati 1 da signalizira da je aÅ¾uriranje u toku
    }
    // Ako je aÅ¾uriranje upravo zavrÅ¡eno (fleg je bio 1, a sada je IsBusFwUpdateActive() false)
    else if (fwmsg == 1) {
        fwmsg = 0; // Resetuj fleg
        scrnsvr_tmr = 0; // Resetuj tajmer za screensaver
        // Forsiraj ponovno iscrtavanje trenutnog ekrana da se ukloni poruka o aÅ¾uriranju
        shouldDrawScreen = 1;
    }
    // Jednokratno iscrtavanje inicijalne grafike na samom poÄetku
    else if (fwmsg == 2) {
        fwmsg = 0;
        GUI_MULTIBUF_BeginEx(1);
        GUI_Clear();
    ...
    return 0; // AÅ¾uriranje nije aktivno
}
/**
 ******************************************************************************
 ...
 * Optimizovana je tako da se ponovno iscrtavanje deÅ¡ava samo kada je
 * to neophodno (kada se promijeni stanje svjetala ili kada se forsira).
 * **AÅ¾urirana verzija dodaje donji meni za scene.**
 ******************************************************************************
 */
static void Service_MainScreen(void)
{
    // StatiÄke varijable za Äuvanje prethodnog stanja radi optimizacije iscrtavanja
    ...
    // Kombinujemo mod i stanje u jednu varijablu radi lakÅ¡e provjere promjene
    ...
    // Iscrtavanje ekrana se deÅ¡ava ako je zatraÅ¾eno, ILI ako se promijenilo stanje bilo kojeg sistema
    if (shouldDrawScreen || (current_light_state != old_light_state) || (current_timer_active_state != old_timer_active_state) || (current_thermostat_state != old_thermostat_state)) {
        ...
        old_thermostat_state = current_thermostat_state; // AÅ¾uriraj staro stanje termostata
        ...
        int16_t x_icon_pos = 5; // PoÄetna pozicija za prvu ikonicu (najviÅ¡e desno)
        ...
        if (current_timer_active_state)
        {
            ...
            x_icon_pos += 30; // Pomjeri "kursor" ulijevo za sljedeÄ‡u ikonicu
        }
...
/**
 ******************************************************************************
 * @brief       Servisira ekran za odabir glavnih kontrola (Svjetla, Termostat, Roletne, DinamiÄka ikona).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Verzija 2.3.2: Potpuno refaktorisano da koristi "Pametnu MreÅ¾u" (Smart Grid).
 * Ekran dinamiÄki iscrtava samo konfigurisane module i prilagoÄ‘ava raspored
 * na osnovu njihovog broja (1, 2, 3 ili 4), koristeÄ‡i nove fontove sa
 * punom podrÅ¡kom za specijalne karaktere.
 * @param       None
 * @retval      None
 ******************************************************************************
 */
...
{
    /**
     * @brief Struktura za Äuvanje podataka o jednom dinamiÄkom meniju.
     */
    typedef struct {
        const GUI_BITMAP* icon;
        TextID text_id;
        eScreen target_screen;
        bool is_active; // Dodatni fleg za dinamiÄku ikonu
    } DynamicMenuItem;
    ...
    // Lista SVIH moguÄ‡ih modula za ovaj ekran
    DynamicMenuItem all_modules[] = {
        { &bmSijalicaOff, TXT_LIGHTS, SCREEN_LIGHTS, false },
        { &bmTermometar, TXT_THERMOSTAT, SCREEN_THERMOSTAT, false },
        { &bmblindMedium, TXT_BLINDS, SCREEN_CURTAINS, false },
        { NULL, TXT_DUMMY, SCREEN_SELECT_1, false } // Mjesto za dinamiÄku ikonu
    };
    ...
    if (shouldDrawScreen) {
        ...
        if (active_modules_count < 4) {
            // Crtaj dugaÄki desni separator za 1, 2, i 3 ikonice
        ...
        // --- KORAK 2: "Pametna MreÅ¾a" - Iscrtavanje na osnovu broja aktivnih modula ---
        switch (active_modules_count) {
        case 1: {
        ...
        case 4:
        default: {
            // KlasiÄna 2x2 mreÅ¾a
            // Samo krstasti separator, bez dugaÄke desne linije
    ...
    // AÅ¾uriranje dinamiÄke ikone ako se njeno stanje promijenilo
    else if (dynamicIconUpdateFlag) {
        dynamicIconUpdateFlag = 0;
        shouldDrawScreen = 1; // NajlakÅ¡i naÄin je ponovno iscrtati cijeli ekran
    }
}

/**
 ******************************************************************************
 * @brief       Servisira drugi ekran za odabir (Kapija, Tajmer, Alarm, DinamiÄka ikona).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        KONAÄŒNA USKLAÄENA VERZIJA. Logika za iscrtavanje je sada
 * identiÄna funkciji `Service_SelectScreen1`, ukljuÄujuÄ‡i sve ofsete
 * i atribute za pozicioniranje, kako bi se osiguralo savrÅ¡eno
 * vizuelno poravnanje.
 ******************************************************************************
 */
static void Service_SelectScreen2(void)
{
    ...
    if (shouldDrawScreen) {
        ...
        // --- "Pametna MreÅ¾a" - Iscrtavanje 1:1 kao u Service_SelectScreen1 ---
        switch (active_modules_count) {
            case 0:
                GUI_SetFont(&GUI_FontVerdana20_LAT);
                GUI_SetColor(GUI_WHITE);
...
/**
 ******************************************************************************
 * @brief       Servisira posljednji ekran za odabir (ÄŒiÅ¡Ä‡enje, WiFi, App, Postavke).
 * @author      Gemini & [VaÅ¡e Ime]
...
static void Service_SelectScreenLast(void)
{
    if (shouldDrawScreen) {
        ...
        // Crtanje krstastog separatora za 2x2 mreÅ¾u
        ...
        // Iscrtavanje "NEXT" dugmeta koje sada vraÄ‡a na poÄetni ekran
        const GUI_BITMAP* iconNext = &bmnext;
        int x_pos = select_screen2_drawing_layout.next_button_x_pos;
        int y_pos = select_screen2_drawing_layout.next_button_y_center - (iconNext->YSize / 2);
        GUI_DrawBitmap(iconNext, x_pos, y_pos);
...
/**
 ******************************************************************************
 * @brief       Servisira ekran sa termostatom ISKLJUÄŒIVO unutar "Scene Wizard" moda.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija je kopija originalne `Service_ThermostatScreen`,
 * modifikovana za rad unutar Äarobnjaka. Uklonjeni su hamburger meni,
...
//                MVUpdateReset();
//                GUI_MULTIBUF_BeginEx(1);
//                // ObriÅ¡i samo staru poziciju za MV Temp
...
//            }
    ...
    if (BUTTON_IsPressed(hButtonWizNext))
    {
        ...
        is_in_scene_wizard_mode = false; // ZavrÅ¡i wizard mod
...
/**
 * @brief Servisira ekran termostata, ukljuÄujuÄ‡i iscrtavanje i obradu unosa.
 * @note Ova funkcija se poziva u petlji dok je `screen == SCREEN_THERMOSTAT`.
 * U njoj se upravlja prikazom zadane temperature, trenutne temperature, te se obraÄ‘uju
 * dodiri za podeÅ¡avanje temperature i paljenje/gaÅ¡enje termostata.
 */
static void Service_ThermostatScreen(void)
{
    if (is_in_scene_wizard_mode)
    ...
    else
    {
        ...
        // AÅ¾uriranje prikaza termostata se radi unutar jedne multibuffer transakcije.
        GUI_MULTIBUF_BeginEx(1);
        ...
        if (thermostatMenuState == 0) {
            ...
            // ÄŒiÅ¡Ä‡enje specifiÄnih dijelova ekrana za dinamiÄke vrijednosti.
            GUI_ClearRect(350, 80, 480, 180);
            GUI_ClearRect(310, 180, 420, 205);
            GUI_MULTIBUF_EndEx(0);

            // Prebacivanje na drugi sloj za dinamiÄke elemente.
            GUI_SelectLayer(1);
            GUI_SetBkColor(GUI_TRANSPARENT);
            GUI_Clear();
            ...
            DISPDateTime();
            // Postavi flag da treba aÅ¾urirati trenutnu temperaturu.
            MVUpdateSet();
            menu_lc = 0;
        } else if (thermostatMenuState == 1) {
            // Ako je ekran veÄ‡ iscrtan, obraÄ‘ujemo samo promjene stanja.

            // Logika za poveÄ‡anje/smanjenje zadane temperature.
            if (btninc && !_btninc) {
                _btninc = 1;
                Thermostat_SP_Temp_Increment(pThst);
                THSTAT_Save(pThst);
            ...
            // AÅ¾uriranje prikaza trenutne temperature i statusa regulatora.
            if (IsMVUpdateActiv()) {
                MVUpdateReset();
        ...
        // Detekcija dugog pritiska za paljenje/gaÅ¡enje termostata.
        if(thermostatOnOffTouch_timer) {
            DISPResetScrnsvr();
...
/**
 * @brief VraÄ‡a sistem na poÄetni ekran i resetuje sve relevantne flagove.
 * @note Ova funkcija se poziva kada se treba vratiti na "Äisto" poÄetno stanje.
 * BriÅ¡e oba grafiÄka sloja i resetuje sve interne varijable stanja.
 */
static void Service_ReturnToFirst(void)
{
    // === "FAIL-SAFE" MEHANIZAM ===
    ...
    // NOVO: Svjetlina se ne mijenja automatski pri povratku na poÄetni ekran.
    // DISPSetBrightnes(DISP_BRGHT_MIN); // Ova linija je uklonjena.

    // Postavi aktivni ekran na glavni meni.
    screen = SCREEN_MAIN;

    // Resetovanje svih flagova i brojaÄa.
    thermostatMenuState = 0;
    menu_lc = 0;
    menu_clean = 0;
    lcsta = 0;
 ...
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        FINALNA VERZIJA 2.1: Logika je aÅ¾urirana. Funkcija sada
 * iscrtava samo konfigurisane scene u 3x2 mreÅ¾i. Ikona za dodavanje
 * nove scene ("Äarobnjak") je izdvojena i prikazuje se u donjem
...
static void Service_SceneScreen(void)
{
    if (shouldDrawScreen) {
        ...
        // --- Iscrtavanje postojeÄ‡ih, konfigurisanih scena u mreÅ¾i ---
        for (int i = 0; i < configured_scenes_count; i++)
        {
            const SceneAppearance_t* appearance = NULL;

            // PronaÄ‘i i-tu po redu konfigurisanu scenu
            ...
            // Pozicioniranje u mreÅ¾i ostaje isto
        ...
        // --- Iscrtavanje ikonice Äarobnjaka odvojeno ---
...
/**
 ******************************************************************************
 * @brief       Servisira ekran sa svjetlima ISKLJUÄŒIVO unutar "Scene Wizard" moda.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija je kopija originalne `Service_LightsScreen` funkcije,
 * modifikovana za rad unutar Äarobnjaka. Uklonjen je "hamburger" meni,
 * a dodato je "[Next]" dugme sa ikonicom (80x80) i "pametnom"
 * navigacijom na sljedeÄ‡i korak u Äarobnjaku.
 ******************************************************************************
 */
static void Service_SceneEditLightsScreen(void)
{
...
/**
 ******************************************************************************
 * @brief       Servisira ekran sa svjetlima, sa dinamiÄkim odabirom fonta.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija dinamiÄki iscrtava ikone svjetala. SadrÅ¾i "pametnu"
 * logiku koja prvo analizira duÅ¾inu svih tekstualnih labela. Na osnovu
 * dostupnog prostora, funkcija bira najveÄ‡i moguÄ‡i font i primjenjuje
 * ga na sve labele radi vizuelne konzistentnosti.
 * AÅ¾urirano: Svi fiksni ("hardkodirani") parametri za pozicioniranje
 * (y-start, visina reda, razmak) su zamijenjeni vrijednostima iz
 * `lights_and_gates_grid_layout` strukture, omoguÄ‡avajuÄ‡i laku
...
static void Service_LightsScreen(void)
{
    ...
    else if(shouldDrawScreen)
    {
        ...
        // =======================================================================
        const GUI_FONT* fontToUse = &GUI_FontVerdana20_LAT; // PoÄetna pretpostavka: koristimo najveÄ‡i font
        const int text_padding = 10; // Sigurnosni razmak u pikselima izmeÄ‘u tekstova susjednih ikonica
        bool downgrade_font = false;

        // Petlja za PRE-KALKULACIJU (prolazimo kroz sve, ali ne iscrtavamo niÅ¡ta)
        for(uint8_t i = 0; i < LIGHTS_getCount(); ++i)
        {
            // IzraÄunaj maksimalnu dozvoljenu Å¡irinu teksta na osnovu rasporeda
            uint8_t lights_total = LIGHTS_getCount();
            uint8_t lights_in_this_row = 0;
            if (lights_total <= 3) lights_in_this_row = lights_total;
            else if (lights_total == 4) lights_in_this_row = 2;
        ...
        if (downgrade_font) {
            fontToUse = &GUI_FontVerdana16_LAT; // ...cijeli ekran Ä‡e koristiti manji font.
        }

        // =======================================================================
        // === FAZA 2: ISCRTAVANJE IKONICA SA KONAÄŒNO ODABRANIM FONTOM ===
        // =======================================================================
            ...
            for(uint8_t idx_in_row = 0; idx_in_row < lightsInRow; ++idx_in_row) {
                ...
                if (handle) {
                    ...
                    if (selection_index < (sizeof(icon_mapping_table) / sizeof(IconMapping_t)))
                    {
                        ...
                        // Postavljamo KONAÄŒNO ODABRANI FONT za iscrtavanje
                        GUI_SetFont(fontToUse);
                        const int font_height = GUI_GetFontDistY();
                        const int icon_height = icon_to_draw->YSize;
                        const int icon_width = icon_to_draw->XSize;
...
/**
 ******************************************************************************
 * @brief       Servisira ekran sa roletnama ISKLJUÄŒIVO unutar "Scene Wizard" moda.
 * @author      Gemini & [VaÅ¡e Ime]
 ...
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija dinamiÄki iscrtava korisniÄki interfejs za kontrolu zavjesa.
...
static void Service_CurtainsScreen(void)
{
    ...
    else if(shouldDrawScreen)
    {
        ...
        // KLJUÄŒNA PROMJENA: Ovdje se bira pravi font
        // << ISPRAVKA: Logika je aÅ¾urirana da koristi novi, sigurni API >>
        if(!Curtain_areAllSelected()) {
            GUI_SetFont(GUI_FONT_D48);
            uint8_t physical_index = 0;
            uint8_t count = 0;
            // Petlja pronalazi fiziÄki indeks (0-15) na osnovu logiÄkog (0-broj konfigurisanih)
            for (uint8_t i = 0; i < CURTAINS_SIZE; i++) {
                Curtain_Handle* handle = Curtain_GetInstanceByIndex(i);
                if (Curtain_hasRelays(handle)) {
                    if (count == curtain_selected) {
        ...
        const uint16_t drawingAreaWidth = 380;       // Å irina radnog prostora za kontrole
        const uint8_t triangleBaseWidth = 180;      // Å irina baze trougla (hipotenusa)
        ...
        // IzraÄunavanje vertikalnih pomaka za trouglove
        const uint8_t verticalOffsetUp = yLinePosition - triangleHeight - verticalGap;
        const uint8_t verticalOffsetDown = yLinePosition + verticalGap;

        // Iscrtavanje bijele linije Äija je duÅ¾ina jednaka Å¡irini baze trougla.
        GUI_SetColor(GUI_WHITE);
        GUI_DrawLine(horizontalOffset, yLinePosition, horizontalOffset + triangleBaseWidth, yLinePosition);

        // Definiranje taÄaka za gornji i donji trougao
        ...
        // 3. ODREÄIVANJE STANJA I CRTANJE
        //--------------------------------------------------------------------------------
        ...
        //--------------------------------------------------------------------------------
        // Ove strelice se crtaju samo ako postoji viÅ¡e od jedne zavjese.
        if (Curtains_getCount() > 1) {
            // Smanjena veliÄina strelica i izraÄunavanje novih pozicija za centriranje
            const uint8_t arrowSize = 50;
            const uint16_t verticalArrowCenter = 192 + (80/2); // Y-koordinata centra zone za strelice

            // IzraÄunavanje pozicija za strelice, centrirano u lijevom i desnom slobodnom prostoru
            const uint16_t leftSpace = horizontalOffset;
            const uint16_t rightSpace = drawingAreaWidth - (horizontalOffset + triangleBaseWidth);
            const uint16_t xLeftArrow = leftSpace/2 - arrowSize/2;
            const uint16_t xRightArrow = horizontalOffset + triangleBaseWidth + (rightSpace/2) - arrowSize/2;

            // Definiranje taÄaka za lijevu strelicu
            ...
            // Definiranje taÄaka za desnu strelicu
 ...
 * @note Ova funkcija generiÅ¡e i iscrtava QR kod na osnovu podataka
...
/**
 ******************************************************************************
 * @brief       Servisira ekran za ÄiÅ¡Ä‡enje ekrana (privremeno onemoguÄ‡ava dodir).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Prikazuje tajmer sa odbrojavanjem od 60 sekundi. Tokom odbrojavanja,
 * ekran je zakljuÄan za unos. Posljednjih 5 sekundi je vizuelno i
 * zvuÄno naglaÅ¡eno. Nakon Å¡to tajmer istekne, automatski se vraÄ‡a
 * na poÄetni ekran. Koristi matematiÄki proraÄun za precizno
...
static void Service_CleanScreen(void)
{
    // --- 1. DEFINICIJA I IZRAÄŒUNI SVIH KONSTANTI NA POÄŒETKU FUNKCIJE ---
    ...
    // Razmaci
    const uint8_t textGap = 10;                 // Razmak izmeÄ‘u naslova i brojaÄa
    ...
    // IzraÄun vertikalnih pozicija
    // BrojaÄ je FIKSIRAN na sredini ekrana
    const uint16_t yCounterPosition = yDisplayCenter;

    // Naslov se pozicionira iznad brojaÄa, uzimajuÄ‡i u obzir ofset
    const uint16_t yTitlePosition = yCounterPosition - (yFontCounterHeight / 2) - textGap - (yFontTitleHeight / 2) + verticalTextOffset;

    // DinamiÄki izraÄun granica za GUI_ClearRect (sada pokriva cijeli dinamiÄni prostor)
    ...
    if (menu_clean == 0) {
        // Prvi ulazak na ekran za ÄiÅ¡Ä‡enje, inicijalizuj stanje i tajmer.
        menu_clean = 1;
        GUI_Clear();
        ...
        if (HAL_GetTick() - clean_tmr >= 1000) {
            ...
            // DinamiÄko ÄiÅ¡Ä‡enje ekrana u zoni teksta
            GUI_ClearRect(0, yClearRectStart, 480, yClearRectEnd);

            // Promjena boje teksta i zvuÄni signal za posljednjih 5 sekundi.
            ...
            // --- Iscrtavanje naslova i broja na matematiÄki odreÄ‘enim pozicijama ---
            // =======================================================================
            // === IZMJENA FONT-a ===
            // =======================================================================
            GUI_SetFont(&GUI_FontVerdana32_LAT);
...
/**
 * @brief  Servisira prvi ekran podeÅ¡avanja (kontrola termostata i ventilatora).
 * @note   Refaktorisana verzija koja koristi iskljuÄivo javni API termostat modula.
 * @param  None
 * @retval None
 */
static void Service_SettingsScreen_1(void)
{
    // Dobijamo handle za termostat na poÄetku funkcije.
    THERMOSTAT_TypeDef* pThst = Thermostat_GetInstance();
    ...
    if (Thermostat_Get_SP_Max(pThst) != SPINBOX_GetValue(hThstMaxSetPoint)) {
        ...
        // OÄitavamo validiranu vrijednost nazad pomoÄ‡u gettera.
        SPINBOX_SetValue(hThstMaxSetPoint, Thermostat_Get_SP_Max(pThst));
        ++thsta;
    }
    ...
    if (BUTTON_IsPressed(hBUTTON_Ok)) {
        ...
        thsta = 0;
        DSP_KillSet1Scrn(); // UniÅ¡tavanje widgeta trenutnog ekrana
        screen = SCREEN_RETURN_TO_FIRST;
    } else if (BUTTON_IsPressed(hBUTTON_Next)) {
        ...
        thsta = 0;
        DSP_KillSet1Scrn();
        DSP_InitSet2Scrn(); // Inicijalizacija sljedeÄ‡eg ekrana
        screen = SCREEN_SETTINGS_2;
    }
}
/**
 * @brief Servisira drugi ekran podeÅ¡avanja (vrijeme, datum, screensaver).
 * @note  Verzija 2.1: Ispravljena logika za aÅ¾uriranje prikaza boje.
 * Ova funkcija sinkronizuje vrijeme i datum s RTC modulom,
 * obraÄ‘uje promjene postavki screensavera i svjetline ekrana,
...
static void Service_SettingsScreen_2(void)
{
    ...
    if (rtcdt.WeekDay != (DROPDOWN_GetSel(hDRPDN_WeekDay) + 1)) { // PoreÄ‘enje sa Dec vrijednoÅ¡Ä‡u
    ...
    /**
     * @brief << ISPRAVKA: AÅ¾uriranje prikaza boje screensaver sata. >>
    ...
    // << ISPRAVKA 3: Ispravljena logika aÅ¾uriranja screensaver postavki >>
    // Prvo aÅ¾uriramo konfiguracionu varijablu na osnovu stanja checkbox-a.
    if (g_display_settings.scrnsvr_on_off != (bool)CHECKBOX_GetState(hCHKBX_ScrnsvrClock)) {
        ...
        settingsChanged = 1; // Signaliziramo da je doÅ¡lo do promjene za snimanje
    }

    // Zatim, na osnovu te konfiguracione varijable, aÅ¾uriramo runtime fleg.
    ...
    /** @brief AÅ¾uriranje ostalih konfiguracionih varijabli. */
    ...
    if (BUTTON_IsPressed(hBUTTON_Ok)) {
        ...
        // Snimi sve promjene prije prelaska na sljedeÄ‡i ekran
        Display_Save();
        EE_WriteBuffer(&tfifa, EE_TFIFA, 1);
        if (thsta) {
            THSTAT_Save(pThst);
...
/**
 * @brief Servisira treÄ‡i ekran podeÅ¡avanja (Ventilator).
 * @note Ova funkcija obraÄ‘uje promjene u postavkama ventilatora (relej, odgode),
...
/**
 * @brief Servisira Äetvrti ekran podeÅ¡avanja (Zavjese).
 * @note Ovaj ekran omoguÄ‡ava podeÅ¡avanje Modbus adresa releja za zavjese.
 * Koristi petlju za dinamiÄko upravljanje widgetima i provjeru promjena.
 */
static void Service_SettingsScreen_4(void)
{
    // AÅ¾uriranje postavki za zavjese unutar petlje.
    for(uint8_t idx = curtainSettingMenu * 4; idx < (((CURTAINS_SIZE - (curtainSettingMenu * 4)) >= 4) ? ((curtainSettingMenu * 4) + 4) : CURTAINS_SIZE); idx++) {

        // << ISPRAVKA: Dobijamo handle za roletnu po fiziÄkom indeksu. >>
        Curtain_Handle* handle = Curtain_GetInstanceByIndex(idx);
        if (!handle) continue; // Ako handle nije validan, preskoÄi

        // << ISPRAVKA: Koristimo gettere za poreÄ‘enje i settere za upis. >>
    ...
    if (BUTTON_IsPressed(hBUTTON_Ok)) {
        ...
        // Logika za prelazak na sljedeÄ‡u stranicu podeÅ¡avanja zavjesa ili na sljedeÄ‡i ekran.
...
/**
 ******************************************************************************
 * @brief       Servisira peti ekran podeÅ¡avanja (detaljne postavke za svjetla).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija u petlji provjerava da li je korisnik promijenio vrijednost
 * nekog widgeta. Ako jeste, poziva odgovarajuÄ‡u `setter` funkciju iz `lights` API-ja
 * da aÅ¾urira stanje u RAM-u. Stvarno snimanje u EEPROM se deÅ¡ava tek pritiskom
 * na 'SAVE' ili 'NEXT' dugme. TakoÄ‘er iscrtava preview ikonice sa
 * odgovarajuÄ‡im opisima, osiguravajuÄ‡i da je podruÄje iscrtavanja
 * prethodno obrisano radi izbjegavanja vizuelnih greÅ¡aka.
 ******************************************************************************
 */
static void Service_SettingsScreen_5(void)
{
    ...
    // Dohvatamo indeks svjetla koje trenutno konfiguriÅ¡emo.
    uint8_t light_index = lightsModbusSettingsMenu;
    ...
    if (!handle) {
        GUI_MULTIBUF_EndEx(1);
        return; // Ako handle nije validan, prekidamo izvrÅ¡avanje
    }

    // Logika za omoguÄ‡avanje/onemoguÄ‡avanje spinbox-a za minute ostaje ista.
    ...
    // KORAK 2: Provjeravamo promjene na SVAKOM widgetu i pozivamo odgovarajuÄ‡i SETTER.
    ...
    // =======================================================================
    // === POÄŒETAK LOGIKE ZA ISCRTAVANJE IKONICE I TEKSTA ===
    // =======================================================================
    ...
    if (selection_index < (sizeof(icon_mapping_table) / sizeof(IconMapping_t)))
    {
        ...
        // VraÄ‡anje poziva za poravnanje
        GUI_SetTextAlign(GUI_TA_HCENTER);
        GUI_SetColor(GUI_ORANGE);
...
/**
 ******************************************************************************
 * @brief       Servisira Å¡esti ekran podeÅ¡avanja (OpÅ¡te postavke).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        KONAÄŒNA VERZIJA. Upravlja sa dva meÄ‘usobno iskljuÄiva DROPDOWN
 * menija. Koristi ispravan mehanizam mapiranja indeksa (`control_mode_map`)
 * za dobijanje stvarne vrijednosti iz odabrane stavke, Äime se
...
static void Service_SettingsScreen_6(void)
{
    ...
    if (sel1 >= 0) {
        ...
        if (current_mode1 != g_display_settings.selected_control_mode) {
            ...
            settingsChanged = 1;
            // Ponovo iscrtaj cijeli ekran da bi se aÅ¾urirala lista u drugom dropdown-u
            DSP_KillSet6Scrn();
            DSP_InitSet6Scrn();
            return; // VaÅ¾no: prekidamo izvrÅ¡avanje
        }
    }
    ...
    if (sel2 >= 0) {
        ...
        if (current_mode2 != g_display_settings.selected_control_mode_2) {
            ...
            settingsChanged = 1;
            // Ponovo iscrtaj cijeli ekran da bi se aÅ¾urirala lista u prvom dropdown-u
            DSP_KillSet6Scrn();
            DSP_InitSet6Scrn();
            return; // VaÅ¾no: prekidamo izvrÅ¡avanje
        }
...
/**
 ******************************************************************************
 * @brief       Servisira sedmi ekran podeÅ¡avanja (Scene Backend).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija se poziva u petlji iz `DISP_Service`. Odgovorna je za:
 * 1. Detekciju promjena na widgetima (checkbox za omoguÄ‡avanje scena
 * i spinbox-ovi za adrese okidaÄa).
 * 2. AÅ¾uriranje `g_display_settings` strukture u RAM-u kada doÄ‘e do promjene.
...
static void Service_SettingsScreen_7(void)
{
    ...
    // 1. Provjera Checkbox-a za omoguÄ‡avanje sistema scena
    ...
    // 2. Provjera 8 Spinbox-ova za adrese okidaÄa
    for (uint8_t i = 0; i < SCENE_MAX_TRIGGERS; i++)
    {
    ...
    if (BUTTON_IsPressed(hBUTTON_Ok)) // Dugme "SAVE"
    {
        ...
        DSP_KillSet7Scrn();
        screen = SCREEN_RETURN_TO_FIRST; // Vrati se na poÄetni ekran
    }
...
/**
 ******************************************************************************
 * @brief       Servisira osmi ekran podeÅ¡avanja (Kapije).
 * @author      Gemini
 * @note        ISPRAVLJENA VERZIJA: Logika je restrukturirana da se
 * omoguÄ‡i/onemoguÄ‡i prikaz kontrola odmah nakon promjene profila,
 * Äime se rjeÅ¡ava bug gdje opcije nisu bile dostupne.
 ******************************************************************************
 */
static void Service_SettingsScreen_8(void)
{
    ...
    if ((SPINBOX_GetValue(hGateSelect) - 1) != settings_gate_selected_index) {
        ...
        DSP_KillSet8Scrn();
        DSP_InitSet8Scrn();
        // VaÅ¾no: odmah izaÄ‘i iz funkcije nakon reinicijalizacije da se sprijeÄi rad sa starim handle-ovima
        return;
    }
    ...
    // Ako korisnik promijeni profil, zabiljeÅ¾i promjenu i postavi fleg
    if (DROPDOWN_GetSel(hGateType) != Gate_GetControlType(handle)) {
        Gate_SetControlType(handle, DROPDOWN_GetSel(hGateType));
        settingsChanged = 1;
        needs_full_update = true; // ZatraÅ¾i puno aÅ¾uriranje nakon ovoga
    }
    ...
    // DinamiÄko omoguÄ‡avanje/onemoguÄ‡avanje widgeta na osnovu maske
    ...
    else if (BUTTON_IsPressed(hBUTTON_Next))
    {
        ...
        DSP_KillSet8Scrn();
        DSP_InitSet9Scrn(); // Vracamo se na prvi ekran podesavanja, praveÄ‡i puni krug
        screen = SCREEN_SETTINGS_9;
    }

    // Ako je bio zahtjev za aÅ¾uriranje, ponovo iscrtaj sve od nule
...
/**
 ******************************************************************************
 * @brief       Servisira deveti ekran za podeÅ¡avanja (Alarm).
 * @note        AÅ¾urirana verzija koja obraÄ‘uje kontrole sa novog layout-a,
 * ukljuÄujuÄ‡i `Enable Security Module` checkbox i preraÄunavanje
 * vrijednosti za duÅ¾inu pulsa.
 ******************************************************************************
 */
static void Service_SettingsScreen_9(void)
{
    ...
    // Provjera promjena na zajedniÄkim postavkama
    // Za duÅ¾inu pulsa, vrijednost iz spinbox-a (0-50) se mnoÅ¾i sa 100 da bi se dobile milisekunde (0-5000)
    ...
    // Obrada premjeÅ¡tenog checkbox-a za omoguÄ‡avanje security modula
    if(g_display_settings.security_module_enabled != (bool)CHECKBOX_GetState(hCHKBX_EnableSecurity)) {
        ...
        settingsChanged = 1; // Ova promjena se snima pomoÄ‡u Display_Save()
    }

    // Obrada navigacije i snimanja
    if (BUTTON_IsPressed(hBUTTON_Ok)) {
        if (settingsChanged) { 
            Security_Save(); // Snima postavke specifiÄne za alarm
            Display_Save();  // Snima opÅ¡te postavke (gdje se nalazi `security_module_enabled` fleg)
            settingsChanged = 0; 
        }
        DSP_KillSet9Scrn();
        screen = SCREEN_RETURN_TO_FIRST;
        ...
        DSP_KillSet9Scrn();
        DSP_InitSet1Scrn(); // VraÄ‡a se na prvi ekran podeÅ¡avanja, praveÄ‡i puni krug
        screen = SCREEN_SETTINGS_1;
    }
}
/**
 ******************************************************************************
 * @brief       Servisira ekran za napredno podeÅ¡avanje svjetla (dimmer i RGB).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija je centralna taÄka za interakciju na ekranu za detaljna
 * podeÅ¡avanja svjetla. Odgovorna je za:
 * 1.  Iscrtavanje korisniÄkog interfejsa, koje se dinamiÄki prilagoÄ‘ava
 * u zavisnosti od toga da li je odabrano jedno ili viÅ¡e svjetala,
 * i da li podrÅ¾avaju dimovanje ili RGB kontrolu.
 * 2.  Obradu rezultata vraÄ‡enog sa alfanumeriÄke tastature nakon Å¡to
...
static void Service_LightSettingsScreen(void)
{
    ...
    // === 2. ISCRTAVANJE EKRANA (ako je zatraÅ¾eno) ===
    if(shouldDrawScreen) {
        shouldDrawScreen = 0;
        ...
        // === POÄŒETAK NOVE LOGIKE ZA ISPIS NASLOVA/NAZIVA ===
        // Nema viÅ¡e dugmeta, samo ispis teksta
        GUI_SetFont(&GUI_FontVerdana20_LAT);
        GUI_SetColor(GUI_WHITE);
        GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_TOP);

        if (light_selectedIndex < LIGHTS_MODBUS_SIZE) {
            // Ako podeÅ¡avamo pojedinaÄno svjetlo -> prikaÅ¾i njegov naziv
            LIGHT_Handle* handle = LIGHTS_GetInstance(light_selectedIndex);
            if (handle) {
                const char* custom_label = LIGHT_GetCustomLabel(handle);

                if (custom_label[0] != '\0') {
                    // Ako postoji custom label, ispiÅ¡i ga
                    GUI_DispStringAt(custom_label, 10, 10);
                } else {
                    // Ako ne, ispiÅ¡i default naziv saÄinjen od dva dijela
                    uint16_t selection_index = LIGHT_GetIconID(handle);
                    if (selection_index < (sizeof(icon_mapping_table) / sizeof(IconMapping_t))) {
                        const IconMapping_t* mapping = &icon_mapping_table[selection_index];
            ...
            // Ako smo u globalnom modu -> prikaÅ¾i prevedeni naslov
    ...
    // === 3. OBRADA KORISNIÄŒKOG UNOSA (Ovaj dio Ä‡emo izmijeniti u sljedeÄ‡em koraku) ===
 ...
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija poziva Init funkciju za iscrtavanje i Äeka na
 * unos korisnika, koji se obraÄ‘uje u `HandlePress_SceneAppearanceScreen`.
 ******************************************************************************
 */
static void Service_SceneAppearanceScreen(void)
{
...
/**
 ******************************************************************************
 * @brief       Servisira glavni ekran "ÄŒarobnjaka" za scene (SCREEN_SCENE_EDIT).
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        FINALNA ISPRAVLJENA VERZIJA. Ova funkcija sada ispravno
 * upravlja navigacijom pozivajuÄ‡i odgovarajuÄ‡e Kill i Init funkcije
 * prilikom promjene ekrana, Äime se osigurava da Äarobnjak radi.
 ******************************************************************************
 */
static void Service_SceneEditScreen(void)
{
    ...
    // --- Obrada pritiska na dugme "SNIMI" / "MEMORIÅ I STANJE" ---
    if (BUTTON_IsPressed(hBUTTON_Ok))
    {
    ...
    // --- Obrada pritiska na dugme "OTKAÅ½I" ---
    else if (BUTTON_IsPressed(hBUTTON_Next))
    {
        DSP_KillSceneEditScreen();
        screen = SCREEN_SCENE;
    ...
    if (WM_IsWindow(hButtonChangeAppearance) && BUTTON_IsPressed(hButtonChangeAppearance))
    {
        ...
        DSP_KillSceneEditScreen();
//        DSP_InitSceneAppearanceScreen(); // Inicijalizuj sljedeÄ‡i ekran
        screen = SCREEN_SCENE_APPEARANCE;
        shouldDrawScreen = 1; // Init je veÄ‡ iscrtao, ne treba ponovo
        return;
    }

    // --- Obrada pritiska na dugme "OBRIÅ I" (samo kod postojeÄ‡e scene) ---
    ...
    // --- Obrada pritiska na dugme "DETALJNA PODEÅ AVANJA" ---
    if (WM_IsWindow(hButtonDetailedSetup) && BUTTON_IsPressed(hButtonDetailedSetup))
    {
        is_in_scene_wizard_mode = true;
        DSP_KillSceneEditScreen();

        // Na osnovu tipa scene, pokreni prvi korak "anketnog" Äarobnjaka
        ...
        shouldDrawScreen = 0; // Init je veÄ‡ iscrtao
        return;
    }
}
...
/**
 ******************************************************************************
 * @brief       Servisira ekran za odabir grupa ureÄ‘aja u Äarobnjaku.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        FINALNA VERZIJA: Ova funkcija sada sadrÅ¾i kompletnu logiku
 * za aÅ¾uriranje bitmaski scene i "pametnu" navigaciju. Pritiskom na
 * dugme "[Dalje]", sistem provjerava koje su grupe ureÄ‘aja odabrane
 * i vodi korisnika na sljedeÄ‡i relevantan ekran.
 ******************************************************************************
 */
static void Service_SceneWizDevicesScreen(void)
{
    ...
    // --- AÅ¾uriranje Stanja Checkbox-ova ---
    ...
    else if (BUTTON_IsPressed(hButtonWizNext))
    {
        ...
        // "Komfor" scene idu na prvi odabrani ureÄ‘aj
        case SCENE_TYPE_STANDARD:
        default:
            if (scene_handle->lights_mask) {
                ...
                // Ako niÅ¡ta nije odabrano, idi na finalni ekran
...
/**
 * @brief Servisira ekran za resetovanje glavnih prekidaÄa/menija.
...
static void Service_MainScreenSwitch(void)
{
    ...
    if(LIGHTS_IsNightTimerActive()) {
        ...
        // KORAK 2: Umjesto ruÄnog raÄunanja, pozivamo API funkciju koja vraÄ‡a preostalo vrijeme.
...
/**
 ******************************************************************************
 * @brief       Servisira finalni ekran Äarobnjaka.
 ******************************************************************************
 */
static void Service_SceneWizFinalizeScreen(void)
{
    ...
    {
        // Ne snimaj promjene u RAM-u, samo izaÄ‘i
        is_in_scene_wizard_mode = false;
        DSP_KillSceneWizFinalizeScreen();
        screen = SCREEN_SCENE;
        shouldDrawScreen = 1;
...
/**
 ******************************************************************************
 * @brief       IzraÄunava dan u sedmici na osnovu datuma.
 ...
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        AÅ¾urirana verzija. Blok koda `if (!IsRtcTimeValid())` je
 * identiÄan VaÅ¡oj radnoj verziji. Blok `else` je redizajniran:
 * - Tekstovi "Svaki dan", "Radnim danima" itd. su premjeÅ¡teni u prevode.
 * - Dugme za podeÅ¡avanja se kreira sa dimenzijama ikonice `bmicons_settings`.
 ******************************************************************************
 */
static void Service_TimerScreen(void)
{
    // Crtamo samo ako je eksplicitno zatraÅ¾eno
    if (shouldDrawScreen) {
        shouldDrawScreen = 0;
        ForceKillAllSettingsWidgets();
        GUI_MULTIBUF_BeginEx(1);
        ...
        // ====================================================================
        if (!IsRtcTimeValid()) {
            // Ako vrijeme NIJE validno, prikaÅ¾i veliko dugme za podeÅ¡avanje
            const GUI_BITMAP* datetime_icon = &bmicons_date_time;

            // Jedina izmjena: pozicije se sada Äitaju iz layout strukture
        ...
        // ====================================================================
            ...
            // Ako je vrijeme validno, prikaÅ¾i glavni interfejs alarma
            char time_str[6];
            ...
            // Prikaz dana ponavljanja KORIÅ TENJEM PREVODA
            GUI_SetFont(&GUI_FontVerdana20_LAT);
            GUI_SetTextAlign(GUI_TA_HCENTER);
            GUI_SetColor(GUI_ORANGE);
            uint8_t repeat_mask = Timer_GetRepeatMask();
            ...
            if (repeat_mask == TIMER_EVERY_DAY)       strcpy(days_str, lng(TXT_TIMER_EVERY_DAY));
                ...
                // SkraÄ‡enice dana su veÄ‡ u prevodima u _acContent tabeli
                for (int i = 0; i < 7; i++) {
                    if (repeat_mask & (1 << i)) {
...
/**
 ******************************************************************************
 * @brief       [VERZIJA 2.5 - 1:1 PREMA VAÅ EM KODU] Servisira ekran za podeÅ¡avanje alarma.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova verzija je bazirana 1:1 na VaÅ¡em funkcionalnom kodu, bez ikakvih
 * izmjena u postojeÄ‡oj logici ili nazivima varijabli.
...
static void Service_SettingsTimerScreen(void)
{
    ...
    // --- NOVE LOKALNE STATIÄŒKE VARIJABLE ZA REPEAT FUNKCIJU ---
    static uint32_t press_time[4] = {0}; // ÄŒuva vreme pritiska za svaki taster
    static bool button_is_held[4] = {false}; // Da li se taster drÅ¾i
    ...
    if (!timer_screen_initialized) {
        ...
        // NOVI DODATAK: UÄitavanje podataka za scenu. VaÅ¡a originalna `scene_state` se sada postavlja na osnovu ovoga.
        // Ako je `needs_full_redraw` postavljen, znaÄi da ulazimo na ekran prvi put,
        // pa treba uÄitati snimljenu vrijednost. Ako nije, znaÄi da se vraÄ‡amo
        // sa pickera i treba da saÄuvamo vrijednost koja je veÄ‡ u `timer_selected_scene_index`.
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (is_day_pressed && !old_button_state[i+4]) { // PoÄinje od indeksa 4
    ...
    if (BUTTON_IsPressed(hButtonTimerSceneSelect) && !old_button_state[13]) {
        ...
        DSP_InitSceneAppearanceScreen();
        screen = SCREEN_SCENE_APPEARANCE;
        return; // VaÅ¾no: odmah izaÄ‘i iz funkcije da se sprijeÄi dalje izvrÅ¡avanje
    ...
    if (shouldDrawScreen) {
        ...
        // Uvijek crtamo pozadinu i statiÄne elemente
        GUI_SetFont(GUI_FONT_D64);
        GUI_SetColor(GUI_ORANGE);
        GUI_SetTextMode(GUI_TM_TRANS);
        ...
        // Brisanje i iscrtavanje dvotaÄke
        ...
        //
        // ==========> POÄŒETAK IZMIJENJENOG KODA ZA ISPIS SCENE <==========
        //
        // DefiniÅ¡emo Y poziciju koja je zajedniÄka za oba teksta (vertikalno centrirana)
        const int16_t y_pos = timer_settings_screen_layout.scene_button_pos.y + (icon_off->YSize / 2);

        // --- 1. Iscrtavanje statiÄkog teksta "Pokreni scenu" (ostaje nepromijenjeno) ---
        GUI_SetFont(&GUI_FontVerdana16_LAT);
        GUI_SetColor(GUI_WHITE);
        GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_VCENTER);
        ...
        // --- 2. Iscrtavanje dinamiÄkog naziva scene (ako je odabrana) ---
        if (timer_selected_scene_index != -1) {
            ...
            if (scene_handle && scene_handle->is_configured) {
                ...
                // DefiniÅ¡emo KRAJNJU desnu poziciju za tekst, 5 piksela lijevo od dugmeta za odabir.
                const int16_t x_end_pos_dynamic = timer_settings_screen_layout.scene_select_btn_pos.x - 20;

                // Logika za skraÄ‡ivanje ostaje ista da sprijeÄi preklapanje sa lijevim tekstom.
                ...
                // Iscrtavamo tekst na izraÄunatoj KRAJNJOJ poziciji.
    ...
    if (BUTTON_IsPressed(hButtonTimerSave)) {
        ...
        //
        // // Timer_SetActionScene je uklonjen iz funkcije (stara linija iz VaÅ¡eg koda)
        ...
        timer_screen_initialized = false; // VraÄ‡eno kako je bilo u VaÅ¡em kodu
        DSP_KillSettingsTimerScreen();
        screen = SCREEN_TIMER;
        shouldDrawScreen = 1;
        Timer_Unsuppress();
...
/**
 ******************************************************************************
 * @brief       Servisira ekran za podeÅ¡avanje datuma i vremena.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija sadrÅ¾i kompletnu logiku za interakciju korisnika.
 * Koristi ne-blokirajuÄ‡i mehanizam za detekciju pritiska (edge detection)
 * kako bi osigurala da svaki pritisak na +/- dugme rezultira taÄno
 * jednim inkrementom/dekrementom. Za praÄ‡enje stanja dugmadi koristi
 * lokalni statiÄki niz `old_button_state`. Prilikom aÅ¾uriranja
 * vrijednosti na ekranu, primjenjuje princip "obriÅ¡i-pa-iscrtaj"
 * unutar fiksne zone TEXT widgeta kako bi se sprijeÄili vizuelni
 * ostaci (artifacts). Sva logika je sadrÅ¾ana unutar ove funkcije
 * bez uvoÄ‘enja novih globalnih varijabli.
 * @param       None
 * @retval      None
 ******************************************************************************
 */
static void Service_SettingsDateTimeScreen(void)
{
    // StatiÄke varijable za Äuvanje stanja unutar funkcije izmeÄ‘u poziva
    static int32_t values[5]; // Lokalni bafer za vrijednosti: 0:Dan, 1:Mjesec, 2:Godina, 3:Sat, 4:Minuta
    static bool initialized = false;
    static bool old_button_state[10] = {0}; // Niz za praÄ‡enje stanja za 5x2 dugmeta (+/-)

    // NOVE, ROBUSTNE VARIJABLE ZA "PRESS-AND-HOLD"
    static uint32_t press_start_time[10] = {0}; // Vrijeme kada je taster pritisnut
    static uint32_t next_trigger_time[10] = {0}; // Vrijeme kada je dozvoljen sljedeÄ‡i inkrement

    const uint32_t INITIAL_DELAY_MS = 500;    // Inicijalno kaÅ¡njenje od 0.5s
    const uint32_t REPEAT_RATE_MS = 350;      // Brzina ponavljanja (svakih 150ms)

    char value_buffer[6];

    // UÄitavanje poÄetnih vrijednosti iz RTC-a samo pri prvom ulasku na ekran
    if (!initialized)
    {
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (BUTTON_IsPressed(hButtonDateTimeUp[i])) {
            ...
            press_start_time[up_idx] = 0; // Resetuj na otpuÅ¡tanje
        }
        ...
        if (BUTTON_IsPressed(hButtonDateTimeDown[i])) {
            ...
            press_start_time[down_idx] = 0; // Resetuj na otpuÅ¡tanje
        }

        // Ako je vrijednost promijenjena, izvrÅ¡i validaciju i iscrtavanje
        if (value_changed) {
            BuzzerOn();
            HAL_Delay(1);
            BuzzerOff();
    ...
    if (BUTTON_IsPressed(hBUTTON_Ok)) {
        ...
        initialized = false; // Reset za sljedeÄ‡i ulazak na ekran
        DSP_KillSettingsDateTimeScreen();
        screen = SCREEN_RETURN_TO_FIRST;
//        screen = SCREEN_TIMER;
        shouldDrawScreen = 1;
 ...
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Koristi ispravan font `GUI_FontVerdana32_LAT` koji podrÅ¾ava slova
...
static void Service_AlarmActiveScreen(void)
{
    if (shouldDrawScreen) {
        ...
        // Dummy poziv za animaciju (kasnije moÅ¾ete implementirati pravu)
        GUI_DrawBitmap(&bmicons_security_sos, 380, 20);

        // KoriÅ¡tenje ispravnog fonta koji podrÅ¾ava slova
        GUI_SetFont(&GUI_FontVerdana32_LAT);
        GUI_SetColor(GUI_RED);
        GUI_SetTextAlign(GUI_TA_HCENTER | GUI_TA_VCENTER);
 ...
 * KONAÄŒNA ISPRAVKA: Logika iscrtavanja, ukljuÄujuÄ‡i dinamiÄki odabir
 * fonta i precizno dvoredno pozicioniranje teksta, sada je 100%
 * identiÄna referentnoj funkciji `Service_LightsScreen`. Svi parametri
 * za pozicioniranje se sada Äitaju iz `lights_and_gates_grid_layout`
 * strukture.
 ******************************************************************************
 */
static void Service_GateScreen(void)
{
    // Iscrtavanje se vrÅ¡i samo ako je eksplicitno zatraÅ¾eno.
    if (shouldDrawScreen) {
        shouldDrawScreen = 0;
        ...
        if (gate_count == 0) {
            ...
            // =======================================================================
            // === FAZA 1: PRE-KALKULACIJA I ODABIR FONTA (identiÄno kao kod svjetala) ===
            // =======================================================================
            const GUI_FONT* fontToUse = &GUI_FontVerdana20_LAT;
            const int text_padding = 10;
            bool downgrade_font = false;
            ...
            for(uint8_t i = 0; i < gate_count; ++i) {
                ...
                if (handle) {
                    ...
                    // === SIGURNOSNA PROVJERA #1 (Dio rjeÅ¡enja) ===
            ...
            for(uint8_t row = 0; row < rows; ++row) {
                ...
                for(uint8_t idx_in_row = 0; idx_in_row < gatesInRow; ++idx_in_row) {
                    ...
                    if (handle) {
                        ...
                        // === POÄŒETAK GLAVNE ISPRAVKE: SIGURNOSNA PROVJERA PRIJE CRTANJA ===
                        // Provjeravamo da li je appearance_id validan indeks za niz.
                        // Ako nije, preskaÄemo crtanje ove ikonice i nastavljamo sa sljedeÄ‡om,
                        // Äime se sprjeÄava pad sistema.
                        if (appearance_id < (sizeof(gate_appearance_mapping_table) / sizeof(IconMapping_t))) {
                            const IconMapping_t* mapping = &gate_appearance_mapping_table[appearance_id];
                            ...
                            // Druga sigurnosna provjera: da li je izraÄunati indeks za ikonicu validan.
                            uint16_t base_icon_index = (visual_icon_type - ICON_GATE_SWING) * 5;
 ...
 * dinamiÄkih informacija na ekranu. SadrÅ¾i internu logiku za
 * praÄ‡enje promjene stanja kapije (`old_state`) i automatski
 * pokreÄ‡e ponovno iscrtavanje kada se stanje promijeni. Iscrtava
 * veliku, centralnu ikonicu koja odgovara trenutnom stanju ureÄ‘aja,
...
static void Service_GateSettingsScreen(void)
{
    ...
    Gate_Handle* handle = Gate_GetInstance(gate_control_panel_index);
    if (handle == NULL) {
        return;         // IzaÄ‘i iz funkcije
    }

    // =========================================================================
    // === ISPRAVLJENA NE-BLOKIRAJUÄ†A LOGIKA ZA PROVJERU DUGMADI ===
    // =========================================================================
    for (int i = 0; i < 6; i++) {
        if (WM_IsWindow(hGateControlButtons[i])) {
            ...
            if (is_pressed && !old_button_state[i]) {
                ...
                shouldDrawScreen = 1; // ZatraÅ¾i ponovno iscrtavanje ODMAH!
                // =========================================================
    ...
    if (shouldDrawScreen) {
        shouldDrawScreen = 0;
        GUI_MULTIBUF_BeginEx(1);
        GUI_ClearRect(0, 0, DRAWING_AREA_WIDTH, 210); // ObriÅ¡i samo dinamiÄki dio

        uint8_t appearance_id = Gate_GetAppearanceId(handle);
        const IconMapping_t* mapping = &gate_appearance_mapping_table[appearance_id];

        // Iscrtavanje naziva ureÄ‘aja
        GUI_SetFont(&GUI_FontVerdana20_LAT);
        GUI_SetColor(GUI_WHITE);
        GUI_SetTextAlign(GUI_TA_HCENTER | GUI_TA_TOP);
        const char* custom_label = Gate_GetCustomLabel(handle);
        ...
        case GATE_STATE_OPEN:
            icon_state_index = 1;
            break;
        // ISPRAVKA: KoriÅ¡tenje indeksa za nove, specifiÄne ikonice
        case GATE_STATE_OPENING:
            icon_state_index = 2;
            break;
        case GATE_STATE_CLOSING:
        ...
        {
            // ISPRAVKA: Novi naÄin raÄunanja indeksa za blok od 5 ikonica
            uint16_t base_icon_index = (visual_icon_type - ICON_GATE_SWING) * 5;
 ...
 * TakoÄ‘e koristi "pametnu logiku" za prikaz korisniÄki definisanih naziva.
 ******************************************************************************
 */
static void Service_SecurityScreen(void)
{
    // Crtanje se izvrÅ¡ava samo ako je zatraÅ¾eno
    if (shouldDrawScreen) {
        shouldDrawScreen = 0;
...
/**
 ******************************************************************************
 * @brief       UniÅ¡tava sve dinamiÄki kreirane widgete sa ekrana za alarm.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija pronalazi i briÅ¡e svu dugmad kreiranu u 
 * `Service_SecurityScreen` kako bi se sprijeÄila pojava "duhova".
 ******************************************************************************
 */
static void DSP_KillSecurityScreen(void)
{
    WM_HWIN hItem;
    // Petlja briÅ¡e dugme "SYSTEM" (ID_USER + 0) i dugmad za particije (ID_USER + 1 do +3)
    for (int i = 0; i <= SECURITY_PARTITION_COUNT; i++) {
...
/**
 ******************************************************************************
 * @brief       Kreira i inicijalizuje GUI za ekran podeÅ¡avanja alarma (ISPRAVLJENA VERZIJA).
 * @author      Gemini & [VaÅ¡e Ime]
...
static void DSP_InitSettingsAlarmScreen(void)
{
    ...
    const int16_t y_gap = 53; // PoveÄ‡an razmak zbog visine reda
...
static void DSP_KillSettingsAlarmScreen(void)
{
    // Provjera i brisanje svakog handle-a pojedinaÄno radi sigurnosti
...
/**
 ******************************************************************************
 * @brief       Servisira ekran za podeÅ¡avanje alarma.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija provjerava pritiske na pet dugmadi. U zavisnosti
 * od pritisnutog dugmeta, pokreÄ‡e ili proceduru za promjenu
 * PIN-a (pozivom Numpad-a) ili proceduru za promjenu naziva
 * (pozivom alfanumeriÄke tastature).
 ******************************************************************************
 */
static void Service_SettingsAlarmScreen(void)
{
    ...
    if (BUTTON_IsPressed(hButtonChangePin))
    {
        ...
        shouldDrawScreen = 0;             // SprijeÄi ponovno crtanje u DISP_Service
        return; 
    }
    ...
    if (BUTTON_IsPressed(hButtonSystemName))
    {
        ...
        shouldDrawScreen = 0;             // SprijeÄi ponovno crtanje
        return;
    }
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje kratak pritisak na ikonicu Alarma na SelectScreen2.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija se poziva iz `HandleTouchReleaseEvent`. Provjerava da li
 * je tajmer za alarm i dalje aktivan. Ako jeste, to znaÄi da dugi
 * pritisak nije detektovan, te se izvrÅ¡ava akcija za kratak pritisak:
 * osvjeÅ¾avanje stanja i prelazak na `SCREEN_SECURITY`.
 ******************************************************************************
 */
static void HandleRelease_AlarmIcon(void)
{
    if (dynamic_icon_alarm_press_timer != 0)
    {
        ...
        dynamic_icon_alarm_press_timer = 0;
        // 1. OsvjeÅ¾i stanje sa hardvera.
        Security_RefreshState();
        
        // 2. AÅ¾uriraj lokalni UI status na osnovu realnog stanja.
        alarm_ui_state[0] = Security_IsAnyPartitionArmed() ? ALARM_UI_STATE_ARMED : ALARM_UI_STATE_DISARMED;
        for (int i = 0; i < SECURITY_PARTITION_COUNT; i++) {
            alarm_ui_state[i + 1] = Security_GetPartitionState(i) ? ALARM_UI_STATE_ARMED : ALARM_UI_STATE_DISARMED;
        }

        // 3. PreÄ‘i na glavni ekran alarma.
        screen = SCREEN_SECURITY;
        shouldDrawScreen = 1;
    }
}
...
/**
 ******************************************************************************
 * @brief       ObraÄ‘uje kratak pritisak na ikonicu Tajmera na SelectScreen2.
 * @author      Gemini & [VaÅ¡e Ime]
 * @note        Ova funkcija se poziva iz `HandleTouchReleaseEvent`. Provjerava da li
 * je tajmer za tajmer i dalje aktivan. Ako jeste, izvrÅ¡ava akciju za
