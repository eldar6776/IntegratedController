...
#define LSE_RESTART_ATTEMPTS                10      // Broj poku�aja prije nego �to predemo na LSI
...
// Defini�emo globalni fleg i postavljamo ga na `false` kao pocetno stanje.
...
int main(void) 
{
    ...
    while(1) {
        ...
        Scene_Service();
        Timer_Service();
        RS485_Service(); // prvo sve obradi pa �alji
        Buzzer_Service();
...
#define HIGH_PRECISION_THRESHOLD    2U  // Visoka osjetljivost (precizno za slajdere)
    ...
    if (IsDISPCleaningActiv()) return;
        ...
        // 1. Defini�emo lokalnu varijablu za treshold.
        uint8_t threshold;
...
/**
  ...
  * dio specificnih modula, kao �to su sistemski flegovi (dijeljeni sa
...
static void RAM_Init(void) 
{
    ...
    // Provjera da li je PIN validan (mora sadržavati samo cifre 0-9)
    bool pin_valid = true;
...
/**
  * @brief inicijalizacija oscilatora sa provjerom starta LSE i ako otka�e prelazak na LSI
  * @param
  * @retval
  */
static void SystemClock_Config(void) {
...
/**
  ...
  * sacuvana, dok je logika za odlucivanje (histereza) premje�tena u termostat modul.
  * @param  None
  * @retval None
  */
static void ADC3_Read(void) {
    ...
    // Lokalne staticke varijable za filtriranje ocitanja (Va�a originalna logika).
    ...
    if ((HAL_GetTick() - adctmr) >= ADC_READOUT_PERIOD) {
        ...
        // Inicijalizacija filtera pri prvom pokretanju (Va�a originalna logika).
        if (first_run) {
            first_run = false;
            uint32_t tmp_init = 0;
        ...
        // Uzimanje novog uzorka sa ADC-a i a�uriranje bafera za prosjek.
        HAL_ADC_Start(&hadc3);
        HAL_ADC_PollForConversion(&hadc3, 10);
        ...
        if ((tmp_avg < 100) || (tmp_avg > 4000)) {
            ...
            // Primjena eksponencijalnog filtera za "peglanje" vrijednosti (Va�a originalna logika).
...
void CheckRTC_Clock(void) {
    ...
    // Provjeri da li je pro�lo dovoljno vremena za novu provjeru
    if ((HAL_GetTick() - lastCheckTime) >= LSE_TIMEOUT) {
        lastCheckTime = HAL_GetTick();  // A�uriraj vrijeme posljednje provjere
        ...
        if (!LSE_Failed) {
            ...
            // A�uriraj posljednje sekundno ocitavanje
